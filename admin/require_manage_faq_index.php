...
/* 取得發佈者資料 */
$colname_RecordFaqListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordFaqListAuthor = $_GET['lang'];
}
$coluserid_RecordFaqListAuthor = "-1";
if (isset($w_userid)) {
  $coluserid_RecordFaqListAuthor = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordFaqListAuthor = sprintf("SELECT * FROM demo_faqitem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordFaqListAuthor, "text"),GetSQLValueString($coluserid_RecordFaqListAuthor, "int"));
$RecordFaqListAuthor = mysqli_query($DB_Conn, $query_RecordFaqListAuthor) or die(mysqli_error($DB_Conn)); 
$row_RecordFaqListAuthor = mysqli_fetch_assoc($RecordFaqListAuthor);
$totalRows_RecordFaqListAuthor = mysqli_num_rows($RecordFaqListAuthor);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/faq_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> <?php echo $ModuleName['Faq']; ?> <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
do {  
?>
                  <option value="<?php echo $row_RecordFaqListType['itemname']?>"><?php echo $row_RecordFaqListType['itemname']?></option>
                  <?php
...
mysqli_free_result($RecordFaqListAuthor);
?>

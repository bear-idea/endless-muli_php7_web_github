...
$query_RecordCount = sprintf("SELECT count(id) as sum FROM demo_tmpleftmenu WHERE userid=%s",GetSQLValueString($coluserid_RecordTmpLeftMenu, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);


$query_RecordTmpLeftMenu = sprintf("SELECT * FROM demo_tmpleftmenu WHERE name LIKE %s && indicate LIKE %s && type LIKE %s && userid=%s $orderSql", GetSQLValueString("%" . $colsearch_RecordTmpLeftMenu . "%", "text"), GetSQLValueString($colindicate_RecordTmpLeftMenu, "text"), GetSQLValueString($coltype_RecordTmpLeftMenu, "text"),GetSQLValueString($coluserid_RecordTmpLeftMenu, "int"));
...
<?php do { ?>
  <?php 
	...
	$dvalue['pic'] = "<span class='label label-purple' data-original-title='版型編號' data-toggle='tooltip' data-placement='right'>#No.".$row_RecordTmpLeftMenu['id']."</span>"."<div class='img-thumbnail m-b-5 m-t-5 overflow-x-hidden overflow-y-hidden'><div class='' style='width:100px;height:100px;'>".$left_menu."</div></div>";
  
    $link_add = "manage_tmp.php?wshop=".$wshop."&amp;Opt=tmpaddleftmenu&amp;lang=".$_SESSION['lang'];
	
	$link_edit = "manage_tmp.php?wshop=".$wshop."&amp;Opt=tmpeditleftmenu&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordTmpLeftMenu['id'];
	...
	$but_del = "<a href='".""."' class='btn btn-xs btn-danger' style='text-align:center' onclick='delete_datatables(".$row_RecordTmpLeftMenu["id"].",\"".""."\",event);'><i class='far fa-trash-alt'></i> 刪除</a>";
	...
	if ($row_RecordTmpLeftMenu['userid'] == $w_userid) { 
		$dvalue['name'] = "<span id='name_".$row_RecordTmpLeftMenu["id"]."' class='ed_name' data-type='text' data-pk='".$row_RecordTmpLeftMenu["id"]."' data-placement='top'>".$row_RecordTmpLeftMenu["name"]."</span>";
	}else{
		$dvalue['name'] = $row_RecordTmpLeftMenu["name"];
	}
	
	if ($row_RecordTmpLeftMenu['userid'] == $w_userid) { 
		$dvalue['type'] = "<span id='type_".$row_RecordTmpLeftMenu["id"]."' class='ed_type' data-type='select' data-pk='".$row_RecordTmpLeftMenu["id"]."' data-placement='top'>".$row_RecordTmpLeftMenu["type"]."</span>";
	}else{

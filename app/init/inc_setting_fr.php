...
use App\Eloquent\Setting_otr;
use App\Eloquent\Tmp;
use App\Eloquent\Tmpbackground;
use App\Eloquent\Tmpboard;
use App\Eloquent\Tmpleftmenu;
use App\Eloquent\Tmpmainmenu;
use App\Eloquent\Tmplogo;
use App\Eloquent\Tmpblock;
use Carbon\Carbon;
?>
<?php
...
if($_SERVER['HTTP_HOST'] != "www.shop3500.com")
{
	...
	*/
	...
	foreach ($GetUrlAry as $value) {	
		...
		$colurl['AccountPlatform'] = "-1";
		if (isset($value)) {
		  $colurl['AccountPlatform'] = $value;
		}

		$row_RecordAccountPlatform = Admin::select(['webname'])
		  ->where('urllink', '=', $colurl['AccountPlatform'])
		  ->orWhere('urllink', '=', $colurl['AccountPlatform'])
		  ->first();
		
		if($row_RecordAccountPlatform != NULL)
		{
			$_GET['wshop'] = $row_RecordAccountPlatform['webname'];
		}
	}
}
/* =================================================================================================================*/
...
// 指定單一網站
if(isset($_GET['Opt'])){}else{$_GET['Opt']=NULL;}
...
$colwebname['Account'] = "-1";
if (isset($_GET['wshop'])) {
  $colwebname['Account'] = $_GET['wshop'];
}

$row_RecordAccount = Admin::select($columns)
  ->where('webname', '=', $colwebname)
  ->first();

/* --------- BOT CHECK --------*/
$_SESSION['Bot_Check_Value'] = true;
/* =================================================================================================================*/
if (isset($row_RecordAccount) && $row_RecordAccount != NULL) { // 若搜尋到網域資料則執行下列程式
/* =================================================================================================================*/
/* ----------店家選擇---------- */
$_SESSION['userid'] = $row_RecordAccount['id'];
/* ----------店家選擇---------- */	

$coluserid['SystemConfig'] = "-1";
if (isset($_SESSION['userid'])) {
  $coluserid['SystemConfig'] = $_SESSION['userid'];
}
	
$row_RecordSystemConfig = Setting::select($columns)
  ->where('userid', '=', $coluserid['SystemConfig'])
  ->first();
	
$coluserid['SystemConfigFr'] = "-1";
if (isset($_SESSION['userid'])) {
  $coluserid['SystemConfigFr'] = $_SESSION['userid'];
}
	
$row_RecordSystemConfigFr = Setting_fr::select($columns)
  ->where('userid', '=', $coluserid['SystemConfigFr'])
  ->first();

	
$coluserid['SystemConfigOtr'] = "-1";
if (isset($_SESSION['userid'])) {
  $coluserid['SystemConfigOtr'] = $_SESSION['userid'];
}
	
$row_RecordSystemConfigOtr = Setting_otr::select($columns)
  ->where('userid', '=', $coluserid['SystemConfigOtr'])
  ->first();
...
$coluserid['TmpConfig'] = "-1";
if (isset($_SESSION['userid'])) {
  $coluserid['TmpConfig'] = $_SESSION['userid'];
}
	
$row_RecordTmpUsedid = Setting_fr::select([$SQL_MSTmpSelect])
  ->where('userid', '=', $coluserid['TmpConfig'])
  ->first();

$row_RecordTmpConfig = Tmp::select($columns)
  ->where('id', '=', $row_RecordTmpUsedid[$SQL_MSTmpSelect])
  ->first();
	
$row_RecordTmpHeaderBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpheaderbackground'])
  ->first();
	
$row_RecordTmpWrpBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpwrpbackground'])
  ->first();

$row_RecordTmpLeftBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpleftbackground'])
  ->first();

$row_RecordTmpRightBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmprightbackground'])
  ->first();

$row_RecordTmpMiddleBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpmiddlebackground'])
  ->first();
	
$row_RecordTmpFooterBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpfooterbackground'])
  ->first();
	
$row_RecordTmpHomeBoard = Tmpboard::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmphomeboard'])
  ->first();
	
$row_RecordTmpWrpBoard = Tmpboard::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpwrpboard'])
  ->first();
	
$row_RecordTmpBannerBoard = Tmpboard::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpbannerboard'])
  ->first();
	
$row_RecordTmpViewlineBoard = Tmpboard::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpviewlineboard'])
  ->first();
		
$row_RecordTmpMiddleBoard = Tmpboard::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpmiddleboard'])
  ->first();
	
$row_RecordTmpTitleBoard = Tmpboard::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmptitleboard'])
  ->first();

$row_RecordTmpTitleBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmptitlebackground'])
  ->first();
	
$row_RecordTmpTitleLineBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmptitlelinebackground'])
  ->first();
	
$row_RecordTmpBodyBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpbodybackground'])
  ->first();
	
$row_RecordTmpLeftMenu = Tmpleftmenu::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpleftmenu'])
  ->first();
	
$row_RecordTmpAnimeBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpanimebackground'])
  ->first();

$row_RecordTmpNewsEvenBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpnewsevenbackground'])
  ->first();
	
$row_RecordTmpNewsOddBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpnewsoddbackground'])
  ->first();
	
$row_RecordTmpMainMenu = Tmpmainmenu::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpmainmenu'])
  ->first();
	
$row_RecordTmpLogo = Tmplogo::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmplogoid'])
  ->first();
	
$row_RecordTmpHomeLogo = Tmplogo::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmphomelogoid'])
  ->first();

$row_RecordTmpLogoDefault = Tmplogo::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['TmpLogoID'])
  ->first();
	
$row_RecordTmpBlock = Tmpblock::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpblock'])
  ->first();
	
$row_RecordTmpBottomBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmpbottombackground'])
  ->first();
	
$row_RecordTmpHomeBodyBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmphomebodybackground'])
  ->first();

$row_RecordTmpHomeAnimeBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmphomeanimebackground'])
  ->first();
	
$row_RecordTmpHomeBottomBg = Tmpbackground::select($columns)
  ->where('id', '=', $row_RecordTmpConfig['tmphomebottombackground'])
  ->first();
...
/* ----------功能啟用設定---------- */
$OptionModuleSelect['Tmp'] = $OptionTmpSelect = $row_RecordSystemConfig['OptionTmpSelect']; // 版型修改是否啟用
$OptionModuleSelect['News'] = $OptionNewsSelect = $row_RecordSystemConfig['OptionNewsSelect']; // 最新訊息是否啟用
$OptionModuleSelect['Letters'] = $OptionLettersSelect = $row_RecordSystemConfig['OptionLettersSelect']; // 新聞快報是否啟用
$OptionModuleSelect['Actnews'] = $OptionActnewsSelect = $row_RecordSystemConfig['OptionActnewsSelect']; // 活動快訊是否啟用
$OptionModuleSelect['Faq'] = $OptionFaqSelect = $row_RecordSystemConfig['OptionFaqSelect']; // 常見問答是否啟用
$OptionModuleSelect['Product'] = $OptionProductSelect = $row_RecordSystemConfig['OptionProductSelect']; // 產品功能是否啟用
$OptionModuleSelect['Meeting'] = $OptionMeetingSelect = $row_RecordSystemConfig['OptionMeetingSelect']; // 會議紀錄是否啟用
$OptionModuleSelect['Sponsor'] = $OptionSponsorSelect = $row_RecordSystemConfig['OptionSponsorSelect']; // 贊助企業是否啟用
$OptionModuleSelect['Frilink'] = $OptionFrilinkSelect = $row_RecordSystemConfig['OptionFrilinkSelect']; // 友站連結是否啟用
$OptionModuleSelect['Otrlink'] = $OptionOtrlinkSelect = $row_RecordSystemConfig['OptionOtrlinkSelect']; // 相關連結是否啟用
$OptionModuleSelect['Careers'] = $OptionCareersSelect = $row_RecordSystemConfig['OptionCareersSelect']; // 求職徵才是否啟用
$OptionModuleSelect['Publish'] = $OptionPublishSelect = $row_RecordSystemConfig['OptionPublishSelect']; // 公佈資訊是否啟用
$OptionModuleSelect['Guestbook'] = $OptionGuestbookSelect = $row_RecordSystemConfig['OptionGuestbookSelect']; // 留言管理是否啟用
$OptionModuleSelect['Member'] = $OptionMemberSelect = $row_RecordSystemConfig['OptionMemberSelect']; // 會員資料是否啟用
$OptionModuleSelect['Activities'] = $OptionActivitiesSelect = $row_RecordSystemConfig['OptionActivitiesSelect']; // 活動花絮是否啟用
$OptionModuleSelect['Project'] = $OptionProjectSelect = $row_RecordSystemConfig['OptionProjectSelect']; // 工程實績是否啟用
//$OptionModuleSelect['Ads'] = $OptionAdsSelect = $row_RecordSystemConfig['OptionAdsSelect']; // 廣告輪播是否啟用
//$OptionModuleSelect['Donation'] = $OptionDonationSelect = $row_RecordSystemConfig['OptionDonationSelect']; // 捐款名錄是否啟用
$OptionModuleSelect['About'] = $OptionAboutSelect = $row_RecordSystemConfig['OptionAboutSelect']; // 關於我們是否啟用
$OptionModuleSelect['Article'] = $OptionArticleSelect = $row_RecordSystemConfig['OptionArticleSelect']; // 文章管理是否啟用
$OptionModuleSelect['DfPage'] = $OptionDfPageSelect = $row_RecordSystemConfig['OptionDfPageSelect']; // 自訂頁面是否啟用
$OptionModuleSelect['About'] = $OptionAboutSelect = $row_RecordSystemConfig['OptionAboutSelect']; // 關於我們是否啟用
$OptionModuleSelect['Contact'] = $OptionContactSelect = $row_RecordSystemConfig['OptionContactSelect']; // 聯絡我們是否啟用
$OptionModuleSelect['Catalog'] = $OptionCatalogSelect = $row_RecordSystemConfig['OptionCatalogSelect']; // 產品型錄是否啟用
$OptionModuleSelect['Knowledge'] = $OptionKnowledgeSelect = $row_RecordSystemConfig['OptionKnowledgeSelect']; // 知識學習是否啟用
$OptionModuleSelect['Cart'] = $OptionCartSelect = $row_RecordSystemConfig['OptionCartSelect']; // 購物車功能是否啟用
//$OptionModuleSelect['Tickets'] = $OptionTicketsSelect = $row_RecordSystemConfig['OptionTicketsSelect']; // 訂票系統是否啟用
$OptionModuleSelect['Org'] = $OptionOrgSelect = $row_RecordSystemConfig['OptionOrgSelect']; // 成員幹部是否啟用
//$OptionModuleSelect['FileMang'] = $OptionFileMangSelect = $row_RecordSystemConfig['OptionFileMangSelect']; // 檔案管理是否啟用
//$OptionModuleSelect['Analysis'] = $OptionAnalysisSelect = $row_RecordSystemConfig['OptionAnalysisSelect']; // 統計資料是否啟用
//$OptionModuleSelect['WebSite'] = $OptionWebSiteSelect = $row_RecordSystemConfig['OptionWebSiteSelect']; // 網站資訊是否啟用
$OptionModuleSelect['EPaper'] = $OptionEPaperSelect = $row_RecordSystemConfig['OptionEPaperSelect']; // 電子期刊是否啟用
$OptionModuleSelect['Blog'] = $OptionBlogSelect = $row_RecordSystemConfig['OptionBlogSelect']; // 部落格是否啟用
//$OptionModuleSelect['Picasa'] = $OptionPicasaSelect = $row_RecordSystemConfig['OptionPicasaSelect']; // 雲端相簿是否啟用
$OptionModuleSelect['Partner'] = $OptionPartnerSelect = $row_RecordSystemConfig['OptionPartnerSelect']; // 合作夥伴是否啟用
$OptionModuleSelect['Forum'] = $OptionForumSelect = $row_RecordSystemConfig['OptionForumSelect']; // 討論專區是否啟用
$OptionModuleSelect['Artlist'] = $OptionArtlistSelect = $row_RecordSystemConfig['OptionArtlistSelect']; // 藝文專欄是否啟用
$OptionModuleSelect['Video'] = $OptionVideoSelect = $row_RecordSystemConfig['OptionVideoSelect']; // 影片管理是否啟用
//$OptionModuleSelect['Coupons'] = $OptionCouponsSelect = $row_RecordSystemConfig['OptionCouponsSelect']; // 優惠票卷是否啟用
$OptionModuleSelect['Timeline'] = $OptionTimelineSelect = $row_RecordSystemConfig['OptionTimelineSelect']; // 歷史沿革是否啟用
$OptionModuleSelect['Imageshow'] = $OptionImageshowSelect = $row_RecordSystemConfig['OptionImageshowSelect']; // 圖片展示是否啟用
$OptionModuleSelect['Album'] = $OptionAlbumSelect = $row_RecordSystemConfig['OptionAlbumSelect']; // 相簿管理是否啟用
$OptionModuleSelect['Stronghold'] = $OptionStrongholdSelect = $row_RecordSystemConfig['OptionStrongholdSelect']; // 經營據點是否啟用
$OptionModuleSelect['Home'] = $OptionTmpHomeSelect = $row_RecordSystemConfig['OptionTmpHomeSelect']; // 版型修改首頁是否啟用
$OptionModuleSelect['Room'] = $OptionRoomSelect = $row_RecordSystemConfig['OptionRoomSelect']; // 房型展示是否啟用
$OptionModuleSelect['Attractions'] = $OptionAttractionsSelect = $row_RecordSystemConfig['OptionAttractionsSelect']; // 鄰近景點是否啟用
$OptionModuleSelect['Mobile'] = $OptionMobileSelect = $row_RecordSystemConfig['OptionMobileSelect']; // 行動裝置是否啟用
$OptionModuleSelect['Dealer'] = $OptionDealerSelect = $row_RecordSystemConfig['OptionDealerSelect']; // 經銷商資料是否啟用
$OptionModuleSelect['SocialChat'] = $OptionSocialChatSelect = $row_RecordSystemConfig['OptionSocialChatSelect']; // 社群聊天是否啟用
$OptionModuleSelect['Booking'] = $OptionBookingSelect = $row_RecordSystemConfig['OptionBookingSelect']; // 預約系統是否啟用

/* ----------地磅系統啟用設定---------- */
$OptionModuleSelect['ScaleSource'] = $OptionScaleSourceSelect = $row_RecordSystemConfig['OptionScaleSourceSelect']; // 貨源管理是否啟用
$OptionModuleSelect['SplitOrder'] = $OptionSplitOrderSelect = $row_RecordSystemConfig['OptionSplitOrderSelect']; // 物料拆分是否啟用
$OptionModuleSelect['ScaleClearance'] = $OptionScaleClearanceSelect = $row_RecordSystemConfig['OptionScaleClearanceSelect']; // 清運明細是否啟用
...
if ($row_RecordAccount['webrenewdate'] != '' && $row_RecordAccount['usetime'] != '') {
	$t_end = Carbon::parse($row_RecordAccount['webrenewdate'])->addYears($row_RecordAccount['usetime']); // 最後續約時間+N年
	$t_now = Carbon::now(); // 目前時間
	// 判断第一个日期是否比第二个日期大
	if($t_now->gt($t_end)) {
...
if (isset($row_RecordAccount) && $row_RecordAccount == NULL) {
...
// 取得assetsurl
if ($SiteBaseUrlOuter != "") {
    $SiteAsstesUrl = $SiteBaseUrlOuter;
} else {
    $SiteAsstesUrl = $SiteBaseUrl;
}
?>
<?php
// 可用來判斷找不到頁面發生情形!!

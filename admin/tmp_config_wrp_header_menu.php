...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Tmp")) {
  $updateSQL = sprintf("UPDATE demo_tmp SET tmpmainmenunowidth=%s, tmpdftmenu_x=%s, tmpdftmenu_y=%s, tmppicmenu_x=%s, tmppicmenu_y=%s, tmppicmenu_style=%s, tmpdfmenucolor=%s, tmpmenuselect=%s, tmpmainmenunowidth=%s WHERE id=%s",
                       GetSQLValueString($_POST['tmpmainmenunowidth'], "int"),
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post" id="form_Tmp" name="form_Tmp">
      <div class="form-group row">
        ...
        <div class="col-md-12"><span class="badge badge-primary"><i class="fa fa-paper-plane"></i> 主選單 <i class="fa fa-info-circle text-white" data-original-title="此選項會將主選單做隱藏，您可在《自訂欄位》中加入《選單總覽》的區塊，並將您的選單區塊作隱藏，如此可將您的選單移至左邊。" data-toggle="tooltip" data-placement="top"></i></span></div>
      </div>
  	  <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordTmp['tmpmenuselect'],"1"))) {echo "checked=\"checked\"";} ?> type="radio" name="tmpmenuselect" id="tmpmenuselect_2" value="1" />
                <label for="tmpmenuselect_2">自訂選單</label>
            </div>
            
            <div class="table-responsive">
            <div style="border:1px #CCCCCC dotted; width:600px; margin-left:25px; margin-top:5px; padding:10px;">
            <div style="border:1px #CCCCCC dotted; width:580px; position:relative; height:150px;">
            	<div style="color: #CCC; font-size: 30px; font-weight: bolder; padding: 10px; position: absolute; right: 50px; top: 50px;border:#CCC dotted 1px; width:350px;">選單<span style="font-size:16px;"> <a href="tmpmainmenu_home.php?id=<?php echo $row_RecordTmp['id']; ?>" class="btn btn-info">主選單選擇</a></span></div>
                <div style="padding: 5px; position: absolute; right: -55px; top: 8px; width: 268px;">
                    <div class="input-group p-0">
            
                      <div class="input-group-prepend"><span class="input-group-text">距離上方 <i class="fa fa-info-circle text-black" data-original-title="若您選擇的選單位置是鎖定此設定不會生效，即選單位置為《主選單置於頁首區塊和橫幅區塊之間》。" data-toggle="tooltip" data-placement="top"></i></span></div>
                          <input name="tmpdftmenu_y" id="tmpdftmenu_y" value="<?php echo $row_RecordTmp['tmpdftmenu_y']; ?>" size="3" class="form-control" maxlength="4" type="number" data-parsley-type="number" data-parsley-trigger="blur" step="1"/>
                              <div class="input-group-append"><span class="input-group-text">px</span></div>                
                    </div>     
    
                </div>
                <div style="padding: 5px; position: absolute; right: -233px; top: 96px; width: 268px;">
                <div class="input-group p-0">
            
                      <div class="input-group-prepend"><span class="input-group-text">距離右方 <i class="fa fa-info-circle text-black" data-original-title="若您選擇的選單位置是鎖定此設定不會生效，即選單位置為《主選單置於頁首區塊和橫幅區塊之間》。" data-toggle="tooltip" data-placement="top"></i></span></div>
                          <input name="tmpdftmenu_x" id="tmpdftmenu_x" value="<?php echo $row_RecordTmp['tmpdftmenu_x']; ?>" size="3" class="form-control" maxlength="4" type="number" data-parsley-type="number" data-parsley-trigger="blur" step="1"/>
                              <div class="input-group-append"><span class="input-group-text">px</span></div>                
                    </div>
                </div>
                <div style="position: absolute; right: 225px; top: 3px;"><img src="images/z_w.png" width="44" height="44" /></div>
                <div style="position: absolute; right: 4px; top: 64px;"><img src="images/z_h.png" alt="" width="44" height="44" /></div>
                <span style="color: #CCC; font-size: 30px; font-weight: bolder; padding: 10px; position: absolute; left: 5px; top: 5px;">頁首區塊</span>
            </div>
        </div>
        </div>
      ...
      <div class="form-group row">
          <label class="col-md-2 col-form-label">是否顯示<span class="text-red">*</span><i class="fa fa-info-circle text-orange" data-original-title="不限制寬度文字可能會有跑出選單樣式外情況，請依據選單外觀及文字內容長度做搭配。" data-toggle="tooltip" data-placement="top"></i></label>                       	
          <div class="col-md-10">
          
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordTmp['tmpmainmenunowidth'],"0"))) {echo "checked=\"checked\"";} ?> type="radio" name="tmpmainmenunowidth" id="tmpmainmenunowidth_2" value="0" />
                <label for="tmpmainmenunowidth_2">使用預設值</label>
            </div>
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordTmp['tmpmainmenunowidth'],"1"))) {echo "checked=\"checked\"";} ?> type="radio" name="tmpmainmenunowidth" id="tmpmainmenunowidth_1" value="1" />
                <label for="tmpmainmenunowidth_1">寬度不設限</label>
            </div>

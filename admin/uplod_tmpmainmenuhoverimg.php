...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Tmp")) {
  $updateSQL = sprintf("UPDATE demo_tmpmainmenu SET tmp_mainmenu_hover_img=IFNULL(%s,tmp_mainmenu_hover_img) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/tmpmainmenu/' . $_POST['oldpic']);
  //@unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/tmp/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_tmpmainmenuhoverimg.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmp = sprintf("SELECT * FROM demo_tmpmainmenu WHERE id = %s", GetSQLValueString($colname_RecordTmp, "int"));
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordTmp['tmp_mainmenu_hover_img']; ?>" />
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 3000,  
		<?php if ($row_RecordTmp['tmp_mainmenu_hover_img'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/tmpmainmenu/<?php echo $row_RecordTmp['tmp_mainmenu_hover_img']; ?>"],
		<?php } ?>

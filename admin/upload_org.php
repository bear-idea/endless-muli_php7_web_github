...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/org";
	$ppu->extensions = "GIF,JPG,JPEG,BMP,PNG";
	$ppu->formName = "form_Org";
	$ppu->storeType = "file";
	$ppu->sizeLimit = "3000";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Org")) {
  $updateSQL = sprintf("UPDATE demo_org SET avatar=IFNULL(%s,avatar) WHERE id=%s",
...
if (isset($_GET['id_edit'])) {
  $colname_RecordOrg = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordOrg = sprintf("SELECT * FROM demo_org WHERE id = %s", GetSQLValueString($colname_RecordOrg, "int"));
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordOrg['id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordOrg['avatar']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Org" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/org/<?php echo $row_RecordOrg['avatar']; ?>"],
		<?php } ?>

...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "") && $_GET['userid'] == $w_userid) {
  ...
  $deleteGoTo = "tmp_config_" . $_GET['board'] . "_column_en.php?lang=" . $_SESSION['lang'] . "&board=" . $_GET['board'] . "&id_edit=" . $_GET['id_edit'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSettingLock = sprintf("SELECT HomeproductlistLock_en, HomefrilinkLock_en, HomemodlinkLock_en, HomearticlelistLock_en, HomefbfanLock_en, HomealllistLock_en, HomenewslistLock_en, HomesiteviewcountLock_en, HomesitewhoscountLock_en FROM demo_setting_fr WHERE userid = %s", GetSQLValueString($coluserid_RecordSettingLock, "text"));
  ...
  <div class="panel-body">
    <div class="table-responsive">
        <table class="table table-hover m-b-0">
			...
			<?php do { ?>
            <?php if (
		  ($row_RecordTmpAddHomeColumn['type'] == 'sitewhoscount' && ($row_RecordSettingLock['HomesitewhoscountLock_en'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'siteviewcount' && ($row_RecordSettingLock['HomesiteviewcountLock_en'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'frilink' && ($row_RecordSettingLock['HomefrilinkLock_en'] == '1' OR $OptionFrilinkSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'modlink' && ($row_RecordSettingLock['HomemodlinkLock_en'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'productlist' && ($row_RecordSettingLock['HomeproductlistLock_en'] == '1' OR $OptionProductSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'articlelist' && ($row_RecordSettingLock['HomearticlelistLock_en'] == '1' OR $OptionArticleSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'fbfan' && ($row_RecordSettingLock['HomefbfanLock_en'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'alllist' && ($row_RecordSettingLock['HomealllistLock_en'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'newslist' && ($row_RecordSettingLock['HomenewslistLock_en'] == '1' OR $OptionNewsSelect == '0'))
		  ) { ?>
          <?php } else { ?>
          <form name="TmpColumn" action="<?php echo $editFormAction; ?>" method="POST" id="TmpColumn<?php echo $row_RecordTmpAddHomeColumn['type']; ?>">
             ...
             <input name="dftname_en" type="hidden" id="dftname_en" value="<?php echo $row_RecordTmpAddHomeColumn['dftname_en']; ?>" />

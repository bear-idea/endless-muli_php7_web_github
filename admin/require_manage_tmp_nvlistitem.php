...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_TmpItemAdd")) {
  $insertSQL = sprintf("INSERT INTO demo_tmpitem (list_id, itemname, itemvalue, lang, userid) VALUES (%s, %s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_TmpItemEdit")) {
	foreach($_POST['item_id'] as $key => $val){
	  $updateSQL = sprintf("UPDATE demo_tmpitem SET list_id=%s, itemname=%s, itemvalue=%s, lang=%s WHERE item_id=%s",
...
$coluserid_RecordTmpListItem = "-1";
if (isset($w_userid)) {
  $coluserid_RecordTmpListItem = $w_userid;
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpListItem = sprintf("SELECT demo_tmpitem.item_id, demo_tmpitem.userid, demo_tmplist.list_id, demo_tmplist.listname, demo_tmpitem.itemname, demo_tmpitem.lang FROM demo_tmplist LEFT OUTER JOIN demo_tmpitem ON demo_tmplist.list_id = demo_tmpitem.list_id WHERE demo_tmplist.list_id = %s && demo_tmpitem.lang=%s && demo_tmpitem.userid=%s", GetSQLValueString($collistid_RecordTmpListItem, "int"),GetSQLValueString($collang_RecordTmpListItem, "text"),GetSQLValueString($coluserid_RecordTmpListItem, "int"));
      ...
      <?php do { ?>
        <tr>
          ...
          <td><label for="itemname[]"></label>
            ...
            值：
            <input name="itemvalue[]" type="text" id="itemvalue[]" value="<?php echo $row_RecordTmpListItem['itemvalue']; ?>" />
            刪除：
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="TB_General_style01">
                ...
                <tr>
                      ...
                      <span class="textfieldRequiredMsg">欄位不可為空。</span></span>                        
                        ...
                        <span class="textfieldRequiredMsg">欄位不可為空。</span><span class="textfieldInvalidFormatMsg">格式無效。</span></span>
...
var sprytextfield2 = new Spry.Widget.ValidationTextField("TmpValue", "currency", {validateOn:["blur"]});
</script>
<?php

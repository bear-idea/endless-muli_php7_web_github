...
/* 取得類別資料 */
$colname_RecordScale = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordScale = $_GET['lang'];
}
...
$query_RecordScale = sprintf("SELECT * FROM erp_scale WHERE lang=%s && userid=%s", GetSQLValueString($colname_RecordScale, "text"),GetSQLValueString($coluserid_RecordScale, "int"));
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/scaleorder_out_datatable_overweight.js"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 過磅明細 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
  ...
  <div class="panel-body">
    <div class="row justify-content-end">
      <div class="col-md-3 col-sm-12 m-b-10">
          <div class="input-group" data-column="5">
              <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 日期</span></span>
              <input name="postdate" type="text" class="form-control search_filter" placeholder="" id="col5_filter" autocomplete="off"/>		
          </div>
      </div>
      <div class="col-md-2 col-sm-12 m-b-10">
        <div class="input-group" data-column="0"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 物料</span></span>
            <select name="scale" class="form-control search_filter" id="col0_filter">
            <option value="" selected="selected">全部</option>
              <?php
do {  
?>
                  <option value="<?php echo $row_RecordScale['name']?>"><?php echo $row_RecordScale['name']?></option>
                  <?php
...
?>
      ...
      <div class="col-md-3 m-b-10">
        <div class="input-group"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 流水號</span></span>
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          <th width="-1" data-priority="1"><strong>物料</strong></th>
          <th width="100" data-priority="1"><strong>流水號</strong></th>
          ...
          <th width="200" data-priority="1"><strong>入庫時間</strong></th>
        </tr>
      </thead>
      <tfoot>
        <tr>
...
$(document).ready(function() {
  $('input[name="postdate"]').daterangepicker({
	...
	ranges: {
		   ...
		   '上上個月': [moment().subtract(2, 'month').startOf('month'), moment().subtract(1, 'month').endOf('month')],

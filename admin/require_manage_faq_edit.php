...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Faq") && $_POST['sdescription'] == "") { 
	$_POST['sdescription'] = TrimSummary($_POST['content']);
}
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Faq")) {
  $updateSQL = sprintf("UPDATE demo_faq SET title=%s, type=%s, content=%s, postdate=%s, indicate=%s, notes1=%s, lang=%s WHERE id=%s",
  ...
  $updateGoTo = "manage_faq.php?Opt=viewpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> <?php echo $ModuleName['Faq']; ?> <small>修改</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
?>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <input name="postdate" type="text" class="form-control" id="postdate" value="<?php $dt = new DateTime($row_RecordFaq['postdate']); echo $dt->format('Y-m-d'); ?>" maxlength="10" data-provide="datepicker" data-date-format="yyyy-mm-dd" data-parsley-trigger="blur" required=""/> 
      ...
      <div class="form-group row">
        ...
        <div class="col-md-10">
          <button type="submit" class="btn btn btn-primary btn-block">送出</button>
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordFaq['id']; ?>" />
            <input name="lang" type="hidden" id="lang" value="<?php echo $row_RecordFaq['lang']; ?>" />
            <input name="editdate" type="hidden" id="editdate" value="<?php echo date("Y-m-d H-i-s"); ?>" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Faq" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>

...
$query_RecordDfType = sprintf("SELECT * FROM demo_dftype WHERE ((title LIKE %s)) && lang = %s && userid=%s ORDER BY sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordDfType . "%", "text"),GetSQLValueString($colnamelang_RecordDfType, "text"),GetSQLValueString($coluserid_RecordDfType, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
	if ((isset($_POST['id_check'])) && ($_POST['id_check'] != "")) {
		// 初始化此頁面所有文章
	  $updateResetSQL = sprintf("UPDATE demo_dftype SET home=0 WHERE id in (%s)", implode(",", $_POST['id']));
	  //mysqli_select_db($database_DB_Conn, $DB_Conn);
	  $Result = mysqli_query($DB_Conn, $updateResetSQL) or die(mysqli_error($DB_Conn));
	  // 更新選取文章
	  $updateSQL = sprintf("UPDATE demo_dftype SET home=1 WHERE id = %s",
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> <?php echo $ModuleName['DfType']; ?> <small>設定</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="alert alert-warning m-10"><i class="fa fa-info-circle"></i> <b>此處的設定為輸入網址的入口頁設置，若該版型未設置首頁則會以此區為準。</b></div>
      ...
      <div class="form-group row">
        ...
        <div class="col-md-12">
                      <div class="checkbox checkbox-css">
                          <input name="id_check" <?php if (!(strcmp($row_RecordDfType['home'],1))) {echo "checked=\"checked\"";} ?> value="<?php echo $row_RecordDfType['id']; ?>" type="radio" id="id_check_<?php echo $row_RecordDfType['id']; ?>" data-parsley-trigger="blur" required=""/>

...
<?php do { ?>
  <?php 
	...
	if (isset($collang_RecordStaff)) {
	  $colname_RecordAttendance = $collang_RecordStaff;
	}
	...
	//mysqli_select_db($database_DB_Conn, $DB_Conn);
	$query_RecordAttendance = sprintf("SELECT * FROM salary_attendance WHERE code=%s && lang = %s && userid=%s", GetSQLValueString($row_RecordStaff["code"], "text"), GetSQLValueString($colname_RecordAttendance, "text"),GetSQLValueString($coluserid_RecordAttendance, "int"));
	...
	if($totalRows_RecordAttendance > 0) {
		do {	
		    
			$link_view = "manage_attendance.php?id=" . $row_RecordAttendance['id'] . "&amp;lang=" . $_SESSION['lang'];
			$link_add = "manage_attendance.php?wshop=".$wshop."&amp;Opt=addpage&amp;lang=".$_SESSION['lang']."&amp;code=".$row_RecordStaff['code'];
			$link_copy = "manage_attendance.php?wshop=".$wshop."&amp;Opt=copypage&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordAttendance['id'];
			$link_edit = "manage_attendance.php?wshop=".$wshop."&amp;Opt=editpage&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordAttendance['id'];
			...
			$but_del = "<a href='".""."' class='btn btn-xs btn-danger' style='text-align:center' onclick='delete_datatables(".$row_RecordAttendance["id"].",event);'><i class='far fa-trash-alt'></i> 刪除</a>";
			...
			$row_RecordAttendance['ClassType'] = "<span id='ClassType_".$row_RecordAttendance["id"]."' class='ed_ClassType' data-type='select' data-pk='".$row_RecordAttendance["id"]."' data-placement='top'>".$row_RecordAttendance["ClassType"]."</span>";
			
			$row_RecordAttendance['ShouldStartWork'] = "<span id='ShouldStartWork_".$row_RecordAttendance["id"]."' class='ed_ShouldStartWork' data-type='text' data-pk='".$row_RecordAttendance["id"]."' data-placement='top'>".$row_RecordAttendance["ShouldStartWork"]."</span>";
			
			$row_RecordAttendance['ShouldEndWork'] = "<span id='ShouldEndWork_".$row_RecordAttendance["id"]."' class='ed_ShouldEndWork' data-type='text' data-pk='".$row_RecordAttendance["id"]."' data-placement='top'>".$row_RecordAttendance["ShouldEndWork"]."</span>";
			
			$row_RecordAttendance['RealStartWork'] = "<span id='RealStartWork_".$row_RecordAttendance["id"]."' class='ed_RealStartWork' data-type='text' data-pk='".$row_RecordAttendance["id"]."' data-placement='top'>".$row_RecordAttendance["RealStartWork"]."</span>";
			
			$row_RecordAttendance['RealEndWork'] = "<span id='RealEndWork_".$row_RecordAttendance["id"]."' class='ed_RealEndWork' data-type='text' data-pk='".$row_RecordAttendance["id"]."' data-placement='top'>".$row_RecordAttendance["RealEndWork"]."</span>";
			
			$dt = new DateTime($row_RecordAttendance['daydate']); 
			$row_RecordAttendance['daydate'] = "<span id='postdate_".$row_RecordAttendance["id"]."' class='ed_daydate' data-type='date' data-format='yyyy-mm-dd' data-pk='".$row_RecordAttendance["id"]."' data-placement='top'>".$dt->format('Y-m-d')."</span>";
			
			$dvalue['details-content'] .=
			 "<tr class='bg-aqua-transparent-2'>"
			."<td>"."<input name='id[]' class='data-check' type='checkbox' id='id[]' value='".$row_RecordAttendance["id"]."'/>"."</td>"
			."<td>".$row_RecordAttendance["ClassType"]."</td>"
			."<td>".$row_RecordAttendance["daydate"]."</td>"
			."<td>".$row_RecordAttendance["ShouldStartWork"]."</td>"
			."<td>".$row_RecordAttendance["ShouldEndWork"]."</td>"
			."<td>".$row_RecordAttendance["RealStartWork"]."</td>"
			."<td>".$row_RecordAttendance["RealEndWork"]."</td>"
			."<td width='1%'>"."<div><div class='btn-group'>".$but_add.$but_edit.$but_del."</div>"."</td>"
			."</tr>";
		} while ($row_RecordAttendance = mysqli_fetch_assoc($RecordAttendance));

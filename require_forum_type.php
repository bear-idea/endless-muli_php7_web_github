...
$coltype1_RecordForum = "-1";
if (isset($_GET['type1'])) {
  $coltype1_RecordForum = $_GET['type1'];
}
$coltype2_RecordForum = "-1";
if (isset($_GET['type2'])) {
  $coltype2_RecordForum = $_GET['type2'];
}
$coltype3_RecordForum = "-1";
if (isset($_GET['type3'])) {
  $coltype3_RecordForum = $_GET['type3'];
}
$colnamelang_RecordForum = "zh-tw";
if (isset($_GET['lang'])) {
  $colnamelang_RecordForum = $_GET['lang'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordForum = sprintf("SELECT * FROM demo_forum WHERE ((name LIKE %s) || (type LIKE %s) || (pdseries LIKE %s)) && lang = %s && type1 = %s && type2 = %s && type3 = %s ORDER BY sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordForum . "%", "text"),GetSQLValueString("%" . $colname_RecordForum . "%", "text"),GetSQLValueString("%" . $colname_RecordForum . "%", "text"),GetSQLValueString($colnamelang_RecordForum, "text"),GetSQLValueString($coltype1_RecordForum, "int"),GetSQLValueString($coltype2_RecordForum, "int"),GetSQLValueString($coltype3_RecordForum, "int"));
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordForumListClass = sprintf("SELECT * FROM demo_forumitem WHERE list_id = 2 && lang=%s && level = 0 ORDER BY sortid ASC", GetSQLValueString($colname_RecordForumListClass, "text"));
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordForumListType = sprintf("SELECT * FROM demo_forumitem WHERE list_id = 1 && lang=%s && level = 0 ORDER BY sortid ASC", GetSQLValueString($colname_RecordForumListType, "text"));

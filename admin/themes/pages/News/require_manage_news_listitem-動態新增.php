...
<div class="panel panel-inverse bg-white-transparent-9">
    <div class="panel-heading">
        ...
        <div class="btn-group float-end mr-10px">
            <a href="news?wshop=<?php echo $wshop; ?>&amp;Opt=listpage&amp;lang=<?php echo $_SESSION['lang']; ?>" class="text-nowrap btn btn-default btn-sm">
                <i class="fa fa-reply fa-fw"></i> 上一頁
            </a>
        </div>
        ...
        <form id="form_NewsItemEdit" name="form_NewsItemEdit" method="POST"
            ...
            <div id="card_sortable">
                <?php foreach ($RecordNewsListItem as $row_RecordNewsListItem) { ?>
                    <div class="widget-card bg-cyan-transparent-1 item-row"
...
<div class="panel panel-inverse bg-white-transparent-9">
    ...
    <div class="panel-body">
        <form id="form_NewsItemAdd" name="form_NewsItemAdd" method="POST" action="<?php echo $request->getRequestUri(); ?>" class="form-horizontal form-bordered" data-parsley-validate>
            <div id="new_item_container"></div>
            <button type="button" class="btn btn-success" id="add_item_button"><i class="fa fa-plus"></i> 新增項目</button>
...
<script>
    document.getElementById('add_item_button').addEventListener('click', function() {
        var container = document.getElementById('new_item_container');
        var itemHTML = `
            <div class="widget-card bg-cyan-transparent-1 item-row">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group row">
                                <div class="input-group p-0">
                                    <span class="input-group-text">名稱</span>
                                    <input name="new_itemname[]" type="text" class="form-control" data-parsley-trigger="blur" required/>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group row">
                                <div class="input-group p-0">
                                    <button type="button" class="btn btn-danger remove_item_button"><i class="fa fa-trash"></i> 刪除</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    document.addEventListener('click', function(e) {
        if (e.target && e.target.classList.contains('remove_item_button')) {

...
$colname_RecordSiteLocation = "%";
if (isset($_GET['name'])) {
  $colname_RecordSiteLocation = $_GET['name'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSiteLocation = sprintf("SELECT demo_admin.id, demo_admin.name, demo_admin.webname, demo_admin.hot, demo_setting_fr.SiteDecsHome, demo_setting_fr.userid, demo_setting_fr.SiteFBShowImage, demo_setting_fr.SiteIndicate, demo_setting_fr.SiteAddr, demo_setting_fr.SiteAddrX, demo_setting_fr.SiteAddrY, demo_setting_fr.SiteName FROM demo_admin LEFT OUTER JOIN demo_setting_fr ON demo_admin.id = demo_setting_fr.userid WHERE demo_setting_fr.SiteIndicate=1 && (demo_setting_fr.SiteAddrX!=NULL || demo_setting_fr.SiteAddrX!=0)  && (DATEDIFF(demo_admin.webrenewdate,NOW())+360*demo_admin.usetime >= 0 || demo_admin.usetime = 0) && demo_admin.name LIKE %s ORDER BY demo_admin.hot DESC, demo_admin.id DESC", GetSQLValueString("%" . $colname_RecordSiteLocation . "%", "text"));
...
<style type="text/css">
<!--
	#map{ width:485px;height:400px;float:left }
	#sideBar{ overflow:auto; width:200px;height:380px;text-align:center;background:#fff;float:right }
...
$(document).ready(function(){ 
	$("#map").bMap({
		...
		markers:{"data":[<?php do { ?>{"lat":"<?php echo $row_RecordSiteLocation['SiteAddrX']; ?>","lng":"<?php echo $row_RecordSiteLocation['SiteAddrY']; ?>","title":"<?php echo $row_RecordSiteLocation['name']; ?> <a href=\"<?php echo $row_RecordSiteLocation['webname'];?>\" target=\"_blank\" ><img src=\"images/googlemap/ftype_web.png\" width=\"26\" height=\"16\" /></a>","rnd":"1","body":"<div style=\"width:350px\"><?php echo DeleteSpace($row_RecordSiteLocation['SiteDecsHome']); ?><hr/><?php if ($row_RecordSiteLocation['SiteAddr'] != '') { ?><a href=\"https://maps.google.com.tw/maps?q=<?php echo urlencode($row_RecordSiteLocation['SiteAddr']); ?>&hl=zh-TW&t=m&z=14&vpsrc=6&iwloc=A&f=d&hnear=<?php echo urlencode($row_RecordSiteLocation['SiteAddr']); ?>\" target=\"_blank\" class=\"g_btn\">路線規劃</a><a href=\"https://maps.google.com.tw/maps?near=<?php echo urlencode($row_RecordSiteLocation['SiteAddr']); ?>&amp;q=停車場&amp;hl=zh-TW\" target=\"_blank\" class=\"g_btn\">搜尋附近停車場</a><?php } ?></div>", "icon":"images/googlemap/pins_compony_s.png"}<?php if ($totalRows_RecordSiteLocation > $SiteCounter) { echo ","; }?><?php $SiteCounter++; ?><?php } while ($row_RecordSiteLocation = mysqli_fetch_assoc($RecordSiteLocation)); ?>
		]}
	});
});
...
<div style="background: #fff; border: 4px solid #fff; position: relative; -webkit-border-radius: 4px; -moz-border-radius: 4px; -o-border-radius: 4px; border-radius: 4px; box-shadow: 0 1px 4px rgba(0,0,0,.2); -webkit-box-shadow: 0 1px 4px rgba(0,0,0,.2); -moz-box-shadow: 0 1px 4px rgba(0,0,0,.2); -o-box-shadow: 0 1px 4px rgba(0,0,0,.2); width:690px;  text-align:center; margin-bottom:20px; height:400px;">
	<div id="map"></div>

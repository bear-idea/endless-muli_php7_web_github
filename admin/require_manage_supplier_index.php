...
/* 取得發佈者資料 */
$colname_RecordSupplierListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordSupplierListAuthor = $_GET['lang'];
}
$coluserid_RecordSupplierListAuthor = "-1";
if (isset($w_userid)) {
  $coluserid_RecordSupplierListAuthor = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSupplierListAuthor = sprintf("SELECT * FROM invoicing_supplieritem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordSupplierListAuthor, "text"),GetSQLValueString($coluserid_RecordSupplierListAuthor, "int"));
$RecordSupplierListAuthor = mysqli_query($DB_Conn, $query_RecordSupplierListAuthor) or die(mysqli_error($DB_Conn)); 
$row_RecordSupplierListAuthor = mysqli_fetch_assoc($RecordSupplierListAuthor);
$totalRows_RecordSupplierListAuthor = mysqli_num_rows($RecordSupplierListAuthor);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/supplier_datatable.js"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 供應廠商 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
?>
      ...
      <div class="col-md-5 m-b-10">
        <div class="input-group"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 標題/代號</span></span>
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          <th width="16"><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input data-check" id="data-table-default-select-all" name="select_all" value="1"><label class="custom-control-label" for="data-table-default-select-all"></label></div></th>
          <th width="-1" data-priority="1"><strong>標題</strong></th>
		  <th width="100" data-priority="1"><strong>代號</strong></th>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_supplier.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_supplier.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',
...
mysqli_free_result($RecordSupplierListAuthor);
?>

...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "")) {
  ...
  if (isset($_GET['pic'])) {
	$colname_RecordScalesourceGet = $_GET['pic'];
  }
  ...
  if($totalRows_RecordScalesourceGet == 1 ){ /* 如果資料庫只有一筆 就刪除 若此圖有多張 */
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/scalesource/' . $_GET['pic']);
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/scalesource/thumb/small_' . GetFileThumbExtend($_GET['pic']));
  }
  
  $deleteSQL = sprintf("DELETE FROM erp_scalesource WHERE userid=%s && id=%s",

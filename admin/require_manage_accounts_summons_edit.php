...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAccounts_summonsdetail = sprintf("SELECT * FROM invoicing_accounts_summonsdetail WHERE aid = %s && userid=%s && state='user'", GetSQLValueString($colname_RecordAccounts_summonsdetail, "int"),GetSQLValueString($coluserid_RecordAccounts_summonsdetail, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Accounts_summons")) {
      ...
      $colname_RecordAccounts_summonsdetail = "-1";
      if (isset($_POST['id'])) {
        $colname_RecordAccounts_summonsdetail = $_POST['id'];
      }
      $coluserid_RecordAccounts_summonsdetail = "-1";
      if (isset($w_userid)) {
        $coluserid_RecordAccounts_summonsdetail = $w_userid;
      ...
      do{
          $data_id[] = $row_RecordAccounts_summonsdetail['id'];
      } while ($row_RecordAccounts_summonsdetail = mysqli_fetch_assoc($RecordAccounts_summonsdetail));
	  ...
	  foreach($_POST['detail_id'] as $key => $val){
		  ...
		  if($_POST['accountingsubjects_code'][$key] != "" && $_POST['debitamount'][$key] != "" && $_POST['creditamount'][$key] != "" && $val == '') {
		  ...
		  // 傳值到 accountingsubjects_code
			  ...
			  $updateSQL = sprintf("UPDATE invoicing_accounts_summonsdetail SET title=%s, ordertype=%s, type=%s, type1=%s, type2=%s, type3=%s, type4=%s, type5=%s, debitamount=%s, creditamount=%s, editdate=%s, notes1=%s, lang=%s WHERE id=%s",
						   GetSQLValueString($accountingsubjects['itemname'], "text"),
						   GetSQLValueString($_POST['type'], "text"),
						   GetSQLValueString($accountingsubjects['itemvalue'], "text"),
						   GetSQLValueString($accountingsubjects[0], "text"),
          ...
          if($val != ''){ // 移除目前的值 剩下的值為取消的
            $data_id = array_diff($data_id, [$val]);
          }
	  }
    
    
      if(count($data_id) > 0){
          $deleteSQL = sprintf("DELETE FROM invoicing_accounts_summonsdetail WHERE id in (%s)", implode(",", $data_id));
                  ...
                  <!--<div class="col-md-6" style="border:0">-->
                      <div class="input-group p-0">
                            <div class="input-group-prepend"><span class="input-group-text">傳票編號<span class="text-red">*</span></span></div>
                             <input name="summonsnumber" type="number" class="form-control" id="summonsnumber" value="<?php echo $row_RecordAccounts_summonsorder['summonsnumber']; ?>" maxlength="200" readonly="readonly" data-parsley-trigger="blur"/>              
                      </div>
                  ...
                  <!--<div class="col-md-6" style="border:0">-->
                      <div class="input-group p-0">
                            <div class="input-group-prepend"><span class="input-group-text">傳票總號 <i class="fa fa-info-circle text-gray" data-original-title="可稽核是否跳號，並在傳票憑證列印出排序後的號碼。" data-toggle="tooltip" data-placement="top"></i></span></div>
                             <input name="totalnumber" type="number" class="form-control" id="totalnumber" value="<?php echo $row_RecordAccounts_summonsorder['totalnumber']; ?>" maxlength="200" readonly="readonly" data-parsley-trigger="blur"/>           
                      </div>
                  ...
                  <!--<div class="col-md-6" style="border:0">-->
                      <div class="input-group p-0">
                            <div class="input-group-prepend"><span class="input-group-text">備註</span></div>
                             <input name="notes1_order" type="text" class="form-control" id="notes1_order" value="<?php echo $row_RecordAccounts_summonsorder['notes1']; ?>" size="50" maxlength="50"/>            
                      </div>
      ...
      <?php do { ?>
		<tr class="clone_group">
          ...
          <td>
            ...
            do {  
            ?>
                              <option value="<?php echo $row_RecordAccounts_summonsListType4['itemvalue']?>"<?php if (!(strcmp($row_RecordAccounts_summonsdetail['type'], $row_RecordAccounts_summonsListType4['itemvalue']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecordAccounts_summonsListType4['itemvalue']?> <?php echo $row_RecordAccounts_summonsListType4['itemname']?></option>
                              <?php
		  ...
		  <td>
			  <input name="notes1" type="text" id="notes1" value="<?php echo $row_RecordAccounts_summonsdetail['notes1']; ?>" size="50" maxlength="50" class="form-control notes1"/>
		  </td>
          <td>
              <?php if($i==0) { ?>
			  <a href="javascript:;" class="btn btn-danger dictpush-plus btn-block"><i class="fa fa-plus"></i></a>
              <?php } else { ?>
              <a href="javascript:;" class="btn btn-danger dictpush-minus btn-block" onclick='delRow(this)'><i class="fa fa-minus"></i></a>
              <?php } ?>
			  <input name="detail_id[]" hidden="hidden" id="detail_id[]" class="detail_id" value="<?php echo $row_RecordAccounts_summonsdetail['id']; ?>" />
...
$(function(){         
    ...
    //$(".dictpush-plus").click(function(){  
        ...
        if($(this).hasClass("dictpush-plus")){//这个是添加一组元素的  
			...
			row_data_count_add_index.push($("#data-table-default").DataTable().rows().count()-1);

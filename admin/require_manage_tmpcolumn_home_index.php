...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "") && $_GET['userid'] == $w_userid) {
  ...
  $deleteGoTo = "manage_tmp.php?wshop=" . $wshop . "&Opt=tmphomecolumn&lang=" . $_SESSION['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "TmpColumnFree")) {
  $insertSQL = sprintf("INSERT INTO demo_tmphomecolumn (type, style, dftname, customname, userid) VALUES (%s, %s, %s, %s, %s)",
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSettingLock = sprintf("SELECT productlistLock, frilinkLock, fbfanLock, articlelistLock, alllistLock, newslistLock, blogplistLock, blogcalendarLock, blogrlistLock, bloglistLock, blogviewcountLock, blogwhoscountLock FROM demo_setting_fr WHERE userid = %s", GetSQLValueString($coluserid_RecordSettingLock, "int"));
...
<div>
  ...
  <td width="100" align="right"><?php echo $row_RecordTmpColumn['dftname']; ?></td>
                     ...
                     <td><?php if ($row_RecordTmpColumn['style'] == 'free') { ?><a href="manage_tmp.php?wshop=<?php echo $wshop;?>&amp;Opt=tmphomecolumn_setting_free&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $row_RecordTmpColumn['id']; ?>&amp;userid=<?php echo $row_RecordTmpColumn['userid']; ?>&amp;type=<?php echo $row_RecordTmpColumn['type']; ?>"><img src="images/edit.gif" width="45" height="18" /></a><?php } else if($row_RecordTmpColumn['style'] == 'menu') { ?><a href="manage_tmp.php?wshop=<?php echo $wshop;?>&amp;Opt=tmphomecolumn_setting_menu&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $row_RecordTmpColumn['id']; ?>&amp;userid=<?php echo $row_RecordTmpColumn['userid']; ?>&amp;type=<?php echo $row_RecordTmpColumn['type']; ?>"><img src="images/edit.gif" width="45" height="18" /></a><?php } else { ?><a href="manage_tmp.php?wshop=<?php echo $wshop;?>&amp;Opt=tmphomecolumn_setting&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $row_RecordTmpColumn['id']; ?>&amp;userid=<?php echo $row_RecordTmpColumn['userid']; ?>&amp;type=<?php echo $row_RecordTmpColumn['type']; ?>"><img src="images/edit.gif" width="45" height="18" /></a><?php } ?><a href="manage_tmp.php?wshop=<?php echo $wshop;?>&amp;Opt=tmphomecolumn&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;id_del=<?php echo $row_RecordTmpColumn['id']; ?>&amp;userid=<?php echo $row_RecordTmpColumn['userid']; ?>&amp;type=<?php echo $row_RecordTmpColumn['type']; ?>"><img src="images/del.gif" width="45" height="18" /></a></td>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-left:10px; margin-right:10px;border:solid 1px #CCCCCC;" class="TB_General_style01">
		  ...
		  <?php do { ?>
          <?php if (
		  ($row_RecordTmpAddColumn['type'] == 'blogwhoscount' && ($row_RecordSettingLock['blogwhoscountLock'] == '1')) || 
		  ($row_RecordTmpAddColumn['type'] == 'blogviewcount' && ($row_RecordSettingLock['blogviewcountLock'] == '1')) || 
		  ($row_RecordTmpAddColumn['type'] == 'blogrlist' && ($row_RecordSettingLock['blogrlistLock'] == '1')) || 
		  ($row_RecordTmpAddColumn['type'] == 'bloglist' && ($row_RecordSettingLock['bloglistLock'] == '1')) || 
		  ($row_RecordTmpAddColumn['type'] == 'blogcalendar' && ($row_RecordSettingLock['blogcalendarLock'] == '1')) || 
		  ($row_RecordTmpAddColumn['type'] == 'blogplist' && ($row_RecordSettingLock['blogplistLock'] == '1')) || 
...
$(document).ready(function() {
	$(".sortid").editable("sqledit/tmphomecolumn_jedit.php", 	{
		//cancel: '取消',
		submit: '修改',
		select:true,
	...
	$(".ed_customname").editable("sqledit/tmphomecolumn_jedit.php", 		{
		//cancel: '取消',
		submit: '修改',
		indicator: '<img src="images/indicator.gif">',
		tooltip: '滑鼠點此可編輯此區塊...',

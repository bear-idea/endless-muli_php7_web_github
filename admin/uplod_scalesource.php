...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Scalesource")) {
  $updateSQL = sprintf("UPDATE erp_scalesource SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/scalesource/' . $_POST['oldpic']);
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/scalesource/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_scalesource.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordScalesource = sprintf("SELECT * FROM erp_scalesource WHERE id = %s", GetSQLValueString($colname_RecordScalesource, "int"));
...
<meta content="width=device-width, initial-scalesource=1.0, maximum-scalesource=1.0, user-scalable=no" name="viewport" />
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordScalesource['pic']; ?>" />
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 3000,  
		<?php if ($row_RecordScalesource['pic'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/scalesource/thumb/small_<?php echo $row_RecordScalesource['pic']; ?>"],
		<?php } ?>

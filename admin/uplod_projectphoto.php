...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/project";
	$ppu->extensions = "GIF,JPG,JPEG,BMP,PNG";
	$ppu->formName = "form_Project";
	$ppu->storeType = "file";
	$ppu->sizeLimit = "3000";
...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
  ...
  $sip->quality = "100";
  $sip->makeThumb = "true";
  $sip->pathThumb = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/project/thumb";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Project")) {
  $updateSQL = sprintf("UPDATE demo_projectalbumphoto SET pic=IFNULL(%s,pic) WHERE actphoto_id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/project/' . $_POST['oldpic']);
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/project/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_projectphoto.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordProject = sprintf("SELECT * FROM demo_projectalbumphoto WHERE actphoto_id = %s", GetSQLValueString($colname_RecordProject, "int"));
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="inner_project.php?wshop=<?php echo $wshop; ?>&amp;Opt=photoeditpage&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $_GET['id_edit']; ?>&amp;act_id=<?php echo $_GET['act_id']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordProject['actphoto_id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordProject['pic']; ?>" />
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/project/thumb/small_<?php echo $row_RecordProject['pic']; ?>"],
		<?php } ?>

...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Tmp")) {
  if($_POST['lang'] == 'zh-tw') {
  $updateSQL = sprintf("UPDATE demo_setting_fr SET homecontenttwsmall=%s WHERE userid=%s",
                       GetSQLValueString($_POST['homecontent'], "text"),
                       GetSQLValueString($w_userid, "int"));
  }
  if($_POST['lang'] == 'cn') {
  $updateSQL = sprintf("UPDATE demo_setting_fr SET homecontentcnsmall=%s WHERE userid=%s",
                       GetSQLValueString($_POST['homecontent'], "text"),
                       GetSQLValueString($w_userid, "int"));
  }
  if($_POST['lang'] == 'en') {
  $updateSQL = sprintf("UPDATE demo_setting_fr SET homecontentensmall=%s WHERE userid=%s",
                       GetSQLValueString($_POST['homecontent'], "text"),
                       GetSQLValueString($w_userid, "int"));
  }
  if($_POST['lang'] == 'kr') {
  $updateSQL = sprintf("UPDATE demo_setting_fr SET homecontentkrsmall=%s WHERE userid=%s",
                       GetSQLValueString($_POST['homecontent'], "text"),
                       GetSQLValueString($w_userid, "int"));
  }
  if($_POST['lang'] == 'sp') {
  $updateSQL = sprintf("UPDATE demo_setting_fr SET homecontentspsmall=%s WHERE userid=%s",
                       GetSQLValueString($_POST['homecontent'], "text"),
                       GetSQLValueString($w_userid, "int"));
  }
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSystemSetting = sprintf("SELECT homecontenttwsmall, homecontentcnsmall, homecontentensmall, homecontentkrsmall, homecontentspsmall FROM demo_setting_fr WHERE userid=%s", GetSQLValueString($coluserid_RecordSystemSetting, "int"));
...
{
	CKEDITOR.replace( 'homecontent',{width : '250px', toolbar : 'Free'} );
};
</script>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post" id="form_TmpColumn" name="form_TmpColumn">
       <div class="form-group row">
        ...
        <div class="col-md-12"><span class="badge badge-primary"><i class="fa fa-paper-plane"></i> 內容區塊</span></div>
      </div>
      ...
      <div class="form-group row">
          ...
          <div class="table-responsive">
                <textarea name="homecontent" id="homecontent" cols="100%" rows="35" class="form-control"><?php if ($_SESSION['lang'] == 'zh-cn') { ?><?php echo $row_RecordSystemSetting['homecontentcnsmall']; ?><?php } else if ($_SESSION['lang'] == 'en') { ?><?php echo $row_RecordSystemSetting['homecontentensmall']; ?><?php } else if ($_SESSION['lang'] == 'kr') { ?><?php echo $row_RecordSystemSetting['homecontentkrsmall']; ?><?php } else if ($_SESSION['lang'] == 'sp') { ?><?php echo $row_RecordSystemSetting['homecontentspsmall']; ?><?php } else { ?><?php echo $row_RecordSystemSetting['homecontenttwsmall']; ?><?php } ?></textarea>  
          </div>
          </div>
      </div>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
                <input type="image" src="images/tmp_df_pic_01.jpg" id="change_unit01" onclick="return false" style="margin-right:5px;"><input type="image" src="images/tmp_df_pic_02.jpg" id="change_unit02" onclick="return false" style="margin-right:5px;"><input type="image" src="images/tmp_df_pic_03.jpg" id="change_unit03" onclick="return false" style="margin-right:5px;"><div style="color:#CC6600;">點選您想要的圖示外觀即可在【詳細內容欄位】之【游標處】加入預設圖片。圖片可雙擊滑鼠左鍵，在視窗中點選【上傳】頁面，在上傳您要的圖片即可。</div>
          </div>
      </div>
...
<script type="text/javascript">
	$(document).ready(function() {
			$("#change_unit01").click(function(){
					CKEDITOR.instances.content.insertHtml("<img src=\"http://www.shop3500.com/images/tmp/image_04.jpg\" alt=\"\" width=\"198\" height=\"60\" style=\"display:block; margin:auto;\" /><br />");
			});
			$("#change_unit02").click(function(){
					CKEDITOR.instances.content.insertHtml("<img src=\"http://www.shop3500.com/images/tmp/image_05.jpg\" alt=\"\" width=\"198\" height=\"120\" style=\"display:block; margin:auto;\" /><br />");
			});
			$("#change_unit03").click(function(){
					CKEDITOR.instances.content.insertHtml("<img src=\"http://www.shop3500.com/images/tmp/image_06.jpg\" alt=\"\" width=\"198\" height=\"240\" style=\"display:block; margin:auto;\" /><br />");
			});
	});
</script>

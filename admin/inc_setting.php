...
$coluserid_RecordTmpConfigGetWidth = "-1";
if (isset($w_userid)) {
  $coluserid_RecordTmpConfigGetWidth = $w_userid;
}

$query_RecordTmpConfigGetWidth = sprintf("SELECT id, name, boxwidth, tmpwebwidth FROM demo_tmp WHERE id = (SELECT $SQL_MSTmpSelect FROM demo_setting_fr WHERE userid=%s)", GetSQLValueString($coluserid_RecordTmpConfigGetWidth, "int"));
$RecordTmpConfigGetWidth = mysqli_query($DB_Conn, $query_RecordTmpConfigGetWidth) or die(mysqli_error($DB_Conn));
$row_RecordTmpConfigGetWidth = mysqli_fetch_assoc($RecordTmpConfigGetWidth);
$totalRows_RecordTmpConfigGetWidth = mysqli_num_rows($RecordTmpConfigGetWidth);

$coluserid_RecordTmpMiddleBoardGetWidth = "-1";
if (isset($w_userid)) {
  $coluserid_RecordTmpMiddleBoardGetWidth = $w_userid;
}

$query_RecordTmpMiddleBoardGetWidth = sprintf("SELECT tmp_l_m_width, tmp_r_m_width FROM demo_tmpboard WHERE id = (SELECT tmpmiddleboard FROM demo_tmp WHERE id=(SELECT $SQL_MSTmpSelect FROM demo_setting_fr WHERE userid=%s)) ", GetSQLValueString($coluserid_RecordTmpMiddleBoardGetWidth, "int"));
$RecordTmpMiddleBoardGetWidth = mysqli_query($DB_Conn, $query_RecordTmpMiddleBoardGetWidth) or die(mysqli_error($DB_Conn));
$row_RecordTmpMiddleBoardGetWidth = mysqli_fetch_assoc($RecordTmpMiddleBoardGetWidth);
$totalRows_RecordTmpMiddleBoardGetWidth = mysqli_num_rows($RecordTmpMiddleBoardGetWidth);

if($totalRows_RecordTmpMiddleBoardGetWidth == 0 ) {$row_RecordTmpMiddleBoardGetWidth['tmp_l_m_width'] = 0; $row_RecordTmpMiddleBoardGetWidth['tmp_r_m_width'] = 0;} 
...
if($CKeditor_ini == "mobile_smarty" && $row_RecordSystemConfigFr['Mobile_Enable'] == '1') { /* 偵測使用裝置是行動且功能使用自動判斷 (RWD) */
    ...
    if($row_RecordTmpConfigGetWidth['boxwidth'] == '3') { $configboxwidth = 1780; }else if($row_RecordTmpConfigGetWidth['boxwidth'] == '2'){ $configboxwidth = 1570; }else if($row_RecordTmpConfigGetWidth['boxwidth'] == '1'){ $configboxwidth = 1360; }else{ $configboxwidth = 1150; }
...
mysqli_free_result($RecordTmpConfigGetWidth);

mysqli_free_result($RecordTmpMiddleBoardGetWidth);

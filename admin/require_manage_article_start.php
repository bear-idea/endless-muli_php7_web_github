...
$query_RecordArticle = sprintf("SELECT * FROM demo_article WHERE ((title LIKE %s)) && lang = %s && userid=%s ORDER BY sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordArticle . "%", "text"),GetSQLValueString($colnamelang_RecordArticle, "text"),GetSQLValueString($coluserid_RecordArticle, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
	if ((isset($_POST['id_check'])) && ($_POST['id_check'] != "")) {
		// 初始化此頁面所有文章
	  $updateResetSQL = sprintf("UPDATE demo_article SET home=0 WHERE id in (%s)", implode(",", $_POST['id']));
	  //mysqli_select_db($database_DB_Conn, $DB_Conn);
	  $Result = mysqli_query($DB_Conn, $updateResetSQL) or die(mysqli_error($DB_Conn));
	  // 更新選取文章
	  $updateSQL = sprintf("UPDATE demo_article SET home=1 WHERE id = %s",
	  ...
	  $insertGoTo = "manage_article.php?Opt=viewpage&lang=" . $_POST['lang'];
	  /*if (isset($_SERVER['QUERY_STRING'])) {
      ...
      <div class="form-group row">
        ...
        <div class="col-md-12">
                      <div class="checkbox checkbox-css">
                          <input name="id_check" <?php if (!(strcmp($row_RecordArticle['home'],1))) {echo "checked=\"checked\"";} ?> value="<?php echo $row_RecordArticle['id']; ?>" type="radio" id="id_check_<?php echo $row_RecordArticle['id']; ?>" data-parsley-trigger="blur" required=""/>
                        <label for="id_check_<?php echo $row_RecordArticle['id']; ?>"><?php echo $row_RecordArticle['title']; ?></label>
                        <?php if($row_RecordArticle['home'] == 1){ ?>

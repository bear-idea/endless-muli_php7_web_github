...
<link href="<?php if($SiteBaseUrlOuter != "") { echo $SiteBaseUrlOuter; } else { echo $SiteBaseUrl; } ?>assets/plugins/sumoselect/sumoselect.min.css" rel="stylesheet" />
<script src="<?php if($SiteBaseUrlOuter != "") { echo $SiteBaseUrlOuter; } else { echo $SiteBaseUrl; } ?>assets/plugins/sumoselect/jquery.sumoselect.min.js"></script>
<style>.select2-search{ width:100%;}</style>
<!-- InstanceEndEditable -->
<meta charset="utf-8">
</head>
<?php 
  ...
  <?php 
    ...
    <?php if ($TmpWrpViewLineLocate == '2') { ?>               
        ...
        <section class="page-header page-header-xxs">
			...
			<?php echo $ModuleName['Member']; ?>
			<!-- InstanceEndEditable --></h1>
            <!-- InstanceBeginEditable name="導覽列" -->
                                    <?php require("require_scaleclearance_viewline_mobile.php"); ?>
                    <!-- InstanceEndEditable -->
        </section>
	...
	<?php } ?>
      ...
      <div class="row">      
        ...
        <?php
			...
			{
				case "viewpage":
				    if ((isset($_SESSION['MM_UserGroup_' . $_GET['wshop']]) && in_array($_SESSION['MM_UserGroup_' . $_GET['wshop']], $arr_MM_authorizedUsers))) {
						include_once("require_member.php");			
					}else{
						include_once("require_member_login.php");	
					}
					break;
				case "order":
					include_once("require_scaleclearance_order.php");				
					break;
				case "editpage":
					include_once("require_member_edit.php");				
					break;
				case "accountpage":
					include_once("require_member_account.php");				
					break;
				case "reg":
					include_once("require_member_reg.php");				
					break;
				case "log":
					include_once("require_member_login.php");				
					break;
				case "pswfgt":
					include_once("require_member_passwordlost.php");				
					break;
				case "pswfgt2":
					include_once("require_member_passwordlost2.php");				
					break;
			    case "order":
					include_once("require_member_order.php");				
					break;
				case "FAQ":
					include_once("require_member_faq.php");				
					break;
				default:
					if ((isset($_SESSION['MM_UserGroup_' . $_GET['wshop']]) && in_array($_SESSION['MM_UserGroup_' . $_GET['wshop']], $arr_MM_authorizedUsers))) {
						include_once("require_member.php");			

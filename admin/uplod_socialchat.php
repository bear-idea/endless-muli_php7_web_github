...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Socialchat")) {
  $updateSQL = sprintf("UPDATE demo_socialchat SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/socialchat/' . $_POST['oldpic']);
  //@unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/socialchat/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_socialchat.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSocialchat = sprintf("SELECT * FROM demo_socialchat WHERE id = %s", GetSQLValueString($colname_RecordSocialchat, "int"));
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordSocialchat['pic']; ?>" />
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 1000,  
		<?php if ($row_RecordSocialchat['pic'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/socialchat/<?php echo $row_RecordSocialchat['pic']; ?>"],
		<?php } ?>

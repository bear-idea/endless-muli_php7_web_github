...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE demo_tmp SET tmpviewlineboard=%s WHERE id=%s",
                       GetSQLValueString($_POST['TmpBoardSelect'], "text"),
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpBoardStyle = sprintf("SELECT * FROM demo_tmp WHERE id = %s", GetSQLValueString($colname_RecordTmpBoardStyle, "int"));
...
if (isset($row_RecordTmpBoardStyle['tmpviewlineboard'])) {
  $colid_RecordTmpBoard = $row_RecordTmpBoardStyle['tmpviewlineboard'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpBoard = sprintf("SELECT * FROM demo_tmpboard WHERE id=%s && (userid=%s || userid=1)", GetSQLValueString($colid_RecordTmpBoard, "int"),GetSQLValueString($coluserid_RecordTmpBoard, "int"));
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/tmpboard_datatable_get.js"></script>
<!-- ================== END Datatables JS ================== -->
...
<style>
.cards tbody tr{float:left;width:20rem;margin:.5rem;border:none;border-radius:.35rem;box-shadow:0 0 2px rgba(0,0,0,.2),0 4px 4px -2px rgba(0,0,0,.2)}.cards tbody td{display:block}.cards tbody td .hidden-text{width:9rem;overflow:hidden;height:20px}.mdl{min-height:1px;_height:1px}.mdl_t{position:relative;font-size:12px}.mdl_t_l,.mdl_t_r{position:absolute;top:0;font-size:1px;overflow:hidden}.mdl_t_l{left:0}.mdl_t_r{right:0;float:right}.mdl_t_m{position:absolute;text-align:left;height:20px;line-height:20px}.mdl_t_m h3{font-size:14px;width:120px}.mdl_t_c{position:relative;text-align:left;overflow:hidden}.mdl_t_c span.a_a{text-decoration:none}.mdl_c{position:relative;overflow:hidden}.mdl_c .mdl_m_t,.mdl_c .mdl_m_c,.mdl_c .mdl_m_b{padding:0 2px}.mdl_c .mdl_m_t,.mdl_c .mdl_m_b{text-align:left}.mdl_c_l,.mdl_c_r{position:absolute;top:0;font-size:1px}.mdl_c_l{left:0}.mdl_c_r{right:0}.mdl_c_c{position:relative;zoom:1}.mdl_b{position:relative;overflow:hidden}.mdl_b_l,.mdl_b_r{position:absolute;top:0;font-size:1px}.mdl_b_l{left:0}.mdl_b_r{right:0}.g_p_fill{padding-bottom:32767px;margin-bottom:-32767px}.g_p_hide{overflow:hidden;display:block!important;display:inline-block}
</style>
...
<div id="page-container" class="page-header-fixed page-sidebar-fixed page-without-sidebar p-0">
  ...
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 外框 <small>選擇</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="tmp_config_<?php echo $row_RecordTmpBoardStyle['name']; // 為版型風格?>.php?lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $row_RecordTmpBoardStyle['id']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
        ...
        <div class="card bg-grey-transparent-1">
          <div class="card-block">
           ...
           <!--外框樣式 END-->
            ...
            <?php } else { ?>
            <div class="alert alert-danger m-t-5 m-b-0"><i class="fa fa-info-circle"></i> <b>目前尚未套用任何外框。</b></div>
            <?php } ?>
          </div>
        </div>
      </div>
    ...
    <table id="data-table-default" class="table table-bordered table-hover cards" style="width:100%">
      <thead>
        <tr>
          ...
          <th data-priority="1"><strong>可選擇外框<div id="error_action"></div></strong></th>
        </tr>
      </thead>
      <tfoot>
        <tr>
         
          <td><span class="label label-danger p-10"><div class='radio radio-css radio-inline'><input type='radio' name='TmpBoardSelect' value='' id='TmpBoardSelect_none' data-parsley-trigger='blur' data-parsley-errors-container='#error_action'>
          <label for='TmpBoardSelect_none' class='text-white'>不使用任何外框</label></div></span><button type="button" id="reset_table" class="btn btn-default btn-sm pull-right"><i class="fa fa-sync"></i> 清除狀態</button></td>
        </tr>
      </tfoot>
    </table>
...
<script>
	$(document).ready(function() {
	  ...
	  $("#TransparentButtom1").click(function(){
		...
		<?php if ($row_RecordTmpBoard['tmpleftfontcolor'] == "transparent") { ?>
			$("#tmpleftfontcolor").val("transparent");
		<?php } ?>
		...
		<?php if ($row_RecordTmpBoard['tmpmiddlefontcolor'] == "transparent") { ?>
			$("#tmpmiddlefontcolor").val("transparent");
		<?php } ?>
	});
</script>

...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "")) {
  ...
  if (isset($_GET['pic'])) {
	$colname_RecordFrilinkGet = $_GET['pic'];
  }
  ...
  if($totalRows_RecordFrilinkGet == 1 ){ /* 如果資料庫只有一筆 就刪除 若此圖有多張 */
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/frilink/' . $_GET['pic']);
	  //@unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/frilink/thumb/small_' . GetFileThumbExtend($_GET['pic']));
  }
  
  $deleteSQL = sprintf("DELETE FROM demo_frilink WHERE userid=%s && id=%s",

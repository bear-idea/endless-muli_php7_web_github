...
switch($_GET['lang'])
	{
		...
		case "jp":
			...
			break;	
		case "kr":
		    if($TmpLogoDefaultImg_kr != '') {
		    $TmpLogoDefaultImg =  $TmpLogoDefaultImg_kr;
			$TmpLogoDefaultWidth =  $TmpLogoDefaultWidth_kr;
			$TmpLogoDefaultHeight =  $TmpLogoDefaultHeight_kr;
			}
			$TmpLogoDefaultLogoName =  $TmpLogoDefaultLogoName_kr;
			if($TmpLogo_kr != '') {
			$TmpLogo =  $TmpLogo_kr;
			$TmpLogoWidth =  $TmpLogoWidth_kr;
			$TmpLogoHeight =  $TmpLogoHeight_kr;
			}
			$TmpLogoLogoName =  $TmpLogoLogoName_kr;
			break;	
		case "sp":
		    if($TmpLogoDefaultImg_sp != '') {
		    $TmpLogoDefaultImg =  $TmpLogoDefaultImg_sp;
			$TmpLogoDefaultWidth =  $TmpLogoDefaultWidth_sp;
			$TmpLogoDefaultHeight =  $TmpLogoDefaultHeight_sp;
			}
			$TmpLogoDefaultLogoName =  $TmpLogoDefaultLogoName_sp;
			if($TmpLogo_sp != '') {
			$TmpLogo =  $TmpLogo_sp;
			$TmpLogoWidth =  $TmpLogoWidth_sp;
			$TmpLogoHeight =  $TmpLogoHeight_sp;
			}
			$TmpLogoLogoName =  $TmpLogoLogoName_sp;
			break;	
		default:
		    break;
	}
...
<?php if ($_GET['wshop'] == $tplwebname) { //如果目前之網頁為Logo之作者 則 使用個別樣板 之 Logo 設定 會依樣板之logo設定變動 ?>
    ...
    <?php if ($TmpLogo != "" && GetFileExtend($TmpLogo) != '.swf') { ?>
        <span  data-scroll-reveal='enter top over 1s after 0.5s'><a href="index.php?wshop=<?php echo $_GET['wshop'] ?>"><img src="<?php echo $SiteImgUrl; ?><?php echo $TmpLogoWebname; ?>/logo/<?php echo $TmpLogo; ?>" alt="<?php echo $SiteName; ?>"  width="<?php echo $TmpLogoWidth; ?>" height="<?php echo $TmpLogoHeight; ?>"/></a></span>
    <?php } else if (GetFileExtend($TmpLogo) == '.swf'){ ?>
       <span  data-scroll-reveal='enter top over 1s after 0.5s'><embed type="application/x-shockwave-flash" src="<?php echo $SiteImgUrl; ?><?php echo $_GET['wshop'] ?>/logo/<?php echo $TmpLogo; ?>" width="<?php echo $TmpLogoWidth; ?>" height="<?php echo $TmpLogoHeight; ?>" play="true" loop="true" quality="high" pluginspage="index.php?wshop=<?php echo $_GET['wshop'] ?>" wmode="transparent"></embed></span>
    <?php } else { ?>
    <div id="logo" style="position:absolute;z-index:11">
      <span  data-scroll-reveal='enter top over 1s after 0.5s'><a href="index.php?wshop=<?php echo $_GET['wshop'] ?>"><img src="images/logo_default_tmp.png" width="149" height="50" /></a></span>
    </div>
    <?php }  ?>
    <?php } else { // 類型 ?>
    <span  data-scroll-reveal='enter top over 1s after 0.5s'><a href="index.php?wshop=<?php echo $_GET['wshop'] ?>"><?php echo $TmpLogoLogoName; ?></a></span>
    <?php }  // 類型 ?>
    </div>
<?php } else { // 若不為作者 則 使用此使用者個別設定黨之預設值 Logo 只有一個?>
    ...
    <?php if ($TmpLogoDefaultLogoType == 0) { //類型 ?>
       <?php if ($TmpLogoDefaultImg != "" && GetFileExtend($TmpLogoDefaultImg) != '.swf') { ?>
        <span  data-scroll-reveal='enter top over 1s after 0.5s'><a href="index.php?wshop=<?php echo $_GET['wshop'] ?>"><img src="<?php echo $SiteImgUrl; ?><?php echo $TmpLogoDefaultWebName; ?>/logo/<?php echo $TmpLogoDefaultImg; ?>" alt="Logo-<?php echo $SiteName; ?>" width="<?php echo $TmpLogoDefaultWidth; ?>" height="<?php echo $TmpLogoDefaultHeight; ?>"/></a></span>
    ...
    <?php } else { ?>
    <div id="logo" style="position:absolute; z-index:11">
      <span  data-scroll-reveal='enter top over 1s after 0.5s'><a href="index.php?wshop=<?php echo $_GET['wshop'] ?>"><img src="images/logo_default.png" alt="Logo-<?php echo $SiteName; ?>" width="149" height="50" /></a></span>
    </div>
    <?php }  ?>
    <?php } else { // 類型 ?>
    <span  data-scroll-reveal='enter top over 1s after 0.5s'><a href="index.php?wshop=<?php echo $_GET['wshop'] ?>"><?php echo $TmpLogoDefaultLogoName; ?></a></span>
    <?php }  // 類型 ?>
    </div>
<?php } ?>

...
$query_RecordBlog = sprintf("SELECT * FROM demo_blog WHERE ((title LIKE %s)) && lang = %s && type1 = %s && type2 = %s && type3 = %s  && indicate = 1 && userid=%s ORDER BY lastdate DESC, sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordBlog . "%", "text"),GetSQLValueString($colnamelang_RecordBlog, "text"),GetSQLValueString($coltype1_RecordBlog, "int"),GetSQLValueString($coltype2_RecordBlog, "int"),GetSQLValueString($coltype3_RecordBlog, "int"),GetSQLValueString($coluserid_RecordBlog, "int"));

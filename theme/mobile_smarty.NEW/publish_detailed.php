...
<!--外框-->
                     ...
                     <?php //require("require_fb_like.php"); ?>
                        ...
                        <?php if($row_RecordPublish['skeyword'] != "") { ?>
                        <div style="clear:both;"></div>
                        <div style="border:0px #CCCCCC dotted; padding:5px;" class="keytag">
                        <a>&nbsp;<i class="fa fa-tag"></i>&nbsp;</a><?php
                        $arr_tag = explode(',', $row_RecordPublish['skeyword']);
						//echo $SiteBaseUrl . url_rewrite("publish",array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'detailed','id'=>$row_RecordPublish['id']),'',$UrlWriteEnable);
                        for($i = 0; $i < count($arr_tag); $i++){ echo "<a href=\"".$SiteBaseUrl . url_rewrite("publish",array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'search'),'',$UrlWriteEnable).$tag_params.$arr_tag[$i] ."\">".$arr_tag[$i]."</a>";}
                        ?></div>

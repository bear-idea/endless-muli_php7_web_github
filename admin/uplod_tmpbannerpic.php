...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/banner";
	$ppu->extensions = "GIF,JPG,JPEG,BMP,PNG";
	$ppu->formName = "form_Banner";
	$ppu->storeType = "file";
	$ppu->sizeLimit = "3000";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Banner")) {
  $updateSQL = sprintf("UPDATE demo_tmp SET tmpbannerpic=IFNULL(%s,tmpbannerpic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/banner/' . $_POST['oldpic']);
  //@unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/tmp/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_tmpbannerpic.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_RecordBanner = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordBanner = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordBanner = sprintf("SELECT * FROM demo_tmp WHERE id = %s", GetSQLValueString($colname_RecordBanner, "int"));
$RecordBanner = mysqli_query($DB_Conn, $query_RecordBanner) or die(mysqli_error($DB_Conn));
$row_RecordBanner = mysqli_fetch_assoc($RecordBanner);
$totalRows_RecordBanner = mysqli_num_rows($RecordBanner);
?>
<!DOCTYPE html>
<html lang="zh-TW">
<!--<![endif]-->
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordBanner['id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordBanner['tmpbannerpic']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Banner" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 3000,  
		<?php if ($row_RecordBanner['tmpbannerpic'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/banner/<?php echo $row_RecordBanner['tmpbannerpic']; ?>"],
		<?php } ?>

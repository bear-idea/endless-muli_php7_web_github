var handleDataTableDefault = function() {
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       
		    "dom":"lBprtip",buttons:[{extend:"copy",className:"btn-sm",title:"物料一覽表"},{extend:"csv",className:"btn-sm",title:"物料一覽表"},{extend:"excel",className:"btn-sm",title:"物料一覽表"},{extend:"pdf",className:"btn-sm",title:"物料一覽表"},{extend:"print",className:"btn-sm",title:"物料一覽表"},{extend:"colvis",className:"btn-sm",title:"物料一覽表"}],
			//"dom": "lprtip",
            "autoWidth": false,
            "ajax": {
                "url": "sqldatatable/scale_view_export.php",
                "type": "post",
				"data": function ( d ) {
					...
					d.search_indicate = $('#col5_filter').val();
					
					/*if ($('#fullIdPath').val() != "") {
						d.search_type1 = $('#fullIdPath').val().split(',')[0];
					}else{
						d.search_type1 = $('#coltype1_filter').val();
					}
					
					if ($('#fullIdPath').val() != "") {
						d.search_type2 = $('#fullIdPath').val().split(',')[1];
					}else{
						d.search_type2 = $('#coltype2_filter').val();
					}
					
					if ($('#fullIdPath').val() != "") {
						d.search_type3 = $('#fullIdPath').val().split(',')[2];
					}else{
						d.search_type3 = $('#coltype3_filter').val();
					}*/
			...
			"pageLength": 100,
            "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"] // change per page values here
            ],
            "language": //把文本變為中文
            {
                "sProcessing": "處理中...",
			...
			"stateLoadParams": function (settings, data) {
				...
				$("#col3_filter").val(data.search.field.indicate);
            ...
            "fnDrawCallback": function(oSettings) {
				...
				$('#data-table-default').editable({
                    ...
                    url: 'sqledit/scale_jedit.php',
                    name: "type",
                    source: "sqledit/scale_get_list.php?list_id=4",
                    /*validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
...
function delete_datatables(id,pic,e) {
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/scale_del.php?id_del=" + id + "&pic=" + pic,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/scale_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

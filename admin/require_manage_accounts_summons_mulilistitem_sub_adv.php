...
$collang_RecordAccounts_summonsListItemGet = "zh-tw";
if (isset($_GET['lang'])) {
  $collang_RecordAccounts_summonsListItemGet = $_GET['lang'];
}
$coluserid_RecordAccounts_summonsListItemGet = "-1";
if (isset($w_userid)) {
  $coluserid_RecordAccounts_summonsListItemGet = $w_userid;
}
$colitemid_RecordAccounts_summonsListItemGet = "-1";
if (isset($_GET['item_id'])) {
  $colitemid_RecordAccounts_summonsListItemGet = $_GET['item_id'];
}
$collistid_RecordAccounts_summonsListItemGet = "-1";
if (isset($_GET['list_id'])) {
  $collistid_RecordAccounts_summonsListItemGet = $_GET['list_id'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAccounts_summonsListItemGet = sprintf("SELECT * FROM invoicing_accounts_summonsitem WHERE list_id = %s && lang=%s && item_id = %s && userid=%s LIMIT 1", GetSQLValueString($collistid_RecordAccounts_summonsListItemGet, "int"),GetSQLValueString($collang_RecordAccounts_summonsListItemGet, "text"),GetSQLValueString($colitemid_RecordAccounts_summonsListItemGet, "int"),GetSQLValueString($coluserid_RecordAccounts_summonsListItemGet, "int"));
$RecordAccounts_summonsListItemGet = mysqli_query($DB_Conn, $query_RecordAccounts_summonsListItemGet) or die(mysqli_error($DB_Conn));
$row_RecordAccounts_summonsListItemGet = mysqli_fetch_assoc($RecordAccounts_summonsListItemGet);
$totalRows_RecordAccounts_summonsListItemGet = mysqli_num_rows($RecordAccounts_summonsListItemGet);

/* 新增類別項目 */
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_Accounts_summonsItemAdd")) {
  $insertSQL = sprintf("INSERT INTO invoicing_accounts_summonsitem (list_id, itemname, itemvalue, subitem_id, state, lang, level, userid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
  ...
  // 更新父節點 目前新增為子節點
  $updateSQL2 = sprintf("UPDATE invoicing_accounts_summonsitem SET endnode='parent' WHERE item_id=%s",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Accounts_summonsItemEdit")) {
	foreach($_POST['item_id'] as $key => $val){
	  $updateSQL = sprintf("UPDATE invoicing_accounts_summonsitem SET list_id=%s, sortid=%s, indicate=%s, state=%s, itemname=%s, itemvalue=%s, lang=%s, level=%s WHERE item_id=%s",
...
if ((isset($_POST['deltype'])) && ($_POST['deltype'] != "")) {
	...
	$MM_flag="MM_update";
		...
		foreach($_POST['deltype'] as $key => $val){
		  ...
		  {
			  	case "1":
		  		$MM_dupKeyRedirect="manage_accounts_summons.php?wshop=" . $wshop . "&Opt=sub_mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&item_id=" . $_GET['item_id'] . "&level=" . $_GET['level'] . "&Operate=delErrorP";
				break;
				case "2":
				case "3":
				case "4":
		  		$MM_dupKeyRedirect="manage_accounts_summons.php?wshop=" . $wshop . "&Opt=sub_mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&item_id=" . $_GET['item_id'] . "&subitem_id=" . $row_RecordAccounts_summonsListItem['subitem_id'] . "&level=" . $_GET['level'] . "&Operate=delErrorP";
				break;
				default:
				break;
		  }
		  ...
		  {
			  	case "1":
		  		$LoginRS__query = sprintf("SELECT * FROM invoicing_accounts_summons WHERE type2=%s", GetSQLValueString($loginUsername, "int")); // 分類
				break;
				case "2":
		  		$LoginRS__query = sprintf("SELECT * FROM invoicing_accounts_summons WHERE type3=%s", GetSQLValueString($loginUsername, "int")); // 分類
				case "3":
		  		$LoginRS__query = sprintf("SELECT * FROM invoicing_accounts_summons WHERE type4=%s", GetSQLValueString($loginUsername, "int")); // 分類
				case "4":
		  		$LoginRS__query = sprintf("SELECT * FROM invoicing_accounts_summons WHERE type5=%s", GetSQLValueString($loginUsername, "int")); // 分類
				break;
				default:
				break;
		  }
	...
	if (isset($_POST[$MM_flag])) {
		...
		foreach($_POST['deltype'] as $key => $val){
		  ...
		  {
			  	case "1":
		  		$MM_dupKeyRedirect="manage_accounts_summons.php?wshop=" . $wshop . "&Opt=sub_mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&item_id=" . $_GET['item_id'] . "&level=" . $_GET['level'] . "&Operate=delErrorT";
				break;
				case "2":
				case "3":
				case "4":
		  		$MM_dupKeyRedirect="manage_accounts_summons.php?wshop=" . $wshop . "&Opt=sub_mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&item_id=" . $_GET['item_id'] . "&subitem_id=" . $row_RecordAccounts_summonsListItem['subitem_id'] . "&level=" . $_GET['level'] . "&Operate=delErrorT";
				break;
				default:
				break;
		  }
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordDateCount = sprintf("SELECT * FROM invoicing_accounts_summonsitem WHERE subitem_id = %s && userid=%s", GetSQLValueString($colname_RecordDateCount, "int"), GetSQLValueString($coluserid_RecordDateCount, "int"));
...
$totalRows_RecordDateCount = mysqli_num_rows($RecordDateCount);
  ...
  {
	// 更新父節點為根節點
  	$updateSQL3 = sprintf("UPDATE invoicing_accounts_summonsitem SET endnode='child' WHERE item_id=%s",
                       GetSQLValueString($_GET['item_id'], "int"));
...
$colitemid_RecordAccounts_summonsListItem = "-1";
if (isset($_GET['item_id'])) {
  $colitemid_RecordAccounts_summonsListItem = $_GET['item_id'];
}
$collevel_RecordAccounts_summonsListItem = "0";
if (isset($_GET['level'])) {
  $collevel_RecordAccounts_summonsListItem = $_GET['level'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAccounts_summonsListItem = sprintf("SELECT * FROM invoicing_accounts_summonsitem WHERE list_id = %s && lang=%s && level = %s && subitem_id = %s && userid=%s ORDER BY sortid ASC, itemvalue ASC", GetSQLValueString($collistid_RecordAccounts_summonsListItem, "int"),GetSQLValueString($collang_RecordAccounts_summonsListItem, "text"),GetSQLValueString($collevel_RecordAccounts_summonsListItem, "int"),GetSQLValueString($colitemid_RecordAccounts_summonsListItem, "int"),GetSQLValueString($coluserid_RecordAccounts_summonsListItem, "int"));
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <?php if($_GET['level'] == '1'){ ?>
    <div class="btn-group pull-right"><a href="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=mulilistitempage_adv&amp;lang=<?php echo $_SESSION['lang']; ?>&list_id=<?php echo $_GET['list_id'] ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
    <?php } ?>
    <?php if($_GET['level'] == '2'){ ?>
    <div class="btn-group pull-right"><a href="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage_adv&amp;lang=<?php echo $_SESSION['lang']; ?>&item_id=<?php echo $row_RecordAccounts_summonsListItemGet['subitem_id'] ?>&level=1&list_id=<?php echo $_GET['list_id'] ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
    <?php } ?>
	<?php if($_GET['level'] == '3'){ ?>
    <div class="btn-group pull-right"><a href="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage_adv&amp;lang=<?php echo $_SESSION['lang']; ?>&item_id=<?php echo $row_RecordAccounts_summonsListItemGet['subitem_id'] ?>&level=2&list_id=<?php echo $_GET['list_id'] ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
    <?php } ?>
	<?php if($_GET['level'] == '4'){ ?>
    <div class="btn-group pull-right"><a href="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage_adv&amp;lang=<?php echo $_SESSION['lang']; ?>&item_id=<?php echo $row_RecordAccounts_summonsListItemGet['subitem_id'] ?>&level=3&list_id=<?php echo $_GET['list_id'] ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
    <?php } ?>
  ...
  <div class="panel-body">
    ...
    <form id="form_Accounts_summonsItemEdit" name="form_Accounts_summonsItemEdit" method="POST" action="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=<?php echo $_GET['Opt'] ?>&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;list_id=<?php echo $_GET['list_id']; ?>&amp;item_id=<?php echo $_GET['item_id'] ?>&amp;level=<?php echo $_GET['level'] ?>" class="form-horizontal form-bordered" data-parsley-validate="">
    <?php do { ?>
          ...
          <div class="card-block">
              ...
              <div class="form-group row">
                  <!--<div class="col-md-6" style="border:0">-->
                      <div class="input-group p-0">
                       <?php if ($_GET['level'] == '') { ?>
                      <a class="btn btn-info btn-block" href="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage_adv&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;item_id=<?php echo $_GET['item_id']; ?>&amp;level=1&amp;list_id=<?php echo $_GET['list_id']; ?>" data-original-title="點選查看下層的分類項目" data-toggle="tooltip" data-placement="top">子分類 <i class="fa fa-chevron-circle-right"></i><?php require("require_manage_accounts_summons_mulilistitem_count.php"); ?></a>  
                       <?php } ?> 
                       <?php if ($_GET['level'] == '1') { ?>
                      <a class="btn btn-info btn-block" href="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage_adv&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;item_id=<?php echo $row_RecordAccounts_summonsListItem['item_id']; ?>&amp;subitem_id=<?php echo $row_RecordAccounts_summonsListItem['subitem_id']; ?>&amp;level=2&amp;list_id=<?php echo $_GET['list_id']; ?>" data-original-title="點選查看下層的分類項目" data-toggle="tooltip" data-placement="top">子分類 <i class="fa fa-chevron-circle-right"></i><?php require("require_manage_accounts_summons_mulilistitem_count.php"); ?></a>  
                       <?php } ?>   
					  <?php if ($_GET['level'] == '2') { ?>
                      <a class="btn btn-info btn-block" href="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage_adv&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;item_id=<?php echo $row_RecordAccounts_summonsListItem['item_id']; ?>&amp;subitem_id=<?php echo $row_RecordAccounts_summonsListItem['subitem_id']; ?>&amp;level=3&amp;list_id=<?php echo $_GET['list_id']; ?>" data-original-title="點選查看下層的分類項目" data-toggle="tooltip" data-placement="top">子分類 <i class="fa fa-chevron-circle-right"></i><?php require("require_manage_accounts_summons_mulilistitem_count.php"); ?></a>  
                       <?php } ?>  
					  <?php if ($_GET['level'] == '3') { ?>
                      <a class="btn btn-info btn-block" href="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage_adv&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;item_id=<?php echo $row_RecordAccounts_summonsListItem['item_id']; ?>&amp;subitem_id=<?php echo $row_RecordAccounts_summonsListItem['subitem_id']; ?>&amp;level=4&amp;list_id=<?php echo $_GET['list_id']; ?>" data-original-title="點選查看下層的分類項目" data-toggle="tooltip" data-placement="top">子分類 <i class="fa fa-chevron-circle-right"></i><?php require("require_manage_accounts_summons_mulilistitem_count.php"); ?></a>  
                       <?php } ?>  
                      </div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <!-- begin panel-body -->
  <div class="panel-body">
      <form id="form_Accounts_summonsItemAdd" name="form_Accounts_summonsItemAdd" method="POST" action="manage_accounts_summons.php?wshop=<?php echo $wshop;?>&amp;Opt=<?php echo $_GET['Opt'] ?>&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;list_id=<?php echo $_GET['list_id']; ?>&amp;item_id=<?php echo $_GET['item_id'] ?>&amp;level=<?php echo $_GET['level'] ?>" class="form-horizontal form-bordered" data-parsley-validate="">
      <div class="card bg-aqua-transparent-1">
          <div class="card-block">
              <div class="row">

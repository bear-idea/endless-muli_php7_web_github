...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_ActnewsItemAdd")) {
  $insertSQL = sprintf("INSERT INTO demo_actnewsitem (list_id, itemname, lang, userid) VALUES (%s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_ActnewsItemEdit")) {
	foreach($_POST['item_id'] as $key => $val){
	  $updateSQL = sprintf("UPDATE demo_actnewsitem SET list_id=%s, sortid=%s, indicate=%s, itemname=%s, lang=%s WHERE item_id=%s",
...
if ((isset($_POST['deltype'])) && ($_POST['deltype'] != "")) {
  $deleteSQL = sprintf("DELETE FROM demo_actnewsitem WHERE item_id in (%s)", implode(",", $_POST['deltype']));
...
if (isset($_GET['lang'])) {
  $collang_RecordActnewsListItem = $_GET['lang'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordActnewsListItem = sprintf("SELECT demo_actnewsitem.item_id, demo_actnewsitem.userid, demo_actnewslist.list_id, demo_actnewslist.listname, demo_actnewsitem.itemname, demo_actnewsitem.sortid, demo_actnewsitem.indicate, demo_actnewsitem.lang FROM demo_actnewslist LEFT OUTER JOIN demo_actnewsitem ON demo_actnewslist.list_id = demo_actnewsitem.list_id WHERE demo_actnewslist.list_id = %s && demo_actnewsitem.lang=%s && demo_actnewsitem.userid=%s ORDER BY demo_actnewsitem.sortid ASC, demo_actnewsitem.item_id DESC", GetSQLValueString($collistid_RecordActnewsListItem, "int"),GetSQLValueString($collang_RecordActnewsListItem, "text"),GetSQLValueString($coluserid_RecordActnewsListItem, "int"));
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> <?php echo $ModuleName['Actnews']; ?> <small>設定</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="manage_actnews.php?wshop=<?php echo $wshop;?>&amp;Opt=listpage&amp;lang=<?php echo $_SESSION['lang']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
  ...
  <div class="panel-body">
    ...
    <form id="form_ActnewsItemEdit" name="form_ActnewsItemEdit" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal form-bordered" data-parsley-validate="">
    <div id="card_sortable">
    <?php do { ?>
    <div class="card bg-aqua-transparent-1" data-post-id="<?php echo $row_RecordActnewsListItem['item_id']; ?>">
          <div class="card-block">
              <div class="row">
              <div class="col-md-3">
                  ...
                  <!--<div class="col-md-6" style="border:0">-->
                      <div class="input-group p-0">
                          <div class="checkbox checkbox-css">
                              <input name="deltype[]" type="checkbox" id="deltype<?php echo $row_RecordActnewsListItem['item_id']; ?>" value="<?php echo $row_RecordActnewsListItem['item_id']; ?>" />
                             <!-- <input type="checkbox" id="cssCheckbox1" />-->
                              <label for="deltype<?php echo $row_RecordActnewsListItem['item_id']; ?>">是否刪除</label>
                            </div>          
                      </div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <!-- begin panel-body -->
  <div class="panel-body">
      <form id="form_ActnewsItemAdd" name="form_ActnewsItemAdd" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal form-bordered" data-parsley-validate="">

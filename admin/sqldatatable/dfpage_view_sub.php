...
//$query_RecordDfPage = sprintf("SELECT * FROM demo_dfpage WHERE title LIKE %s && indicate LIKE %s && userid=%s && lang=%s ORDER BY sortid ASC, id DESC",GetSQLValueString($colsearch_RecordDfPage, "text"), GetSQLValueString($colindicate_RecordDfPage, "text"),GetSQLValueString($coluserid_RecordDfPage, "int"),GetSQLValueString($collang_RecordDfPage, "text"));
	...
	$query_RecordCount = sprintf("SELECT count(id) as sum FROM demo_dfpage WHERE aid=%s && lang = %s && userid=%s",GetSQLValueString($colaid_RecordDfPage, "int"),GetSQLValueString($collang_RecordDfPage, "text"),GetSQLValueString($coluserid_RecordDfPage, "int"));
...
<?php do { ?>
  <?php 
	...
	$link_start = "manage_dfpage.php?wshop=".$wshop."&amp;Opt=startpage&amp;lang=".$_SESSION['lang']."&amp;aid=".$search_aid;
	...
	if($row_RecordDfPage['home'] == 1){$dvalue['title'] = $dvalue['title']."<a href='".$link_start."' class='btn btn-warning btn-xs pull-right'><i class='fa fa-check-circle'></i> 起始頁<a>";}else{$dvalue['title'] = $dvalue['title']."<a href='".$link_start."' class='btn btn-grey btn-xs pull-right'><i class='fa fa-circle'></i> 起始頁<a>";}

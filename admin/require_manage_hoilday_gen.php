...
$query_RecordHoilday = sprintf("SELECT * FROM salary_hoilday WHERE userid=%s",GetSQLValueString($coluserid_RecordHoilday, "int"));
...
if ((isset($_POST["MM_del"])) && ($_POST["MM_del"] == "form_Hoilday") && $totalRows_RecordHoilday > 0 ) {
  $deleteSQL = sprintf("DELETE FROM salary_hoilday WHERE userid=%s",
                       GetSQLValueString($_POST['userid'], "int"));

  //mysqli_select_db($database_DB_Conn, $DB_Conn);
  $Result1 = mysqli_query($DB_Conn, $deleteSQL) or die(mysqli_error($DB_Conn));
  
  $insertGoTo = "manage_hoilday.php?Opt=genpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_Hoilday") && $totalRows_RecordHoilday == 0 ) {
  ...
  foreach($_POST['title'] as $key => $val){
    ...
    $insertSQL = sprintf("INSERT INTO salary_hoilday (title, indicate, hyear, hdate, notes1, lang, userid) VALUES (%s, %s, %s, %s, %s, %s, %s)",
					   ...
					   GetSQLValueString($_POST['hdate'][$key], "date"),
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 例假維護 <small>產生</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      ...
      <div class="alert alert-warning m-5"><i class="fa fa-info-circle"></i> <b>目前產生為 <?php echo $ThisYear; ?>年 之例假日，因每年農曆轉換國曆日期會變動，故此項目需每年重新產生建立。</b></div>
      ...
      <div class="form-group row">
          <label class="col-md-2 col-form-label">例假日<span class="text-red">*</span></label>
          <div class="col-md-3">
          <div class="input-group p-0">

            <div class="input-group-prepend"><span class="input-group-text">日期</span></div>
                   <input name="hdate[]" type="text" class="form-control date-picker" id="hdate[]" value="<?php echo $hdate[$i-1]; ?>" maxlength="10" data-provide="datepicker" data-date-format="mm-dd" data-parsley-trigger="blur" data-date-language="zh-TW" required="" autocomplete="off"/>                        
            </div> 
          ...
          <div class="input-group p-0">
            ...
            <div class="input-group-prepend"><span class="input-group-text">標題</span></div>
                   <input name="title[]" type="text" class="form-control col-md-12" id="title[]" step="1" value="<?php echo $title[$i-1]; ?>" data-parsley-trigger="blur" >                        
            </div> 
          ...
          <div class="input-group p-0">
            ...
            <div class="input-group-prepend"><span class="input-group-text">是否放假</span></div>
                   <select name="indicate[]" id="indicate[]" class="form-control" data-parsley-trigger="blur" required="">
                      <option value="1" <?php if (!(strcmp(1, $indicate[$i-1]))) {echo "selected=\"selected\"";} ?>>是</option>
                      <option value="0" <?php if (!(strcmp(0, $indicate[$i-1]))) {echo "selected=\"selected\"";} ?>>否</option>
                    </select>         
            </div> 
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      ...
      <input type="hidden" name="MM_del" value="form_Hoilday" />
  </form>
  <?php } ?>

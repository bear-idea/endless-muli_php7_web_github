...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Project")) {
  $updateSQL = sprintf("UPDATE demo_projectalbum SET title=%s, author=%s, type=%s, location=%s, sdescription=%s, skeyword=%s, content=%s, startdate=%s, enddate=%s, indicate=%s, postdate=%s, notes1=%s, lang=%s WHERE act_id=%s",
  ...
  $updateGoTo = "manage_project.php?Opt=viewpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
/* 取得發布單位資料 */
$collang_RecordProjectListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $collang_RecordProjectListAuthor = $_GET['lang'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordProjectListAuthor = sprintf("SELECT * FROM demo_projectitem WHERE list_id = 2 && lang = %s && userid=%s", GetSQLValueString($collang_RecordProjectListAuthor, "text"),GetSQLValueString($coluserid_RecordProjectListAuthor, "int"));
...
if (isset($_GET['act_id'])) {
  $colname_RecordProject = $_GET['act_id'];
}
...
?>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <input name="postdate" type="text" class="form-control" id="postdate" value="<?php $dt = new DateTime($row_RecordProject['postdate']); echo $dt->format('Y-m-d'); ?>" maxlength="10" data-provide="datepicker" data-date-format="yyyy-mm-dd" data-date-language="zh-TW" data-parsley-trigger="blur" required=""/> 
      ...
      <div class="form-group row">
        ...
        <div class="col-md-10">
          <button type="submit" class="btn btn btn-primary btn-block">送出</button>
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordProject['id']; ?>" />
            <input name="lang" type="hidden" id="lang" value="<?php echo $row_RecordProject['lang']; ?>" />

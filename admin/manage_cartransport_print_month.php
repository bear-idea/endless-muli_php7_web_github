...
// 日期範圍
$query_RecordScaleorder_in = sprintf("SELECT * FROM erp_scaleorderout WHERE lang = %s && userid=%s && postdate BETWEEN %s AND %s ORDER BY postdate",GetSQLValueString($collang_RecordScaleorder_in, "text"),GetSQLValueString($coluserid_RecordScaleorder_in, "int"),GetSQLValueString($colstartdate_RecordScaleorder_in, "date"),GetSQLValueString($colenddate_RecordScaleorder_in, "date"));
...
$query_RecordScaleListCartransport = sprintf("SELECT * FROM erp_scaleitem WHERE list_id = 5 && lang=%s && userid=%s", GetSQLValueString($colname_RecordScaleListCartransport, "text"),GetSQLValueString($coluserid_RecordScaleListCartransport, "int"));
...
do { 
  $Cartransport['itemname'][] =  $row_RecordScaleListCartransport['itemname'];
...
do{
	...
	//{
		//if($Cartransport['itemname'][$j] == $row_RecordScaleorder_in['cartransport']) { 
		  ...
		  $NowCartransport[$dt->format('Y-m-d')][] = $row_RecordScaleorder_in['cartransport'];
		  }
		//}
	//}
...
<body class="A4 landscape">
        ...
        $ChangePageNumber = cal_days_in_month(CAL_GREGORIAN,$_POST['month'],$_POST['year']);
		?>
    
        <?php //$i = 0; ?>
        <?php for($i=0; $i<cal_days_in_month(CAL_GREGORIAN,$_POST['month'],$_POST['year']); $i++) { ?>
        ...
        <table width="100%" class="paleBlueRows">
          <thead>  
          <tr>
            <td colspan="15" style="text-align:center; color:#fff;"><?php echo $_POST['year'];?>-<?php echo $_POST['month'];?> <strong>車趟統計表</strong></td>
          </tr>
          <tr>
            <td>日期</td>
          ...
          <tr>
			...
			<?php 
				...
				if(count($NowCartransport[$NowDate]) > 0) {
					for($c=0; $c<count($NowCartransport[$NowDate]); $c++)
					{
						if($Cartransport['itemname'][$j] == $NowCartransport[$NowDate][$c])
						{
							//echo "@";
							$NowRun++;
							//var_dump($NowCartransport[$NowDate]);
        ...
        <tfoot>
        <tr>
            <td colspan="15" style="text-align:center; color:#fff;"></td>
          </tr>
          <tr>
            <td></td>
            <?php for($j=0; $j<count($Cartransport['itemname']); $j++) { ?>

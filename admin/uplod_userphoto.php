...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
  ...
  $sip->quality = "100";
  $sip->makeThumb = "true";
  $sip->pathThumb = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Admin")) {
  $updateSQL = sprintf("UPDATE demo_admin SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_userphoto.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_RecordAdmin = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordAdmin = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAdmin = sprintf("SELECT * FROM demo_admin WHERE id = %s", GetSQLValueString($colname_RecordAdmin, "int"));
$RecordAdmin = mysqli_query($DB_Conn, $query_RecordAdmin) or die(mysqli_error($DB_Conn));
$row_RecordAdmin = mysqli_fetch_assoc($RecordAdmin);
$totalRows_RecordAdmin = mysqli_num_rows($RecordAdmin);
?>
<!DOCTYPE html>
<html lang="zh-TW">
<!--<![endif]-->
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordAdmin['pic']; ?>" />
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 3000,  
		<?php if ($row_RecordAdmin['pic'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/<?php echo $row_RecordAdmin['pic']; ?>"],
		<?php } ?>

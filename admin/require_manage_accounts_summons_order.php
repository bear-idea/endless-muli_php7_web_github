...
$query_RecordAccounts_summonsListType = sprintf("SELECT * FROM invoicing_accounts_summonsitem WHERE list_id = 2 && lang=%s && userid=%s ORDER BY sortid ASC", GetSQLValueString($colname_RecordAccounts_summonsListType, "text"),GetSQLValueString($coluserid_RecordAccounts_summonsListType, "int"));
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/accounts_summonsorder_datatable.js"></script>
<!-- ================== END Datatables JS ================== -->
  ...
  <div class="panel-body">
    <div class="row justify-content-end">
      <div class="col-md-3 m-b-10">
        <div class="input-group" data-column="2"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 傳票類別</span></span>
          <select name="type" class="form-control search_filter" id="col2_filter">
            <option value="">全部</option>
            <?php
do {  
?>
            <option value="<?php echo $row_RecordAccounts_summonsListType['itemname']?>"><?php echo $row_RecordAccounts_summonsListType['itemname']?></option>
            <?php
...
?>
	  ...
	  <div class="col-md-3 col-sm-12 m-b-10">
        <div class="input-group" data-column="1"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 時間</span></span>
          <input name="postdate" type="text" class="form-control search_filter" placeholder="" id="col1_filter" autocomplete="off"/>
        </div>
      </div>
      <div class="col-md-4 col-sm-12 m-b-10">
        <div class="input-group" data-column="searchtype"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 搜尋欄位</span></span>
          <select name="searchtype" class="form-control search_filter" id="colsearchtype_filter">
              <option value="">---</option>
			  <option value="summonsnumber">傳票編號</option>
			  <option value="totalnumber">傳票總號</option>
			  <option value="notes1">備註</option>
          </select>
    ...
    <div class="alert alert-warning m-b-10"><i class="fa fa-info-circle"></i> <b>轉帳傳票：傳票單身有【借方金額】和【貸方金額】，選擇此類別，借方科目和貸方科目都要輸入，且借貸金額需平衡</b></div>
    <div class="alert alert-warning m-b-10"><i class="fa fa-info-circle"></i> <b>收入傳票：傳票單身僅有【貸方金額】，只需輸入貸方科目及金額，借方的部份為<a href="manage_accounts_book.php?wshop=<?php echo $wshop;?>&amp;Opt=settingpage&amp;lang=<?php echo $_SESSION['lang']; ?>" class="btn btn-primary btn-xs" data-original-title="前往更改設定" data-toggle="tooltip" data-placement="top"><i class="fa fa-chevron-right"></i> 現金與損益</a>設定中的現金項目。</b></div>
    <div class="alert alert-warning m-b-10"><i class="fa fa-info-circle"></i> <b>支出傳票：傳票單身僅有【借方金額】，只需輸入借方科目及金額，對方科目(貸方科目)的部份同收入傳票設定中的現金項目。</b></div>
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          ...
          <th width="100" data-priority="1"><strong>日期</strong></th>
          <th width="100" data-priority="1"><strong>類別</strong></th>
          <th width="100" data-priority="1"><strong>傳票編號</strong></th>
          <th width="100"><strong>傳票總號</strong></th>
          <th width="100"><strong>總金額</strong></th>
          <th width="100"><strong>來源單據</strong></th>
          <th width="-1"><strong>備註</strong></th>

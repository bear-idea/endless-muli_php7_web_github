<ol class="breadcrumb hidden-xs" data-scroll-reveal='enter top after 0.8s'>
    ...
    <li><a href="<?php echo $SiteBaseUrl . url_rewrite("stronghold",array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'viewpage'),'',$UrlWriteEnable);?>"><?php echo $ModuleName['Stronghold']; //組織成員 ?></a>
    </li>
    <li class="<?php if (isset($_GET['Opt']) && ($_GET['Opt']=='viewpage' || $_GET['Opt']=='typepage')) {echo 'current';} ?>"><a href="#"><?php echo $ViewLinetype; ?></a>
    </li>
	<?php } else { // Show if recordset not empty ?>
    <li class="<?php if ($_GET['Opt']!='detailed') {echo 'current';} ?>"><a href="stronghold"><?php echo $ModuleName['Stronghold']; //組織成員 ?></a></li>
    <?php if (isset($_GET['Opt']) && $_GET['Opt']=='detailed') {?>
    <li class="current"><a><?php echo $row_RecordStrongholdKeyWord['title']; ?></a></li>
    <?php } ?>
    <?php } ?>
</ol>

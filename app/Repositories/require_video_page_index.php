...
$rows_module[$Tp_Page] = Video::select($columns)
...
foreach ($rows_module[$Tp_Page]['data'] as $key => $video) {
	
	$row_data[$key]['id'] = $video['id'];
	$row_data[$key]['name'] = $video['name'];
	$row_data[$key]['pic'] = $video['pic'];
	$row_data[$key]['link_url'] = $SiteBaseUrl . url_rewrite($Tp_MdName,array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'detailed','id'=>$row_RecordVideo['id']),'',$UrlWriteEnable);
	$row_data[$key]['link'] = $video['link'];
	$row_data[$key]['postdate'] = Carbon::parse($video['postdate'])->toDateString();
}

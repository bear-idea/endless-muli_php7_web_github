  ...
  <div id="Left_column">
  	<div id="context">
        ...
        <?php 
			...
			if ($MSLMenuArticlePlus == '1' && $MSLMenuArticlePlusR != '1') {
				include("require_article_leftmenu_vertical_mega_menu.php");
			}
			if ($MSNewsNLink == '1' && $MSNewsNLinkR != '1') {
				include_once("require_news_autolist_scroll.php");
			}
			if ($MSProductHot == '1' && $MSProductHotR != '1') {
				include("require_product_popularity.php");
			} 
			if ($MSFriLinkQLink == '1' && $MSFriLinkQLinkR != '1') {
				include("require_frilink_qlink.php");
			}  
		?>
              	<!-- 粉絲頁 -->
            <?php if ($SiteFBFan != '' && $MSFbFan == '1' && $MSFbFanR != '1' && $TmpFBFanSelect =='1') { ?>
            ...
            <div class="fb-like-box" data-href="<?php echo $SiteFBFan; ?>" data-width="198" data-show-faces="true" data-border-color="<?php echo $TmpFBFanBoardColor; ?>" data-stream="false" data-left="true" style="background-color:<?php echo $TmpFBFanBkColor?>"></div>
            <?php } ?>
            <!-- 粉絲頁 -->
    </div>
  </div>

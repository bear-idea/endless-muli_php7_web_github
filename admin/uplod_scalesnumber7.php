...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/scaleorder";
	$ppu->extensions = "GIF,JPG,JPEG,BMP,PNG";
	$ppu->formName = "form_Scaleorder";
	$ppu->storeType = "file";
	$ppu->sizeLimit = "3000";
...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
  ...
  $sip->quality = "100";
  $sip->makeThumb = "true";
  $sip->pathThumb = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/scaleorder/thumb";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Scaleorder")) {
  $updateSQL = sprintf("UPDATE erp_scaleorderout SET pic7=IFNULL(%s,pic7) WHERE oid=%s",
                       GetSQLValueString($_POST['pic7'], "text"),
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/scaleorder/' . $_POST['oldpic']);
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/scaleorder/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_scalesnumber7.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordScaleorder = sprintf("SELECT * FROM erp_scaleorderout WHERE oid = %s", GetSQLValueString($colname_RecordScaleorder, "int"));
...
<meta content="width=device-width, initial-scaleorder=1.0, maximum-scaleorder=1.0, user-scalable=no" name="viewport" />
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      <div class="form-group row">
          ...
          <div class="col-md-10">
               <input id="pic7" name="pic7" type="file" size="50" maxlength="50" class="file" data-parsley-trigger="blur" data-parsley-errors-container="#error_pic" required/>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordScaleorder['oid']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordScaleorder['pic7']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Scaleorder" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>
...
$(document).ready(function() {
	$("#pic7").fileinput({
		...
		<?php if ($row_RecordScaleorder['pic7'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/scaleorder/thumb/small_<?php echo $row_RecordScaleorder['pic7']; ?>"],
		initialPreviewConfig: [<?php echo "{url: 'sqldatatable/scaleorder_scalesnumber7_photo_del.php?id_del=".$row_RecordScaleorder['oid']."', key: ".$row_RecordScaleorder['oid'].", extra: {id:".$row_RecordScaleorder['oid']."} }"; ?>],
		<?php } ?>

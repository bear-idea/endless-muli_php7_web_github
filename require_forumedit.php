...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Forum_Post")) {
  $updateSQL = sprintf("UPDATE demo_forum SET name=%s, type=%s, content=%s, editdate=%s, lang=%s WHERE id=%s",
  ...
  $updateGoTo = "forum.php?Operate=editSuccess&Opt=viewpage&lang=" . $_POST['lang'] . "&wshop=" . $_POST['wshop'] ;
  if (isset($_SERVER['QUERY_STRING'])) {
...
if (isset($_GET['id_edit'])) {
  $colname_RecordForum = $_GET['id_edit'];
}
...
<?php } else { ?>
<?php include($TplPath . "/forum_edit.php"); ?>
<?php } ?>
<?php

...
$search_pricecheck = $_POST['search_pricecheck'];
...
if(isset($order_column)){
    ...
    switch($i){
        ...
        case 3;$orderSql = " ORDER BY price ".$order_dir;break;
		case 4;$orderSql = " ORDER BY spprice ".$order_dir;break;
		case 5;$orderSql = " ORDER BY costprice ".$order_dir;break;
		case 6;$orderSql = " ORDER BY sortid ".$order_dir;break;
		case 7;$orderSql = " ORDER BY indicate ".$order_dir;break;
		case 8;$orderSql = " ORDER BY pricecheck ".$order_dir;break;
        default;$orderSql = '';
    }
}
if($orderSql == "") {
...
$colpricecheck_RecordProduct = "%";
if (isset($search_pricecheck) && $search_pricecheck != "") {
  if($search_pricecheck == "已審核") {$search_pricecheck = '1';}
  if($search_pricecheck == "未審核") {$search_pricecheck = '0';}
  $colpricecheck_RecordProduct = $search_pricecheck;
}
...
$query_RecordProduct = sprintf("SELECT * FROM demo_product WHERE name LIKE %s && indicate LIKE %s && pricecheck LIKE %s && type1 LIKE %s && type2 LIKE %s && type3 LIKE %s && lang = %s && userid=%s $orderSql", GetSQLValueString("%" . $colsearch_RecordProduct . "%", "text"), GetSQLValueString($colindicate_RecordProduct, "text"), GetSQLValueString($colpricecheck_RecordProduct, "text"),GetSQLValueString($coltype1_RecordProduct, "text"),GetSQLValueString($coltype2_RecordProduct, "text"),GetSQLValueString($coltype3_RecordProduct, "text"),GetSQLValueString($collang_RecordProduct, "text"),GetSQLValueString($coluserid_RecordProduct, "int"));
...
<?php do { ?>
  ...
  <?php 
	...
	$link_edit = "manage_product.php?wshop=".$wshop."&amp;Opt=editpricecheck&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordProduct['id'];
	...
	if($row_RecordProduct["price"] != ""){        
		$dvalue['price'] = "<span id='price_".$row_RecordProduct["id"]."' class='ed_price' data-type='text' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>".$row_RecordProduct["price"]."</span>";
	}else{
		$dvalue['price'] = "<span id='price_".$row_RecordProduct["id"]."' class='ed_price editable-click editable-empty' data-type='text' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>"."Empty"."</span>";
	}
	
	if($row_RecordProduct["spprice"] != ""){ 
		$dvalue['spprice'] = "<span id='spprice_".$row_RecordProduct["id"]."' class='ed_spprice' data-type='text' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>".$row_RecordProduct["spprice"]."</span>";
	}else{
		$dvalue['spprice'] = "<span id='spprice_".$row_RecordProduct["id"]."' class='ed_spprice editable-click editable-empty' data-type='text' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>"."Empty"."</span>";
	}
	
	if($row_RecordProduct["costprice"] != ""){ 
		$dvalue['costprice'] = "<span id='costprice_".$row_RecordProduct["id"]."' class='ed_costprice' data-type='text' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>".$row_RecordProduct["costprice"]."</span>";
	}else{
		$dvalue['costprice'] = "<span id='costprice_".$row_RecordProduct["id"]."' class='ed_costprice editable-click editable-empty' data-type='text' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>"."Empty"."</span>";
	}
	...
	if($row_RecordProduct["pricecheck"] == '1') {$row_RecordProduct["pricecheck"] = "已審核";}else{$row_RecordProduct["pricecheck"] = "未審核";}
	$dvalue['pricecheck'] = "<span id='pricecheck_".$row_RecordProduct["id"]."' class='ed_pricecheck' data-type='select' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>".$row_RecordProduct["pricecheck"]."</span>";

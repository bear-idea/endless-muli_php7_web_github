...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "")) {
  ...
  @unlink($SiteImgFilePathAdmin . $wshop . '/image/dfpage/' . $_GET['pic']);
  @unlink($SiteImgFilePathAdmin . $wshop . '/image/dfpage/thumb/small_' . $_GET['pic']);
}
...
$colnamelang_RecordDfPage = "zh_TW";
if (isset($_GET['lang'])) {
  $colnamelang_RecordDfPage = $_GET['lang'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordDfPage = sprintf("SELECT * FROM demo_dfpage WHERE ((title LIKE %s)) && lang = %s && aid=%s && userid=%s ORDER BY sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordDfPage . "%", "text"),GetSQLValueString($colnamelang_RecordDfPage, "text"),GetSQLValueString($colaid_RecordDfPage, "int"),GetSQLValueString($coluserid_RecordDfPage, "int"));
...
$totalRows_RecordDfPage = mysqli_num_rows($RecordDfPage);$colname_RecordDfPage = "%";
if (isset($_GET['searchkey'])) {
  $colname_RecordDfPage = $_GET['searchkey'];
}
$coluserid_RecordDfPage = "-1";
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordDfPage = sprintf("SELECT * FROM demo_dfpage WHERE ((title LIKE %s)) && lang = %s && aid=%s && userid=%s ORDER BY sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordDfPage . "%", "text"),GetSQLValueString($colnamelang_RecordDfPage, "text"),GetSQLValueString($colaid_RecordDfPage, "text"),GetSQLValueString($coluserid_RecordDfPage, "int"));
$RecordDfPage = mysqli_query($DB_Conn, $query_RecordDfPage) or die(mysqli_error($DB_Conn));
$row_RecordDfPage = mysqli_fetch_assoc($RecordDfPage);
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordDfPageListType = sprintf("SELECT * FROM demo_dfpageitem WHERE list_id = 1 && lang=%s && userid=%s", GetSQLValueString($colname_RecordDfPageListType, "text"),GetSQLValueString($coluserid_RecordDfPageListType, "int"));
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  class="TB_General_style00">
          <tr>
            ...
            <?php if ($ManageDfPageSearchSelect == "1") { ?>
            <form id="form_DfPage" name="form_DfPage" method="get" action="<?php echo $editFormAction; ?>">
              ...
              <label>
                ...
                <input type="text" name="searchkey" id="searchkey" data-bs-original-title="搜尋標題"/>
    ...
    <?php if ($totalRows_RecordDfPage > 0) { // Show if recordset not empty ?>
  ...
  <thead>
    ...
    <?php do { ?>
      <tr>
        ...
        <td><span class="ed_title" id="title_<?php echo $row_RecordDfPage['id']; ?>"><?php echo highLight($row_RecordDfPage['title'], @$_GET['searchkey'], $HighlightSelect); ?></span>
        ...
        <td width="45" align="left" valign="top" class="MenuViewPage"><img src="images/see.gif" width="45" height="18" /></td>
        <td width="45" align="left" valign="top"class="MenuViewPage"><a href="manage_dfpage.php?wshop=<?php echo $wshop;?>&amp;Opt=addpage&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;aid=<?php echo $_GET['aid']; ?>&amp;tpt=<?php echo $row_RecordTpt['title']; ?>"><img src="images/add.gif" width="45" height="18" /></a></td>
        <td width="45" align="left" valign="top" class="MenuViewPage"><a href="manage_dfpage.php?wshop=<?php echo $wshop;?>&amp;Opt=editpage&amp;id_edit=<?php echo $row_RecordDfPage['id']; ?>&amp;aid=<?php echo $row_RecordDfPage['aid']; ?>&amp;lang=<?php echo $_SESSION['lang']; ?>"><img src="images/edit.gif" width="45" height="18" /></a></td>
        <td width="45" align="left" valign="top" class="MenuViewPage"><a href="manage_dfpage.php?<?php echo $_SERVER[QUERY_STRING]; ?>&amp;id_del=<?php echo $row_RecordDfPage['id']; ?>&amp;Operate=delSuccess&amp;pic=<?php echo $row_RecordDfPage['pic']; ?>"><img src="images/del.gif" width="45" height="18" /></a></td>
      </tr>
      ...
      <?php
       ...
       <tr>
        ...
        <td><input name="button3" type="button" id="button3" onClick="flevToggleCheckboxes('form1',false,true);javascript:$(':checkbox').d_checkbox('check');" value="全選" />
          ...
          <span class = "InnerPage" style="float:none;"><a href="#" data-bs-original-title="您可在左方選取您欲刪除的項目，亦可透過左側之按鈕項目來配合使用，選擇完後點選右側《刪除選取項目》按鈕來進行多筆刪除。" data-bs-toggle="tooltip" data-bs-placement="right">?</a></span></td>
        <td>&nbsp;</td>
        <td align="right">&nbsp;</td>
  ...
  <?php } // Show if recordset not empty ?>
    <?php if ($totalRows_RecordDfPage == 0) { // Show if recordset empty ?>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablesorter" id="TBSort">
    <thead>
      <tr>
        <th>&nbsp;</th>

...
<ol class="breadcrumb hidden-xs" data-scroll-reveal='enter top after 0.8s'>
	...
	switch($_GET['Opt'])
	{
		case "viewpage":
	?>
    <li class="current"><a href="<?php echo $SiteBaseUrl . url_rewrite('member',array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'viewpage'),'',$UrlWriteEnable);?>"><?php echo $ModuleName['Member']; //會員專區 ?></a></li>
    <?php 
		break;
		case "editpage":
	?>
    <li class="current"><a href="<?php echo $SiteBaseUrl . url_rewrite('member',array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'viewpage'),'',$UrlWriteEnable);?>"><?php echo $ModuleName['Member']; //會員專區 ?></a></li>
    <li class="current"><a href="#">修改個人資料</a></li>
    <?php 
		break;
		case "regpage":
	?>
    <li class="current"><a>會員註冊</a></li>
    <?php 
		break;
		default:
	?>

...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/tmpblock";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Tmp")) {
  $updateSQL = sprintf("UPDATE demo_tmpblock SET tmp_middle_pic=IFNULL(%s,tmp_middle_pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/tmpblock/' . $_POST['oldpic']);
  //@unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/tmp/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_tmpblockmiddle.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmp = sprintf("SELECT * FROM demo_tmpblock WHERE id = %s", GetSQLValueString($colname_RecordTmp, "int"));
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/tmpblock/<?php echo $row_RecordTmp['tmp_middle_pic']; ?>"],
		<?php } ?>

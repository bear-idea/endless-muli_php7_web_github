...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_Album") && $_POST['sdescription'] == "") { 
	$_POST['sdescription'] = TrimSummary($_POST['content']);
}
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_Album")) {
  $insertSQL = sprintf("INSERT INTO demo_album (title, author, type, location, sdescription, skeyword, content, startdate, enddate, indicate, postdate, notes1, lang, userid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
  ...
  $insertGoTo = "manage_album.php?Opt=viewpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
/* 取得類別資料 */
$colname_RecordAlbumListType = "zh-tw";
if (isset($_GET["lang"])) {
  $colname_RecordAlbumListType = $_GET["lang"];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAlbumListType = sprintf("SELECT * FROM demo_albumitem WHERE list_id = 1 && lang=%s && userid=%s", GetSQLValueString($colname_RecordAlbumListType, "text"),GetSQLValueString($coluserid_RecordAlbumListType, "int"));
...
/* 取得作者資料 */
$colname_RecordAlbumListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordAlbumListAuthor = $_GET['lang'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAlbumListAuthor = sprintf("SELECT * FROM demo_albumitem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordAlbumListAuthor, "text"),GetSQLValueString($coluserid_RecordAlbumListAuthor, "int"));
...
if (isset($_GET['id_edit'])) {
  $colname_RecordAlbum = $_GET['id_edit'];
}
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> <?php echo $ModuleName['Album']; ?> <small>複增</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>

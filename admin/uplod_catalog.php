...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Catalog")) {
  $updateSQL = sprintf("UPDATE demo_catalog SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/catalog/' . $_POST['oldpic']);
  //@unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/catalog/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_catalog.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordCatalog = sprintf("SELECT * FROM demo_catalog WHERE id = %s", GetSQLValueString($colname_RecordCatalog, "int"));
...
<div id="page-container" class="page-header-fixed page-sidebar-fixed page-without-sidebar p-0">
  ...
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 檔案 <small>修改</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordCatalog['id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordCatalog['pic']; ?>" />
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/catalog/<?php echo $row_RecordCatalog['pic']; ?>"],
		<?php } ?>

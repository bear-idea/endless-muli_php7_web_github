...
<h4 class="classic-title"><span><?php echo $ModuleName['Attractions']; // 標題文字 ?></span></h4>
...
$(document).ready(function(){ 
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0"  class="TB_General_style00">
    <tr>
      ...
      $separator = "&nbsp;";
      $max_links = 6;
      $pages_navigation_RecordAttractions = buildNavigation($pageNum_RecordAttractions,$totalPages_RecordAttractions,$prev_RecordAttractions,$next_RecordAttractions,$separator,$max_links,true); 
       ?>
      <?php if ($pageNum_RecordAttractions > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_RecordAttractions=%d%s", $currentPage, 0, $queryString_RecordAttractions); ?>"><i class="fa fa-angle-double-left"></i></a>
...
<?php print $pages_navigation_RecordAttractions[0]; ?> 
      ...
      <?php if ($pageNum_RecordAttractions < $totalPages_RecordAttractions) { // Show if not last page ?>
  <a href="<?php printf("%s?pageNum_RecordAttractions=%d%s", $currentPage, $totalPages_RecordAttractions, $queryString_RecordAttractions); ?>"><i class="fa fa-angle-double-right"></i></a>
  <?php } // Show if not last page ?>
<?php if (ceil($totalRows_RecordAttractions/$maxRows_RecordAttractions) > 1) { ?><span class="Record_Board"><?php echo $Lang_PageNum;; // 頁數?>：<?php print $pageNum_RecordAttractions+1; ?> / <?php echo ceil($totalRows_RecordAttractions/$maxRows_RecordAttractions); ?></span><?php } ?>
      </div>  
 	  ...
 	  <?php $i=$startRow_RecordAttractions + 1; // 取得頁面第一項商品之編號 ?>
          <?php do { ?> 
          <div class="column">
              <div class="container attractions_inner_board">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <div class="attractions_inner_board_relative">
                      <div class="div_table-cell">
                      <?php if ($row_RecordAttractions['pic'] != "") { ?>	 
                    ...
                    <br />
                    <h2><span data-scroll-reveal="enter left after 0.2s"><a href="<?php if($row_RecordAttractions['link'] != ''){echo $row_RecordAttractions['link'];} else {echo "#";}?>" target="_blank"><?php echo $row_RecordAttractions['name']; ?></a></span></h2>
          ...
          <?php if ($i%1 == 0) {echo "<div class=\"column \"><div class=\"container attractions_inner_board\"><hr></div></div>";}?>
...
<script language="javascript" type="text/javascript">
    $(document).ready(function () { /* Div 自動調整100%高度 */
        //$(".attractions_inner_board").css("height", $(".AutoHightTr").height()+"px");

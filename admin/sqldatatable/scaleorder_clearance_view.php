...
if(isset($order_column)){
    ...
    switch($i){
	    ...
	    case 2;$orderSql = " ORDER BY wastecode ".$order_dir;break;
		...
		case 8;$orderSql = " ORDER BY manufacturer ".$order_dir;break;
...
$colstartdate_RecordScaleorder_clearance = "1900-01-01";
...
<?php do { ?>
  <?php 
	...
	$dvalue['type'] = "<span id='type_".$row_RecordScaleorder_clearance["id"]."' class='ed_type' data-type='select' data-pk='".$row_RecordScaleorder_clearance["id"]."' data-placement='top'>".$row_RecordScaleorder_clearance["type"]."</span>";
	
	
	
	// -- 取得物料 --
	$colname_RecordScale = "-1";
	if (isset($row_RecordScaleorder_clearance['code'])) {
	  $colname_RecordScale = $row_RecordScaleorder_clearance['code'];
	}
	...
	if($row_RecordScaleorder_clearance["snumber"] != ""){
		$dvalue['snumber'] = "<span id=snumber_".$row_RecordScaleorder_clearance["id"]."' class='ed_snumber text-lime-darker' data-type='text' data-pk='".$row_RecordScaleorder_clearance["id"]."' data-placement='top'>".$row_RecordScaleorder_clearance["snumber"]."</span>";
	}else{
	...
	$dvalue['postdate'] = "<span id='postdate_".$row_RecordScaleorder_clearance["id"]."' class='ed_postdate' data-type='date' data-format='yyyy-mm-dd' data-pk='".$row_RecordScaleorder_clearance["id"]."' data-placement='top'>".$dt->format('Y-m-d H:i A')."</span>";

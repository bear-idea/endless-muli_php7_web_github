...
$colname_RecordAccounts_summonsorderListType = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordAccounts_summonsorderListType = $_GET['lang'];
}
$coluserid_RecordAccounts_summonsorderListType = "-1";
if (isset($w_userid)) {
  $coluserid_RecordAccounts_summonsorderListType = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAccounts_summonsorderListType = sprintf("SELECT * FROM invoicing_accounts_summonsitem WHERE list_id = 2 && lang=%s && userid=%s ORDER BY sortid ASC", GetSQLValueString($colname_RecordAccounts_summonsorderListType, "text"),GetSQLValueString($coluserid_RecordAccounts_summonsorderListType, "int"));
$RecordAccounts_summonsorderListType = mysqli_query($DB_Conn, $query_RecordAccounts_summonsorderListType) or die(mysqli_error($DB_Conn));
$row_RecordAccounts_summonsorderListType = mysqli_fetch_assoc($RecordAccounts_summonsorderListType);
$totalRows_RecordAccounts_summonsorderListType = mysqli_num_rows($RecordAccounts_summonsorderListType);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/accounts_summonsbook_detailledger_datatable.js"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 明細分類帳 <small>總覽</small>
...
?>
          ...
          <select name="type5" class="form-control search_filter" id="coltype5_filter">
            <option value="-1">-- 選擇分類5 --</option>
          </select>
          <input id="fullIdPath" type="hidden" />
        </div>
      </div>
	  <div class="col-md-3 col-sm-12 m-b-10">
        <div class="input-group" data-column="3"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 時間</span></span>
          <input name="postdate" type="text" class="form-control search_filter" placeholder="" id="col3_filter" autocomplete="off"/>
        </div>
      </div>
      
      <div class="col-md-3 m-b-10">
        <div class="input-group" data-column="4"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 傳票類別</span></span>
          <select name="type" class="form-control search_filter" id="col4_filter">
            <option value="">全部</option>
            <?php
do {  
?>
            <option value="<?php echo $row_RecordAccounts_summonsorderListType['itemname']?>"><?php echo $row_RecordAccounts_summonsorderListType['itemname']?></option>
            <?php
} while ($row_RecordAccounts_summonsorderListType = mysqli_fetch_assoc($RecordAccounts_summonsorderListType));
  $rows = mysqli_num_rows($RecordAccounts_summonsorderListType);
  if($rows > 0) {
      mysqli_data_seek($RecordAccounts_summonsorderListType, 0);
	  $row_RecordAccounts_summonsorderListType = mysqli_fetch_assoc($RecordAccounts_summonsorderListType);
  }
?>
          </select>
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          ...
          <th width="100"><strong>傳票日期</strong></th>
          <th width="100"><strong>類別</strong></th>
          <th width="100"><strong>傳票編號</strong></th>
          <th width="100"><strong>借方金額</strong></th>
          <th width="100"><strong>貸方金額</strong></th>
          <th width="100"><strong>期初餘額</strong></th>
				...
				$('#coltype4_filter').removeOption(/.?/).ajaxAddOption(
					...
					function () {
						...
						// 值=val() // 標籤=text
						{
							...
							//$("#coltype4_filter").show();
							// 觸發第五階下拉式選單
							$('#coltype5_filter').removeOption(/.?/).ajaxAddOption(
								'selectbox_action/accounts_summons_show.php?&<?php echo time();?>', 
								{ 'id': $(this).val(), 'lv': 4 }, 
								false, // true/false 的功能在於是否要瀏覽器記住次選單的選項
								function () { 
										...
										// 設定欄位隱藏/開啟
										if( $('#coltype4_filter option:selected').val() != '' && $('#coltype5_filter option:selected').val() != '')
										// 值=val() // 標籤=text
										{
											$("#coltype5_filter").show();
											//$("#coltype4_filter").val(-1);
										}else{
											$("#coltype5_filter").hide();
											$("#coltype5_filter").val('-1');
            ...
            // 觸發第五階下拉式選單
				$('#coltype5_filter').removeOption(/.?/).ajaxAddOption(
					...
					function () {
						...
						// 設定預設選項
						if (defaultValue) {
							$(this).selectOptions($fullIdPath[4]);
						}
						// 設定欄位隱藏/開啟
						if( $('#coltype4_filter option:selected').val() != '' && $('#coltype5_filter option:selected').val() != '')
						// 值=val() // 標籤=text
...
$(document).ready(function() {
  $('input[name="postdate"]').daterangepicker({
    ...
    endDate: "<?php $dt = new DateTime(); echo $dt->format('Y-m-t'); ?>",

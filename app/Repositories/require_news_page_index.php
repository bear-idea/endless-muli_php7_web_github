...
$table = new Table;
//$app->bind( 'Tlr\Tables\Elements\Table', function(){return new Tlr\Tables\Elements\Table;});
//$table = $app->make(Tlr\Tables\Elements\Table::class);
...
foreach ($row_data as $row_data) {
    ...
    $row->linkCell($row_data['link_url'])->content($row_data['title']);
    $row->cell($row_data['postdate'])->class('width-100 hidden-xs');
    //$row_data['postdate'] = $dt->format('Y-m-d');
}

//$dt = new DateTime($news['postdate']);

...
if (isset($_POST['Wshop_account']) && isset($_SESSION["fb_id"])  && isset($_SESSION['success_fb_login_backstage_'.$_SESSION['login_wshop']]) ) {
  ...
  $LoginRS__query=sprintf("SELECT account, psw, level, fbid, userid FROM demo_member WHERE account=%s && psw=%s && userid=%s",
  ...
  if ($loginFoundUser) {
	...
	if($_SESSION["fb_id"] != "") {
		...
		$query_RecordThirdparty = sprintf("SELECT fbid FROM demo_member WHERE fbid = %s && userid=%s", GetSQLValueString($colname_RecordThirdparty, "text"), GetSQLValueString($_SESSION['userid'], "int"));
	...
	$updateThirdpartySQL = sprintf("UPDATE demo_member SET fbid=%s WHERE account=%s && userid=%s",
	...
	$updateSQL = sprintf("UPDATE demo_member SET logdate=NOW() WHERE fbid=%s",
                       GetSQLValueString($_SESSION["fb_id"], "text"));
	//mysqli_select_db($database_DB_Conn, $DB_Conn);	
...
請更換您登入的 Facebook 帳號！！ 
        ...
        <form id="ADReg" name="ADReg" method="post" action="reg.php" class="">
            <div class="text-center">
              <button type="submit" class="btn btn-block btn-social btn-facebook margin-top-10"><i class="fa fa-facebook"></i> 使用此 Facebook 註冊新帳號</button>
            </div>
...
<footer id="footer" class="sticky">
	<div class="copyright" style="background-color:#333; color:#FFF; text-align:center; padding:10px;">
		<div class="container_full">
			<?php //require_once("../../../require_tmpfooter_mobile.php"); ?>
		</div>
	</div>
</footer>

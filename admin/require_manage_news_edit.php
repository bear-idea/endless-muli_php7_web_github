...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_News") && $_POST['sdescription'] == "") { 
	$_POST['sdescription'] = TrimSummary($_POST['content']);
}
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_News")) {
  $updateSQL = sprintf("UPDATE demo_news SET title=%s, author=%s, type=%s, content=%s, postdate=%s, indicate=%s, sdescription=%s, skeyword=%s, skeywordindicate=%s, pushtop=%s, notes1=%s, lang=%s WHERE id=%s",
  ...
  $updateGoTo = "manage_news.php?Opt=viewpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
?>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <input name="postdate" type="text" class="form-control" id="postdate" value="<?php $dt = new DateTime($row_RecordNews['postdate']); echo $dt->format('Y-m-d'); ?>" maxlength="10" data-provide="datepicker" data-date-format="yyyy-mm-dd" data-date-language="zh-TW" data-parsley-trigger="blur" required="" autocomplete="off"/> 

...
try
{
	...
	//基本參數(請依系統規劃自行調整)
	$oPayment->Send['ReturnURL']         = "http://" . $_SERVER['HTTP_HOST'] . "/allpay_order_get_cart.php?wshop=" . $_POST['wshop'];    //付款完成通知回傳的網址
...
'Currency' => "元(NTD.)", 'Quantity' => (int) $_POST['dcquantiry'][$i], 'URL' => ""));
}
if($_POST['ocfreightprice'] != '') {
	if($_POST['ocfreepriceok'] == '1' && $_POST['ocfreightprice'] == '0')
	{
		array_push($oPayment->Send['Items'], array('Name' => "運費(滿額免運)", 'Price' => (int) $_POST['ocfreightprice'],'Currency' => "元(NTD.)", 'Quantity' => (int) "1", 'URL' => ""));
	}else{
		array_push($oPayment->Send['Items'], array('Name' => "運費", 'Price' => (int) $_POST['ocfreightprice'],'Currency' => "元(NTD.)", 'Quantity' => (int) "1", 'URL' => ""));
	}
}
if($_POST['ocotherprice'] != '0' && $_POST['ocotherprice'] != '') {
array_push($oPayment->Send['Items'], array('Name' => "金物流加收", 'Price' => (int) $_POST['ocotherprice'],'Currency' => "元(NTD.)", 'Quantity' => (int) "1", 'URL' => ""));
}
if($_POST['ocexprice'] != '0' && $_POST['ocexprice'] != '') {
array_push($oPayment->Send['Items'], array('Name' => $_POST['ocexpricename'], 'Price' => (int) $_POST['ocexprice'],'Currency' => "元(NTD.)", 'Quantity' => (int) "1", 'URL' => ""));
}
if($_POST['ocinvoiceprice'] != '0' && $_POST['ocinvoiceprice'] != '') {
array_push($oPayment->Send['Items'], array('Name' => "營業稅", 'Price' => (int) $_POST['ocinvoiceprice'],'Currency' => "元(NTD.)", 'Quantity' => (int) "1", 'URL' => ""));
}
/* 產生訂單 */
$oPayment->CheckOut();

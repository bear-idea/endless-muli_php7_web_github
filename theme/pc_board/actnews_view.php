...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  class="TB_General_style00">
  <tr>
    <td width="50%"><?php echo $Lang_Content_Count_Display; // 顯示 ?> <?php echo ($startRow_RecordActnews + 1) ?> - <?php echo min($startRow_RecordActnews + $maxRows_RecordActnews, $totalRows_RecordActnews) ?> <?php echo $Lang_Content_Count_Lots; //筆 ?> <?php echo $Lang_Content_Count_Total; // 共計?> <?php echo $totalRows_RecordActnews ?> <?php echo $Lang_Content_Count_Lots; //筆 ?></td>
    <td width="50%" align="right"><?php if ($ActnewsSearchSelect == "1") { ?>
      <form id="form_Actnews" name="form_Actnews" method="get" action="<?php echo $editFormAction; ?>">
        <label>
      ...
      $prev_RecordActnews = "<i class=\"fa fa-angle-left\"></i>";
      $next_RecordActnews = "<i class=\"fa fa-angle-right\"></i>";
      $separator = "&nbsp;";
      $max_links = 6;
      $pages_navigation_RecordActnews = buildNavigation($page,$totalPages_RecordActnews,$prev_RecordActnews,$next_RecordActnews,$separator,$max_links,true); 
       ?>
        <?php if ($page > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?page=%d%s", $currentPage, 0, $queryString_RecordActnews); ?>"><i class="fa fa-angle-double-left"></i></a>
        <?php } // Show if not first page ?>
        <?php print $pages_navigation_RecordActnews[0]; ?> <?php print $pages_navigation_RecordActnews[1]; ?> <?php print $pages_navigation_RecordActnews[2]; ?>
        <?php if ($page < $totalPages_RecordActnews) { // Show if not last page ?>
        <a href="<?php printf("%s?page=%d%s", $currentPage, $totalPages_RecordActnews, $queryString_RecordActnews); ?>"><i class="fa fa-angle-double-right"></i></a>
        <?php } // Show if not last page ?>
        <?php if (ceil($totalRows_RecordActnews/$maxRows_RecordActnews) > 1) { ?>
        <span class="Record_Board"><?php echo $Lang_PageNum;; // 頁數?>：<?php print $page+1; ?> / <?php echo ceil($totalRows_RecordActnews/$maxRows_RecordActnews); ?></span>
        <?php } ?>
      </div></td>
  </tr>
</table>

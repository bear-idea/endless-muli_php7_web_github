...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_Accounts_summonsItemAdd")) {
  $insertSQL = sprintf("INSERT INTO invoicing_accounts_summonsitem (list_id, itemname, lang, userid) VALUES (%s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Accounts_summonsItemEdit")) {
	foreach($_POST['item_id'] as $key => $val){
	  $updateSQL = sprintf("UPDATE invoicing_accounts_summonsitem SET list_id=%s, sortid=%s, indicate=%s, itemname=%s, lang=%s WHERE item_id=%s",
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAccounts_summonsListItem = sprintf("SELECT invoicing_accounts_summonsitem.item_id, invoicing_accounts_summonsitem.userid, invoicing_accounts_summonslist.list_id, invoicing_accounts_summonslist.listname, invoicing_accounts_summonsitem.itemname, invoicing_accounts_summonsitem.sortid, invoicing_accounts_summonsitem.indicate, invoicing_accounts_summonsitem.lang FROM invoicing_accounts_summonslist LEFT OUTER JOIN invoicing_accounts_summonsitem ON invoicing_accounts_summonslist.list_id = invoicing_accounts_summonsitem.list_id WHERE invoicing_accounts_summonslist.list_id = %s && invoicing_accounts_summonsitem.lang=%s && invoicing_accounts_summonsitem.userid=%s ORDER BY invoicing_accounts_summonsitem.sortid ASC, invoicing_accounts_summonsitem.item_id DESC", GetSQLValueString($collistid_RecordAccounts_summonsListItem, "int"),GetSQLValueString($collang_RecordAccounts_summonsListItem, "text"),GetSQLValueString($coluserid_RecordAccounts_summonsListItem, "int"));
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-body">
    ...
    <form id="form_Accounts_summonsItemEdit" name="form_Accounts_summonsItemEdit" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal form-bordered" data-parsley-validate="">
    <?php do { ?>
              ...
              <div class="form-group row">
                  <!--<div class="col-md-6" style="border:0">-->
                      <div class="input-group p-0">
                            ...
                            <input name="sortid[]" type="number" id="sortid[]" value="<?php echo $row_RecordAccounts_summonsListItem['sortid']; ?>" class=" form-control" maxlength="10" data-parsley-trigger="blur" required=""/>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <!-- begin panel-body -->
  <div class="panel-body">
      <form id="form_Accounts_summonsItemAdd" name="form_Accounts_summonsItemAdd" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal form-bordered" data-parsley-validate="">
      <div class="card bg-aqua-transparent-1">
          <div class="card-block">
              <div class="row">

...
//定义查询数据总记录数sql
//$sumSql = sprintf("SELECT count(id) as sum FROM erp_scaleorder_in WHERE userid=%s && lang=%s",GetSQLValueString($coluserid_RecordScaleorder_in, "int"),GetSQLValueString($colnamelang_RecordScaleorder_in, "text"));


//mysqli_select_db($database_DB_Conn, $DB_Conn);
//$query_RecordScaleorder_in = sprintf("SELECT * FROM erp_scaleorder_in WHERE title LIKE %s && indicate LIKE %s && userid=%s && lang=%s ORDER BY sortid ASC, id DESC",GetSQLValueString($colsearch_RecordScaleorder_in, "text"), GetSQLValueString($colindicate_RecordScaleorder_in, "text"),GetSQLValueString($coluserid_RecordScaleorder_in, "int"),GetSQLValueString($collang_RecordScaleorder_in, "text"));

$query_RecordCount = sprintf("SELECT count(id) as sum FROM erp_scaleorderindetail WHERE bound = 'in' && lang = %s && userid=%s",GetSQLValueString($collang_RecordScaleorder_in, "text"),GetSQLValueString($coluserid_RecordScaleorder_in, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);


$query_RecordScaleorder_in = sprintf("SELECT * FROM erp_scaleorderindetail WHERE (title LIKE binary %s || num = %s) && (author LIKE binary %s || author IS NULL) && (people LIKE binary %s || people IS NULL) && indicate LIKE %s && (type LIKE %s || type IS NULL) && lang = %s && userid=%s && bound ='in' && postdate BETWEEN %s AND %s $orderSql", GetSQLValueString("%" . $colsearch_RecordScaleorder_in . "%", "text"), GetSQLValueString( $colsearch_RecordScaleorder_in, "text"), GetSQLValueString("%" . $colauthor_RecordScaleorder_in . "%", "text"), GetSQLValueString("%" . $colpeople_RecordScaleorder_in . "%", "text"), GetSQLValueString($colindicate_RecordScaleorder_in, "text"), GetSQLValueString($coltype_RecordScaleorder_in, "text"),GetSQLValueString($collang_RecordScaleorder_in, "text"),GetSQLValueString($coluserid_RecordScaleorder_in, "int"),GetSQLValueString($colstartdate_RecordScaleorder_in, "date"),GetSQLValueString($colenddate_RecordScaleorder_in, "date"));
...
<?php do { ?>
  <?php 
    ...
    $link_add = "manage_scaleorder_in.php?wshop=".$wshop."&amp;Opt=addpage&amp;lang=".$_SESSION['lang'];
	$link_copy = "manage_scaleorder_in.php?wshop=".$wshop."&amp;Opt=copypage&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordScaleorder_in['id'];
	$link_edit = "manage_scaleorder_in.php?wshop=".$wshop."&amp;Opt=editpage&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordScaleorder_in['id'];
	$link_start = "manage_scaleorder_in.php?wshop=".$wshop."&amp;Opt=startpage&amp;lang=".$_SESSION['lang'];
	...
	$but_del = "<a href='".$link_del."' class='btn btn-xs btn-danger' style='text-align:center' onclick='delete_datatables(".$row_RecordScaleorder_in["id"].",event);'><i class='far fa-trash-alt'></i> 刪除</a>";

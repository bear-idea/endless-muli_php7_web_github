...
<?php //do { ?>
  <?php 
	...
	if($row_RecordAccounts_summonsdetail["state"] == 'system'){
		...
		$but_add = "<a href='javascript:;' class='btn btn-xs btn-primary dictpush-plus' style='text-align:center'><i class='fa fa-plus'></i> 新增</a>";
	...
	$dvalue['detail_type'] = 'detail_type';
	
	$dvalue['summonsnumber'] = $row_RecordAccounts_summonsdetail["summonsnumber"];
	
	//if ($row_RecordAccounts_summonsorder['type'] == '收入'){
		$dvalue['debitamount'] = "<input name='debitamount[]' id='debitamount[]' value='0' maxlength='11' class='form-control debitamount' data-parsley-min='0' data-parsley-max='9999999' type='number' data-parsley-type='number' data-parsley-trigger='blur' step='1' onfocus='this.select()' required='required'/>";
	//}else{

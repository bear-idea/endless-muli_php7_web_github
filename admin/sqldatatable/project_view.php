...
if (isset($search_type) && $search_type != "") {
  $coltype_RecordProject = $search_type;
}
...
$query_RecordCount = sprintf("SELECT count(act_id) as sum FROM demo_projectalbum WHERE lang = %s && userid=%s",GetSQLValueString($collang_RecordProject, "text"),GetSQLValueString($coluserid_RecordProject, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);


$query_RecordProject = sprintf("SELECT demo_projectalbum.act_id, demo_projectalbum.userid, demo_projectalbum.title, demo_projectalbum.type, demo_projectalbum.sdescription, demo_projectalbum.indicate, demo_projectalbum.author, demo_projectalbum.postdate, demo_projectalbumphoto.pic, demo_projectalbum.sortid, demo_projectalbumphoto.actphoto_id, demo_projectalbum.lang, count(demo_projectalbumphoto.act_id) AS photonum FROM demo_projectalbum LEFT OUTER JOIN demo_projectalbumphoto ON demo_projectalbum.act_id = demo_projectalbumphoto.act_id GROUP BY demo_projectalbum.act_id HAVING (demo_projectalbum.lang = %s) && (demo_projectalbum.type LIKE %s) && (demo_projectalbum.title LIKE %s) && demo_projectalbum.userid=%s ORDER BY demo_projectalbum.act_id DESC", GetSQLValueString($collang_RecordProject, "text"),GetSQLValueString($coltype_RecordProject, "text"),GetSQLValueString("%" . $colsearch_RecordProject . "%", "text"),GetSQLValueString($coluserid_RecordProject, "int"));
...
<?php do { ?>
  <?php 
    ...
    $link_add = "manage_project.php?wshop=".$wshop."&amp;Opt=addpage&amp;lang=".$_SESSION['lang'];
	$link_copy = "manage_project.php?wshop=".$wshop."&amp;Opt=copypage&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordProject['act_id'];
	$link_edit = "manage_project.php?wshop=".$wshop."&amp;Opt=editpage&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordProject['act_id'];
	$link_mutiphoto = "inner_project.php?wshop=".$wshop."&amp;Opt=photoviewpage&amp;lang=".$_SESSION['lang']."&amp;act_id=".$row_RecordProject['act_id'];
    //$link_del = "#modal-alert";
	
	$but_add = "<a href='".$link_add."' class='btn btn-xs btn-primary' style='text-align:center'><i class='fa fa-plus'></i> 新增</a>";
	$but_copy = "<a href='".$link_copy."' class='btn btn-xs btn-primary' style='text-align:center'><i class='fa fa-copy'></i> 複增</a>";
	$but_edit = "<a href='".$link_edit."' class='btn btn-xs btn-primary' style=text-align:center'><i class='fa fa-edit'></i> 修改</a>";
	$but_del = "<a href='".""."' class='btn btn-xs btn-danger' style='text-align:center' onclick='delete_datatables(".$row_RecordProject["act_id"].",event);'><i class='far fa-trash-alt'></i> 刪除</a>";
	$but_mutiphoto = "<a href='".$link_mutiphoto."' class='btn btn-xs btn-primary colorbox_iframe_cd' style=text-align:center'><i class='fa fa-image'></i> 圖片一覽</a>";
  	...
  	//$nestedData = array();
	$dvalue['id'] = $row_RecordProject["act_id"];
	
  	$dvalue['chk'] = "<input name='delProject[]' type='checkbox' id='delProject[]' value='".$row_RecordProject["act_id"]."\'/>";
	
	$dvalue['title'] = "<span id='title_".$row_RecordProject["act_id"]."' class='ed_title' data-type='text' data-pk='".$row_RecordProject["act_id"]."' data-placement='top'>".$row_RecordProject["title"]."</span>";
	
	$dvalue['type'] = "<span id='type_".$row_RecordProject["act_id"]."' class='ed_type' data-type='select' data-pk='".$row_RecordProject["act_id"]."' data-placement='top'>".$row_RecordProject["type"]."</span>";
	
	$dvalue['sortid'] = "<span id='sortid_".$row_RecordProject["act_id"]."' class='ed_sortid' data-type='text' data-pk='".$row_RecordProject["act_id"]."' data-placement='top'>".$row_RecordProject["sortid"]."</span>";
	
	if($row_RecordProject["indicate"] == '1') {$row_RecordProject["indicate"] = "公佈";}else{$row_RecordProject["indicate"] = "隱藏";}
	$dvalue['indicate'] = "<span id='indicate_".$row_RecordProject["act_id"]."' class='ed_indicate' data-type='select' data-pk='".$row_RecordProject["act_id"]."' data-placement='top'>".$row_RecordProject["indicate"]."</span>";
	
	$dt = new DateTime($row_RecordProject['postdate']); 
	$dvalue['postdate'] = "<span id='postdate_".$row_RecordProject["act_id"]."' class='ed_postdate' data-type='date' data-format='yyyy-mm-dd' data-pk='".$row_RecordProject["act_id"]."' data-placement='top'>".$dt->format('Y-m-d')."</span>";
	
	$dvalue['photonum'] = "<span class='label label-secondary'>".$row_RecordProject['photonum']."</span>";

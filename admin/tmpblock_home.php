...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE demo_tmp SET tmpblock=%s WHERE id=%s",
                       GetSQLValueString($_POST['TmpBlockSelect'], "text"),
...
$colname_RecordTmpBlockBoardStyle = "-1";
if (isset($_GET['id'])) {
  $colname_RecordTmpBlockBoardStyle = $_GET['id'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpBlockBoardStyle = sprintf("SELECT * FROM demo_tmp WHERE id = %s", GetSQLValueString($colname_RecordTmpBlockBoardStyle, "int"));
$RecordTmpBlockBoardStyle = mysqli_query($DB_Conn, $query_RecordTmpBlockBoardStyle) or die(mysqli_error($DB_Conn));
$row_RecordTmpBlockBoardStyle = mysqli_fetch_assoc($RecordTmpBlockBoardStyle);
$totalRows_RecordTmpBlockBoardStyle = mysqli_num_rows($RecordTmpBlockBoardStyle);

$colid_RecordTmpBlock = "-1";
if (isset($row_RecordTmpBlockBoardStyle['tmpblock'])) {
  $colid_RecordTmpBlock = $row_RecordTmpBlockBoardStyle['tmpblock'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpBlock = sprintf("SELECT * FROM demo_tmpblock WHERE id=%s && (userid=%s || userid=1)", GetSQLValueString($colid_RecordTmpBlock, "int"),GetSQLValueString($coluserid_RecordTmpBlock, "int"));
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/tmpblock_datatable_get.js"></script>
<!-- ================== END Datatables JS ================== -->
...
<div id="page-container" class="page-header-fixed page-sidebar-fixed page-without-sidebar p-0">
  ...
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 側邊裝飾外框 <small>選擇</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="tmp_config_<?php echo $row_RecordTmpBlockBoardStyle['name']; // 為版型風格?>.php?lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $row_RecordTmpBlockBoardStyle['id']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
  ...
  <div class="row">
      <div class="col-md-12">
        <div class="card bg-grey-transparent-1">
          <div class="card-block">
            <?php if($row_RecordTmpBlockBoardStyle['tmpblock'] != "") { ?>
            
            <?php if ($SiteBaseUrlOuter != '' && $row_RecordTmpBlock['userid'] == '1') { ?>
            <div class="pull-left m-r-10" style="width:160px">
              <span class="label label-purple" data-original-title="目前套用背景編號" data-toggle="tooltip" data-placement="right">#No.<?php echo $row_RecordTmpBlock['id']; ?></span>
              <span class="label label-purple"><?php echo $row_RecordTmpBlock['type']; ?></span>
              <div class="img-thumbnail m-b-5 m-t-5">
              <div class="" style="width:100%; height:150px;">
              	<?php if ($row_RecordTmpBlock['tmp_title_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpBlock['webname']; ?>/image/tmpblock/<?php echo $row_RecordTmpBlock['tmp_title_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
                <?php if ($row_RecordTmpBlock['tmp_middle_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpBlock['webname']; ?>/image/tmpblock/<?php echo $row_RecordTmpBlock['tmp_middle_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
                <?php if ($row_RecordTmpBlock['tmp_bottom_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpBlock['webname']; ?>/image/tmpblock/<?php echo $row_RecordTmpBlock['tmp_bottom_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
              </div>
              </div>
            ...
            <div class="pull-left m-r-10" style="width:160px">
              ...
              <div class="" style="width:100%; height:150px;">
              	<?php if ($row_RecordTmpBlock['tmp_title_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpBlock['webname']; ?>/image/tmpblock/<?php echo $row_RecordTmpBlock['tmp_title_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
                <?php if ($row_RecordTmpBlock['tmp_middle_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpBlock['webname']; ?>/image/tmpblock/<?php echo $row_RecordTmpBlock['tmp_middle_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
                <?php if ($row_RecordTmpBlock['tmp_bottom_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpBlock['webname']; ?>/image/tmpblock/<?php echo $row_RecordTmpBlock['tmp_bottom_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
              </div>
              </div>
            ...
            <?php } else { ?>
            <div class="alert alert-danger m-t-5 m-b-0"><i class="fa fa-info-circle"></i> <b>目前尚未套用任何<span class="card-title m-0">側邊裝飾外框</span>。</b></div>
            <?php } ?>
          </div>
        </div>
      </div>
    ...
    <table id="data-table-default" class="table table-bordered table-hover cards" style="width:100%">
      <thead>
        <tr>
          ...
          <th data-priority="1"><strong>可選擇側邊裝飾外框<div id="error_action"></div></strong></th>
        </tr>
      </thead>
      <tfoot>
        <tr>
         
          <td><span class="label label-danger p-10"><div class='radio radio-css radio-inline'><input type='radio' name='TmpBlockSelect' value='' id='TmpBlockSelect_none' data-parsley-trigger='blur' data-parsley-errors-container='#error_action'><label for='TmpBlockSelect_none' class='text-white'>不使用任何側邊裝飾外框</label></div></span><button type="button" id="reset_table" class="btn btn-default btn-sm pull-right"><i class="fa fa-sync"></i> 清除狀態</button></td>
        </tr>
      </tfoot>
    </table>
...
<script>
	$(document).ready(function() {
	  ...
	  $("#TransparentButtom1").click(function(){
		...
		<?php if ($row_RecordTmpBlock['tmpleftfontcolor'] == "transparent") { ?>
			$("#tmpleftfontcolor").val("transparent");
		<?php } ?>
		...
		<?php if ($row_RecordTmpBlock['tmpmiddlefontcolor'] == "transparent") { ?>
			$("#tmpmiddlefontcolor").val("transparent");
		<?php } ?>
	});
</script>
...
mysqli_free_result($RecordTmpBlockBoardStyle);

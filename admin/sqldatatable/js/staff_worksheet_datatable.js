var handleDataTableDefault = function() {
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
            ...
            "autoWidth": false,
            "ajax": {
                "url": "sqldatatable/staff_worksheet_view.php",
                "type": "post",
				"data": function ( d ) {
					...
					d.search_department = $('#col4_filter').val();
					d.search_job = $('#col5_filter').val();
			...
			"stateLoadParams": function (settings, data) {
				...
				$("#col3_filter").val(data.search.field.type);
            ...
            "fnDrawCallback": function(oSettings) {
				...
				$('#data-table-default').editable({
                    selector: ".ed_worksheet",
                    url: 'sqledit/staff_jedit.php',
                    name: "worksheet",
                    source: "sqledit/staff_get_list_worksheet.php",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/staff_del.php?id_del=" + id,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/staff_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

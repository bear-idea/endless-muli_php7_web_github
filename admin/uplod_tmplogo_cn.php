...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	$ppu = new pureFileUpload();
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/logo";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Tmplogo")) {
  echo $updateSQL = sprintf("UPDATE demo_tmplogo SET logoimage_cn=IFNULL(%s,logoimage_cn), width=%s, height=%s WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $wshop . '/logo/' . $_POST['oldpic']);
  //@unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/tmplogo/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
...
$colname_RecordTmplogo = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordTmplogo = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmplogo = sprintf("SELECT * FROM demo_tmplogo WHERE id = %s", GetSQLValueString($colname_RecordTmplogo, "int"));
...
<div id="page-container" class="page-header-fixed page-sidebar-fixed page-without-sidebar p-0">
  ...
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 圖片 <small>修改</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      <div class="form-group row">
          ...
          <div class="col-md-10">
               <input id="logoimage" name="logoimage" type="file" size="50" maxlength="50" class="file" data-parsley-trigger="blur" data-parsley-errors-container="#error_pic" required/>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
                      <input name="width" type="number" required class="form-control" id="width" value="<?php echo $row_RecordTmplogo['width']; ?>" maxlength="4" data-parsley-min="0" data-parsley-type="number" data-parsley-trigger="blur"/>
                      
                      <div class="alert alert-warning m-t-5 m-b-0"><i class="fa fa-info-circle"></i> <b>目前寬度以圖片<?php if($defaultlang == "zh-tw") {echo "【繁體】";}elseif($defaultlang == "zh-cn"){echo "【简体】";}elseif($defaultlang == "en"){echo "【English】";}elseif($defaultlang == "jp"){echo "【日本語】";} ?>為準。</b></div>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
                      <input name="height" type="number" required class="form-control" id="height" value="<?php echo $row_RecordTmplogo['height']; ?>" maxlength="4" data-parsley-min="0" data-parsley-type="number" data-parsley-trigger="blur"/>
                      
                      <div class="alert alert-warning m-t-5 m-b-0"><i class="fa fa-info-circle"></i> <b>目前高度以圖片<?php if($defaultlang == "zh-tw") {echo "【繁體】";}elseif($defaultlang == "zh-cn"){echo "【简体】";}elseif($defaultlang == "en"){echo "【English】";}elseif($defaultlang == "jp"){echo "【日本語】";} ?>為準。</b></div>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordTmplogo['id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordTmplogo['logoimage_cn']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
            <?php if($defaultlang == "zh-cn") { ?>
            <?php } else { ?>
            <input name="width" type="hidden" id="width" value="<?php echo $row_RecordTmplogo['width']; ?>" />
            <input name="height" type="hidden" id="height" value="<?php echo $row_RecordTmplogo['height']; ?>" />
            <?php } ?>
          </div>
      </div>
...
$(document).ready(function() {
	$("#logoimage").fileinput({
		...
		maxFileSize: 2000,  
		<?php if ($row_RecordTmplogo['logoimage_cn'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/logo/<?php echo $row_RecordTmplogo['logoimage_cn']; ?>"],
		<?php } ?>

...
/* 取得發佈者資料 */
$colname_RecordCarnumberListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordCarnumberListAuthor = $_GET['lang'];
}
$coluserid_RecordCarnumberListAuthor = "-1";
if (isset($w_userid)) {
  $coluserid_RecordCarnumberListAuthor = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordCarnumberListAuthor = sprintf("SELECT * FROM erp_carnumberitem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordCarnumberListAuthor, "text"),GetSQLValueString($coluserid_RecordCarnumberListAuthor, "int"));
$RecordCarnumberListAuthor = mysqli_query($DB_Conn, $query_RecordCarnumberListAuthor) or die(mysqli_error($DB_Conn)); 
$row_RecordCarnumberListAuthor = mysqli_fetch_assoc($RecordCarnumberListAuthor);
$totalRows_RecordCarnumberListAuthor = mysqli_num_rows($RecordCarnumberListAuthor);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/carnumber_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 車號管理 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
do {  
?>
                  <option value="<?php echo $row_RecordCarnumberListType['itemname']?>"><?php echo $row_RecordCarnumberListType['itemname']?></option>
                  <?php
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_carnumber.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_carnumber.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',
...
mysqli_free_result($RecordCarnumberListAuthor);
?>

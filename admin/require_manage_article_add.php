...
$query_RecordArticleListType = sprintf("SELECT * FROM demo_articleitem WHERE list_id = 1 && lang=%s && level='0' && userid=%s ORDER BY sortid ASC, item_id DESC", GetSQLValueString($colname_RecordArticleListType, "text"),GetSQLValueString($coluserid_RecordArticleListType, "int"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_Article") && $_POST['sdescription'] == "") { 
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_Article")) {
  $insertSQL = sprintf("INSERT INTO demo_article (title, type1, type2, type3, content, indicate, skeyword, sdescription, notes1, lang, userid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> <?php echo $ModuleName['Article']; ?> <small>新增</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
?>
      ...
      <input type="hidden" name="MM_insert" value="form_Article" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>

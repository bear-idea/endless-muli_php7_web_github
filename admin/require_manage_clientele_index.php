...
/* 取得發佈者資料 */
$colname_RecordClienteleListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordClienteleListAuthor = $_GET['lang'];
}
$coluserid_RecordClienteleListAuthor = "-1";
if (isset($w_userid)) {
  $coluserid_RecordClienteleListAuthor = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordClienteleListAuthor = sprintf("SELECT * FROM invoicing_clienteleitem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordClienteleListAuthor, "text"),GetSQLValueString($coluserid_RecordClienteleListAuthor, "int"));
$RecordClienteleListAuthor = mysqli_query($DB_Conn, $query_RecordClienteleListAuthor) or die(mysqli_error($DB_Conn)); 
$row_RecordClienteleListAuthor = mysqli_fetch_assoc($RecordClienteleListAuthor);
$totalRows_RecordClienteleListAuthor = mysqli_num_rows($RecordClienteleListAuthor);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/clientele_datatable.js"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 客戶管理 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
do {  
?>
                  <option value="<?php echo $row_RecordClienteleListType['itemname']?>"><?php echo $row_RecordClienteleListType['itemname']?></option>
                  <?php
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_clientele.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_clientele.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',
...
mysqli_free_result($RecordClienteleListAuthor);
?>

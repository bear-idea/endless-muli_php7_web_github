...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Employees")) {
  $updateSQL = sprintf("UPDATE demo_employees SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/employees/' . $_POST['oldpic']);
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/employees/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_employees.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordEmployees = sprintf("SELECT * FROM demo_employees WHERE id = %s", GetSQLValueString($colname_RecordEmployees, "int"));
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 3000,  
		<?php if ($row_RecordEmployees['pic'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/employees/thumb/small_<?php echo $row_RecordEmployees['pic']; ?>"],
		<?php } ?>

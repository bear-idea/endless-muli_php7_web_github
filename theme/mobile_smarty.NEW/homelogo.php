...
<?php if ($_GET['wshop'] == $tplwebname) { //如果目前之網頁為Logo之作者 則 使用個別樣板 之 Logo 設定 會依樣板之logo設定變動 ?>
    ...
    <?php if ($TmpHomeLogo != "" && GetFileExtend($TmpHomeLogo) != '.swf') { ?>
        <span data-scroll-reveal='enter top over 1s after 0.5s'><a href="index.php?wshop=<?php echo $_GET['wshop'] ?>"><img src="<?php echo $SiteImgUrl; ?><?php echo $TmpHomeLogoWebname; ?>/logo/<?php echo $TmpHomeLogo; ?>" width="<?php echo $TmpHomeLogoWidth; ?>" height="<?php echo $TmpHomeLogoHeight; ?>"/></a></span>
...
<?php } else { // 若不為作者 則 使用此使用者個別設定黨之預設值 Logo 只有一個?>
	<div id="Homelogo" style="position:absolute;">
       ...
       <?php if ($TmpLogoDefaultImg != "" && GetFileExtend($TmpLogoDefaultImg) != '.swf') { ?>
        <span  data-scroll-reveal='enter top over 1s after 0.5s'><a href="index.php?wshop=<?php echo $_GET['wshop'] ?>"><img src="<?php echo $SiteImgUrl; ?><?php echo $TmpLogoDefaultWebName; ?>/logo/<?php echo $TmpLogoDefaultImg; ?>" /></a></span>

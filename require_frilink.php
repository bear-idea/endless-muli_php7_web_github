...
function buildNavigation($page,$totalPages_RecordFrilink,$prev_RecordFrilink,$next_RecordFrilink,$separator=" | ",$max_links=10, $show_page=true)
{
    GLOBAL $maxRows_RecordFrilink,$totalRows_RecordFrilink;
	$pagesArray = ""; $firstArray = ""; $lastArray = "";
	if($max_links<2)$max_links=2;
	if($page<=$totalPages_RecordFrilink && $page>=0)
	{
		if ($page > ceil($max_links/2))
		{
			$fgp = $page - ceil($max_links/2) > 0 ? $page - ceil($max_links/2) : 1;
			$egp = $page + ceil($max_links/2);
			if ($egp >= $totalPages_RecordFrilink)
			{
				$egp = $totalPages_RecordFrilink+1;
				$fgp = $totalPages_RecordFrilink - ($max_links-1) > 0 ? $totalPages_RecordFrilink  - ($max_links-1) : 1;
			}
		}
		else {
			$fgp = 0;
			$egp = $totalPages_RecordFrilink >= $max_links ? $max_links : $totalPages_RecordFrilink+1;
		}
		if($totalPages_RecordFrilink >= 1) {
			#	------------------------
			...
			$successivo = $page+1;
			$precedente = $page-1;
			$firstArray = ($page > 0) ? "<a href=\"$_SERVER[PHP_SELF]?page=$precedente$_get_vars\">$prev_RecordFrilink</a>" :  "<span>$prev_RecordFrilink</span>";/* css */
			# ----------------------
			# page numbers
			# ----------------------
			for($a = $fgp+1; $a <= $egp; $a++){
				...
				if($show_page)
				{
					...
					$min_l = (($a-1)*$maxRows_RecordFrilink) + 1;
					$max_l = ($a*$maxRows_RecordFrilink >= $totalRows_RecordFrilink) ? $totalRows_RecordFrilink : ($a*$maxRows_RecordFrilink);
			...
			$theNext = $page+1;
			$offset_end = $totalPages_RecordFrilink;
			$lastArray = ($page < $totalPages_RecordFrilink) ? "<a href=\"$_SERVER[PHP_SELF]?page=$successivo$_get_vars\">$next_RecordFrilink</a>" : "<span>$next_RecordFrilink</span>"; /* css */
		}
	}
...
$maxRows_RecordFrilink = 24;
$page = 0;
if (isset($_GET['page'])) {
  $page = $_GET['page'];
}
$startRow_RecordFrilink = $page * $maxRows_RecordFrilink;

$colname_RecordFrilink = "%";
if (isset($_GET['searchkey'])) {
  $colname_RecordFrilink = $_GET['searchkey'];
}
$coluserid_RecordFrilink = "-1";
if (isset($_SESSION['userid'])) {
  $coluserid_RecordFrilink = $_SESSION['userid'];
}
$colnamelang_RecordFrilink = "zh-tw";
if (isset($_GET['lang'])) {
  $colnamelang_RecordFrilink = $_GET['lang'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordFrilink = sprintf("SELECT * FROM demo_frilink WHERE (name LIKE %s || type LIKE %s) && lang = %s && userid=%s && typemenu = 'Link' ORDER BY sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordFrilink . "%", "text"),GetSQLValueString("%" . $colname_RecordFrilink . "%", "text"),GetSQLValueString($colnamelang_RecordFrilink, "text"),GetSQLValueString($coluserid_RecordFrilink, "int"));
...
$queryString_RecordFrilink = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "page") == false && 
        stristr($param, "totalRows_RecordFrilink") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_RecordFrilink = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_RecordFrilink = sprintf("&totalRows_RecordFrilink=%d%s", $totalRows_RecordFrilink, $queryString_RecordFrilink);
...
.frilink_outer_board tr td{
	margin: 0px;
	padding: 0px;
}

/* 外框 */
div .frilink_inner_board{
...
.frilink_inner_board_relative{
	position: relative; /* FF 定位 */
}

.frilink_inner_board_relative_buttom{
	position: relative; /* FF 定位 */
}
...
div .frilink_inner_board_context{
	text-align: left;
}
</style>
...
<div class="columns on-1">
        <div class="container">
            <div class="column">
                ...
                <?php echo $Lang_Content_Title_Frilink; // 標題文字 ?></h3>
                </div>
            </div>
        </div>        
</div>
    ...
    <tr>
      ...
      <form id="form_Frilink" name="form_Frilink" method="get" action="<?php echo $editFormAction; ?>">
        <label>
          ...
          <input type="submit" name="button" id="button" value="<?php echo $Lang_Form_Search_Frilink; ?>" />
        </label>
      </form>
      <?php } ?>
                  ...
                  <tr>
                    ...
                    <div class='photoFram_Block_paper-clip'></div>
                      ...
                      <?php if ($row_RecordFrilink['pic'] != "") { ?>	 
                            <a href="<?php echo $row_RecordFrilink['link']; ?>" target="_blank"><img src="upload/image/frilink/<?php echo $row_RecordFrilink['pic']; ?>" alt="<?php echo $row_RecordFrilink['sdescription']; ?>" alumb="true" _w="198" _h="60"/></a><span></span>
                      <?php } else { ?>      
...
<script language="javascript" type="text/javascript">
    $(document).ready(function () { /* Div 自動調整100%高度 */
        //$(".frilink_inner_board").css("height", $(".AutoHightTr").height()+"px");
...
<?php } else { ?>
<?php include($TplPath . "/frilink_view.php"); ?>
<?php } ?>
<?php

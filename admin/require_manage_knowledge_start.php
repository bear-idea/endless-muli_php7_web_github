...
$query_RecordKnowledge = sprintf("SELECT * FROM demo_knowledge WHERE ((title LIKE %s)) && lang = %s && userid=%s ORDER BY sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordKnowledge . "%", "text"),GetSQLValueString($colnamelang_RecordKnowledge, "text"),GetSQLValueString($coluserid_RecordKnowledge, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
	if ((isset($_POST['id_check'])) && ($_POST['id_check'] != "")) {
		// 初始化此頁面所有文章
	  $updateResetSQL = sprintf("UPDATE demo_knowledge SET home=0 WHERE id in (%s)", implode(",", $_POST['id']));
	  //mysqli_select_db($database_DB_Conn, $DB_Conn);
	  $Result = mysqli_query($DB_Conn, $updateResetSQL) or die(mysqli_error($DB_Conn));
	  // 更新選取文章
	  $updateSQL = sprintf("UPDATE demo_knowledge SET home=1 WHERE id = %s",
      ...
      <div class="form-group row">
        ...
        <div class="col-md-12">
                      <div class="checkbox checkbox-css">
                          <input name="id_check" <?php if (!(strcmp($row_RecordKnowledge['home'],1))) {echo "checked=\"checked\"";} ?> value="<?php echo $row_RecordKnowledge['id']; ?>" type="radio" id="id_check_<?php echo $row_RecordKnowledge['id']; ?>" data-parsley-trigger="blur" required=""/>
                        <label for="id_check_<?php echo $row_RecordKnowledge['id']; ?>"><?php echo $row_RecordKnowledge['title']; ?></label>
                        <?php if($row_RecordKnowledge['home'] == 1){ ?>

...
$query_RecordCount = sprintf("SELECT count(id) as sum FROM demo_bannershow WHERE (userid=%s || userid=1)",GetSQLValueString($coluserid_RecordTmpBanner, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);


$query_RecordTmpBanner = sprintf("SELECT * FROM demo_bannershow WHERE (id LIKE %s) && indicate LIKE %s && (userid=%s || userid=1) $orderSql", GetSQLValueString("%" . $colsearch_RecordTmpBanner . "%", "text"), GetSQLValueString($colindicate_RecordTmpBanner, "text"),GetSQLValueString($coluserid_RecordTmpBanner, "int"));
...
<?php do { ?>
  <?php 
	...
	$dvalue['action'] = "<div class='radio radio-css radio-inline'><input type='radio' name='tmpselectbannerid' value='".$row_RecordTmpBanner["id"]."' id='tmpselectbannerid_".$row_RecordTmpBanner["id"]."' data-parsley-trigger='blur' data-parsley-errors-container='#error_action' required=''><label for='tmpselectbannerid_".$row_RecordTmpBanner["id"]."'>選擇目前橫幅圖片</label></div>";

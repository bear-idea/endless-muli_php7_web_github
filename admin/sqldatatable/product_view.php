...
<?php do { ?>
  ...
  <?php 
	...
	$pagelink = $SiteBaseUrl . url_rewrite(strtolower('product'),array('wshop'=>$wshop,'lang'=>$_SESSION['lang'],'Opt'=>'detailed','id'=>$row_RecordProduct['id']),'',$UrlWriteEnable);
	
	$dvalue['name'] .= "<div>"."<a href='$pagelink' class='btn btn-link btn-xs'>".$pagelink."</a>"."</div>";
  ...
  switch($row_RecordProduct['plot'])
	...
	$dvalue['action'] = "<div><div class='btn-group'>".$but_copy.$but_add.$but_edit.$but_del."</div>";
	$dvalue['action'] .= "<div class='btn-group'>".$but_mainphoto.$but_mutiphoto.$but_tab."</div>";
	if($_SESSION['MM_UserGroup'] == 'superadmin' || $OptionCartSelect == '1'){
		$dvalue['action'] .= "<div class='btn-group'>".$but_format."</div></div>";
	}else{
		$dvalue['action'] .= "</div>";
	}

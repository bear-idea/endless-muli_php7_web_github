...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE demo_tmp SET tmpleftmenu=%s WHERE id=%s",
                       GetSQLValueString($_POST['TmpLeftMenuSelect'], "text"),
...
$colname_RecordTmpLeftMenuBoardStyle = "-1";
if (isset($_GET['id'])) {
  $colname_RecordTmpLeftMenuBoardStyle = $_GET['id'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpLeftMenuBoardStyle = sprintf("SELECT * FROM demo_tmp WHERE id = %s", GetSQLValueString($colname_RecordTmpLeftMenuBoardStyle, "int"));
$RecordTmpLeftMenuBoardStyle = mysqli_query($DB_Conn, $query_RecordTmpLeftMenuBoardStyle) or die(mysqli_error($DB_Conn));
$row_RecordTmpLeftMenuBoardStyle = mysqli_fetch_assoc($RecordTmpLeftMenuBoardStyle);
$totalRows_RecordTmpLeftMenuBoardStyle = mysqli_num_rows($RecordTmpLeftMenuBoardStyle);

$colid_RecordTmpLeftMenu = "-1";
if (isset($row_RecordTmpLeftMenuBoardStyle['tmpleftmenu'])) {
  $colid_RecordTmpLeftMenu = $row_RecordTmpLeftMenuBoardStyle['tmpleftmenu'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpLeftMenu = sprintf("SELECT * FROM demo_tmpleftmenu WHERE id=%s && (userid=%s || userid=1)", GetSQLValueString($colid_RecordTmpLeftMenu, "int"),GetSQLValueString($coluserid_RecordTmpLeftMenu, "int"));
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/tmpleftmenu_datatable_get.js"></script>
<!-- ================== END Datatables JS ================== -->
...
<div id="page-container" class="page-header-fixed page-sidebar-fixed page-without-sidebar p-0">
  ...
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 選單樣式 <small>選擇</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="tmp_config_<?php echo $row_RecordTmpLeftMenuBoardStyle['name']; // 為版型風格?>.php?lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $row_RecordTmpLeftMenuBoardStyle['id']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
  ...
  <div class="row">
      <div class="col-md-12">
        <div class="card bg-grey-transparent-1">
          <div class="card-block">
            <?php if($row_RecordTmpLeftMenuBoardStyle['tmpleftmenu'] != "") { ?>
            
            <?php if ($SiteBaseUrlOuter != '' && $row_RecordTmpLeftMenu['userid'] == '1') { ?>
            <div class="pull-left m-r-10" style="width:160px">
              <span class="label label-purple" data-original-title="目前套用背景編號" data-toggle="tooltip" data-placement="right">#No.<?php echo $row_RecordTmpLeftMenu['id']; ?></span>
              <span class="label label-purple"><?php echo $row_RecordTmpLeftMenu['type']; ?></span>
              <div class="img-thumbnail m-b-5 m-t-5">
              <div class="" style="width:100%; height:100px;">
              	<?php if ($row_RecordTmpLeftMenu['tmp_title_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_title_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
                <?php if ($row_RecordTmpLeftMenu['tmp_middle_o_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_middle_o_pic']; ?>" style="display:block" class="width-full"/>
                <?php } else { ?>
                <img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_middle_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
                <img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_middle_pic']; ?>" style="display:block" class="width-full"/>
                <?php if ($row_RecordTmpLeftMenu['tmp_bottom_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_bottom_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
              </div>
              </div>
            ...
            <div class="pull-left m-r-10" style="width:160px">
              ...
              <div class="" style="width:100%; height:100px;">
              	<?php if ($row_RecordTmpLeftMenu['tmp_title_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_title_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
                <?php if ($row_RecordTmpLeftMenu['tmp_middle_o_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_middle_o_pic']; ?>" style="display:block" class="width-full"/>
                <?php } else { ?>
                <img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_middle_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
                <img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_middle_pic']; ?>" style="display:block" class="width-full"/>
                <?php if ($row_RecordTmpLeftMenu['tmp_bottom_pic'] != '') { ?>
                <img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpLeftMenu['webname']; ?>/image/tmpleftmenu/<?php echo $row_RecordTmpLeftMenu['tmp_bottom_pic']; ?>" style="display:block" class="width-full"/>
                <?php } ?>
              </div>
              </div>
            ...
            <?php } else { ?>
            <div class="alert alert-danger m-t-5 m-b-0"><i class="fa fa-info-circle"></i> <b>目前尚未套用任何<strong>選單樣式</strong>。</b></div>
            <?php } ?>
          </div>
        </div>
      </div>
    ...
    <table id="data-table-default" class="table table-bordered table-hover cards" style="width:100%">
      <thead>
        <tr>
          ...
          <th data-priority="1"><strong>可選擇選單樣式<div id="error_action"></div></strong></th>
        </tr>
      </thead>
      <tfoot>
        <tr>
...
<script>
	$(document).ready(function() {
	  ...
	  $("#TransparentButtom1").click(function(){
		...
		<?php if ($row_RecordTmpLeftMenu['tmpleftfontcolor'] == "transparent") { ?>
			$("#tmpleftfontcolor").val("transparent");
		<?php } ?>
		...
		<?php if ($row_RecordTmpLeftMenu['tmpmiddlefontcolor'] == "transparent") { ?>
			$("#tmpmiddlefontcolor").val("transparent");
		<?php } ?>
	});
</script>
...
mysqli_free_result($RecordTmpLeftMenuBoardStyle);

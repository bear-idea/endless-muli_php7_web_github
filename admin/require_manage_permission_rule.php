...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/permission_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 權限規則 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
  ...
  <div class="panel-body">
    <div class="row justify-content-end">
      <div class="col-md-2 col-sm-12 m-b-10">
        <div class="input-group" data-column="3"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i>模組</span></span>
            <select name="module" class="form-control search_filter" id="col3_filter">
            <option value="" selected="selected">全部</option>
              <?php
do {  
?>
                  <option value="<?php echo $row_RecordModList['itemvalue']?>"><?php echo $row_RecordModList['itemname']?> - <?php echo $row_RecordModList['itemvalue']?></option>
                  <?php
} while ($row_RecordModList = mysqli_fetch_assoc($RecordModList));
  $rows = mysqli_num_rows($RecordModList);
  if($rows > 0) {
      mysqli_data_seek($RecordModList, 0);
	  $row_RecordModList = mysqli_fetch_assoc($RecordModList);
  }
?>
          </select>
      ...
      <div class="col-md-2 col-sm-12 m-b-10">
        <div class="input-group" data-column="4"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i>模組類別群組</span></span>
            <select name="type" class="form-control search_filter" id="col4_filter">
            <option value="" selected="selected">全部</option>
              <?php
do {  
?>
                  <option value="<?php echo $row_RecordPermissionListType['itemvalue']?>"><?php echo $row_RecordPermissionListType['itemname']?></option>
                  <?php
...
?>
      ...
      <div class="col-md-2 col-sm-12 m-b-10" style="display:none">
        <div class="input-group" data-column="5"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 使用者群組</span></span>
            <select name="usegroup" class="form-control search_filter" id="col5_filter">
            <option value="" selected="selected">全部</option>
              <?php
do {  
?>
              <option value="<?php echo $row_RecordPermissionListAuthor['itemvalue']?>"><?php echo $row_RecordPermissionListAuthor['itemname']?></option>
                  <?php
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          ...
          <th width="100" data-priority="1"><strong>功能代號</strong></th>
          <th width="100" data-priority="1"><strong>模組</strong></th>
          <th width="100" data-priority="1"><strong>模組類別群組</strong></th>
          <th width="100" data-priority="1"><strong>使用者群組</strong></th>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_permission.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_permission.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',

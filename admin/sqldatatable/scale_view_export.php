...
if(isset($order_column)){
    ...
    switch($i){
		...
		case 4;$orderSql = " ORDER BY code ".$order_dir;break;
		case 5;$orderSql = " ORDER BY title ".$order_dir;break;
        case 6;$orderSql = " ORDER BY state ".$order_dir;break;
        case 7;$orderSql = " ORDER BY splitscale ".$order_dir;break;
        default;$orderSql = '';
    }
}
if($orderSql == "") {
	$orderSql = $orderSql . "ORDER BY type DESC";
}else{
	$orderSql = $orderSql . ",type DESC";
}
...
<?php do { ?>
  <?php 
	...
	//mysqli_select_db($database_DB_Conn, $DB_Conn);
	$query_RecordScaleViewLine_l1 = sprintf("SELECT * FROM erp_scaleitem WHERE list_id = 1 && lang = %s && item_id = %s && userid=%s ORDER BY sortid ASC, item_id DESC", GetSQLValueString($collang_RecordScaleViewLine_l1, "text"),GetSQLValueString($row_RecordScale['type1'], "int"),GetSQLValueString($coluserid_RecordScaleViewLine_l1, "int"));
	...
	$dvalue['type'] = $row_RecordScaleViewLine_l1["itemname"];
	
	$dvalue['code'] = $row_RecordScale["code"];

	$dvalue['wastecode'] = $row_RecordScaleViewLine_l1["itemvalue"];
	...
	if($row_RecordScale["type"] != ""){
		$dvalue['bigtype'] = "<span id='type_".$row_RecordScale["id"]."' class='ed_type' data-type='select' data-pk='".$row_RecordScale["id"]."' data-placement='top'>".$row_RecordScale["type"]."</span>";
	}else{
		$dvalue['bigtype'] = "<span id='type_".$row_RecordScale["id"]."' class='ed_type' data-type='select' data-pk='".$row_RecordScale["id"]."' data-placement='top'>"."<span class='editable-empty'>".'Empty'."</span>"."</span>";
	}
	...
	if($row_RecordScale["state"] == 'pay' || $row_RecordScale["state"] == 'buy') {$row_RecordScale["state"] = "應付(我方付費，對方取款)";}

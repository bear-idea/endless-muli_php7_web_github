...
$query_RecordNews = sprintf("SELECT * FROM demo_news WHERE (type LIKE %s || skeyword LIKE %s) && (indicate=1) && (lang = %s)  && userid=%s ORDER BY pushtop DESC, sortid ASC, id DESC", GetSQLValueString("%" . $colname_RecordNews . "%", "text"),GetSQLValueString("%" . $colname_RecordNews . "%", "text"),GetSQLValueString($collang_RecordNews, "text"),GetSQLValueString($coluserid_RecordNews, "int"));

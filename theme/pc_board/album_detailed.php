...
<?php if (ceil($totalRows_RecordAlbum/$maxRows_RecordAlbum) > 1) { ?><span class="Record_Board"><?php echo $Lang_PageNum;; // 頁數?>：<?php print $page+1; ?> / <?php echo ceil($totalRows_RecordAlbum/$maxRows_RecordAlbum); ?></span><?php } ?>
        ...
        <?php if ($row_RecordAlbum['location'] != '') { ?><?php echo $Lang_Classify_Context_Location_Album //地點： ?><?php echo $row_RecordAlbum['location']; ?><?php } ?> <?php if  ($row_RecordAlbum['startdate'] != '' || $row_RecordAlbum['enddate'] != '') { ?><strong><?php echo $Lang_Classify_Context_Date_Album //時間： ?></strong><?php echo $row_RecordAlbum['startdate']; ?><?php if  ($row_RecordAlbum['startdate'] != '' && $row_RecordAlbum['enddate'] != '') { ?>~<?php } ?><?php echo $row_RecordAlbum['enddate']; ?><?php } ?>
        </div>
 ...
 <?php if ($row_RecordAlbum['content'] != '') { ?>
 <hr />
 <?php } ?>
 <div class="columns on-3">
      ...
      <?php $m_count=1; ?>
          <?php do { ?> 
          <div class="column">
              <div class="container album_inner_board" data-scroll-reveal="enter top after <?php echo $m_count/100; ?>s">
                   <!-- 內容 -->
                   <div class="photoFrame_<?php echo $TmpAlbumBoard; ?>">
                	...
                	<div class='<?php echo $TmpAlbumBoardIcon; ?>'></div>
                       ...
                       <?php if ($row_RecordAlbum['pic'] != "") { ?>
                         <a href="<?php echo $SiteImgUrl; ?><?php echo $_GET['wshop']; ?>/image/album/<?php echo $row_RecordAlbum['pic']; ?>" title="<?php echo $row_RecordAlbum['name']; ?>" rel="prettyPhoto[pp_gal]"><img src="<?php echo $SiteImgUrl; ?><?php echo $_GET['wshop']; ?>/image/album/thumb/small_<?php echo GetFileThumbExtend($row_RecordAlbum['pic']); ?>" alt="<?php echo $row_RecordAlbum['sdescription']; ?>" /></a><span></span>
                         <?php } else { ?>      

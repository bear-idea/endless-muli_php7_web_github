var handleDataTableDefault = function() {
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
            ...
            "autoWidth": false,
            "ajax": {
                "url": "sqldatatable/supplier_view.php",
                "type": "post",
				"data": function ( d ) {
            ...
            'columnDefs': [{
                ...
                'render': function(data, type, full, meta) {
					...
					return '<div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input data-check" id="checkbox' + $('<div/>').text(data).html() + '" name="id[]" value="' + $('<div/>').text(data).html() + '"><label class="custom-control-label" for="checkbox' + $('<div/>').text(data).html() + '"></label></div>';
					}
                }],
			"select": true,
            ...
            "fnDrawCallback": function(oSettings) {
                ...
                $('#data-table-default').editable({
                    selector: ".ed_title",
                    url: 'sqledit/supplier_jedit.php',
                    type: 'text',
                    name: 'title',
                    //title: '輸入標題',
				...
				$('#data-table-default').editable({
                    ...
                    url: 'sqledit/supplier_jedit.php',
                    name: "type",
                    source: "sqledit/supplier_get_list.php?list_id=1",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/supplier_del.php?id_del=" + id,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/supplier_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

...
if (isset($_POST['MM_insert']) && isset($_SESSION["line_id"])) {
  $loginThirdpartyID=$_SESSION["line_id"];
  //$loginUsername=$_POST['Wshop_account'];
  //$password=$_POST['psw'];
  $MM_fldUserAuthorization = "level";
  echo $MM_redirectLoginSuccess = "../../../Thirdparty/line/oauth/login.php?wshop=" . $_POST['wshop'];
  ...
  if($totalRows_LoginRS > 0){ // 此ID已被註冊
	  ...
	  echo $insertSQL = sprintf("INSERT INTO demo_member (lineid, name, mail, regdate, level, indicate, agreeprovision, agreemailsend, lang, auth, userid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_SESSION["line_id"], "text"),
                       GetSQLValueString($_SESSION["line_name"], "text"),

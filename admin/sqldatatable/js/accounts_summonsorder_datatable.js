var handleDataTableDefault = function() {
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
            ...
            "autoWidth": false,
            "ajax": {
                "url": "sqldatatable/accounts_summonsorder_view.php",
                "type": "post",
				"data": function ( d ) {
					...
					d.search_searchtype = $('#colsearchtype_filter').val();
					if(d.search_searchtype != ''){
						$('#global_filter').attr('disabled', false);
					}else{
						$('#global_filter').attr('disabled', true);
						$('#global_filter').val('');
					}
					//console.log($('#colsearchtype_filter').val());
			...
			"stateSaveParams": function (settings, data) {
				...
				data.search.field = {
					...
					searchtype: $("#colsearchtype_filter").val(),
			...
			"stateLoadParams": function (settings, data) {
				...
				$("#colsearchtype_filter").val(data.search.field.searchtype);
            ...
            "fnDrawCallback": function(oSettings) {
                ...
                $('#data-table-default').editable({
                    ...
                    url: 'sqledit/accounts_summonsorder_jedit.php',
                    type: 'text',
                    name: 'supplier',
                    //title: '輸入標題',
				...
				$('#data-table-default').editable({
                    ...
                    url: 'sqledit/accounts_summonsorder_jedit.php',
                    name: "type",
                    source: "sqledit/accounts_summonsorder_get_list.php?list_id=1",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/accounts_summonsorder_del.php?id_del=" + id,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/accounts_summonsorder_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

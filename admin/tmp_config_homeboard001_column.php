...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "") && $_GET['userid'] == $w_userid) {
  ...
  $deleteGoTo = "tmp_config_" . $_GET['board'] . "_column.php?lang=" . $_SESSION['lang'] . "&board=" . $_GET['board'] . "&id_edit=" . $_GET['id_edit'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSettingLock = sprintf("SELECT HomeproductlistLock, HomefrilinkLock, HomemodlinkLock, HomearticlelistLock, HomefbfanLock, HomealllistLock, HomenewslistLock, HomesiteviewcountLock, HomesitewhoscountLock FROM demo_setting_fr WHERE userid = %s", GetSQLValueString($coluserid_RecordSettingLock, "text"));
  ...
  <div class="panel-body">
    <div class="table-responsive">
        <table class="table table-hover m-b-0">
			...
			<?php do { ?>
            <?php if (
		  ($row_RecordTmpAddHomeColumn['type'] == 'sitewhoscount' && ($row_RecordSettingLock['HomesitewhoscountLock'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'siteviewcount' && ($row_RecordSettingLock['HomesiteviewcountLock'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'frilink' && ($row_RecordSettingLock['HomefrilinkLock'] == '1' OR $OptionFrilinkSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'modlink' && ($row_RecordSettingLock['HomemodlinkLock'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'productlist' && ($row_RecordSettingLock['HomeproductlistLock'] == '1' OR $OptionProductSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'articlelist' && ($row_RecordSettingLock['HomearticlelistLock'] == '1' OR $OptionArticleSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'fbfan' && ($row_RecordSettingLock['HomefbfanLock'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'alllist' && ($row_RecordSettingLock['HomealllistLock'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'newslist' && ($row_RecordSettingLock['HomenewslistLock'] == '1' OR $OptionNewsSelect == '0'))
		  ) { ?>
          <?php } else { ?>

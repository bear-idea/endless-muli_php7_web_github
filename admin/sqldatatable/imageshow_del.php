...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "")) {
  ...
  if (isset($_GET['pic'])) {
	$colname_RecordImageshowGet = $_GET['pic'];
  }
  ...
  if($totalRows_RecordImageshowGet == 1 ){ /* 如果資料庫只有一筆 就刪除 若此圖有多張 */
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/imageshow/' . $_GET['pic']);
	  //@unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/imageshow/thumb/small_' . GetFileThumbExtend($_GET['pic']));
  }
  
  $deleteSQL = sprintf("DELETE FROM demo_imageshow WHERE userid=%s && id=%s",

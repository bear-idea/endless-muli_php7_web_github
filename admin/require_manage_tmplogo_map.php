...
$coluserid_RecordTmpLogoSelect = "-1";
if (isset($w_userid)) {
  $coluserid_RecordTmpLogoSelect = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpLogoSelect = sprintf("SELECT TmpLogoID FROM demo_setting_fr WHERE userid=%s", GetSQLValueString($coluserid_RecordTmpLogoSelect, "int"));
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> Logo <small>導覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
      ...
      <div class="col-md-6">
        ...
        <div class="card bg-aqua-transparent-1">
          ...
          <?php if ($row_RecordTmpShowSlect['userid'] == '1') { // 目前使用為官方版型?>
          	<?php if($row_RecordTmpLogoSelect['TmpLogoID'] != "") {?>
            	<div class="alert alert-info fade show"><i class="fa fa-check-circle"></i> 目前已設定Logo</div>
            <?php }else{ ?>
            	<div class="alert alert-danger fade show"><i class="fa fa-info-circle"></i> 目前尚未設定Logo</div>
            <?php }?>
          <?php } else { // 目前使用為個人版型?>
			    <?php if($row_RecordTmpShowSlect['tmplogoid'] != "") {?>
                  <div class="alert alert-info fade show"><i class="fa fa-check-circle"></i> 目前已設定Logo</div>
				<?php }else{?>
                  <div class="alert alert-danger fade show"><i class="fa fa-info-circle"></i> 目前尚未設定Logo</div>
                <?php } ?>
		  <?php } ?>
            ...
            <?php /* 動作 */ ?>
            <?php if ($row_RecordTmpShowSlect['userid'] == '1') { ?>
            <a href="logo_home.php?wshop=<?php echo $wshop;?>&amp;lang=<?php echo $_SESSION['lang']; ?>" data-original-title="前往設定公版Logo(設定後須重新整理此頁面)。" data-toggle="tooltip" data-placement="top" class="btn btn-primary m-t-10 colorbox_iframe_cd" id="Step_Change">Logo設定 <i class="fa fa-chevron-circle-right"></i></a>
			<?php } else { ?>
            <a href="tmp_config_wrp_header_logo.php?id_edit=<?php echo $row_RecordTmpShowSlect['id']; ?>" data-original-title="前往設定此版型Logo(設定後須重新整理此頁面)。" data-toggle="tooltip" data-placement="top" class="btn btn-primary m-t-10 colorbox_iframe_cd" id="Step_Change">Logo設定 <i class="fa fa-chevron-circle-right"></i></a>
            <?php }?>
            <?php /* 動作 */ ?>
      ...
      <div class="col-md-6">
        ...
        <div class="card bg-aqua-transparent-1">
          ...
          <?php if ($row_RecordTmpShowSlectRwd['userid'] == '1') { // 目前使用為官方版型?>
            ...
            <?php }?>
          <?php } else { // 目前使用為個人版型?>
			    <?php if($row_RecordTmpShowSlectRwd['tmplogoid'] != "") {?>
                  <div class="alert alert-info fade show"><i class="fa fa-check-circle"></i> 目前已設定Logo</div>
				<?php }else{?>
                  <div class="alert alert-danger fade show"><i class="fa fa-info-circle"></i> 目前尚未設定Logo</div>
                <?php } ?>
		  <?php } ?>
            ...
            <?php /* 動作 */ ?>
            <?php if ($row_RecordTmpShowSlectRwd['userid'] == '1') { ?>
            <a href="logo_home.php?wshop=<?php echo $wshop;?>&amp;lang=<?php echo $_SESSION['lang']; ?>" data-original-title="前往設定公版Logo(設定後須重新整理此頁面)。" data-toggle="tooltip" data-placement="top" class="btn btn-primary m-t-10 colorbox_iframe_cd" id="Step_Change">Logo設定 <i class="fa fa-chevron-circle-right"></i></a>
			<?php } else { ?>
            <a href="tmp_config_wrp_header_logo.php?id_edit=<?php echo $row_RecordTmpShowSlectRwd['id']; ?>" data-original-title="前往設定此版型Logo(設定後須重新整理此頁面)。" data-toggle="tooltip" data-placement="top" class="btn btn-primary m-t-10 colorbox_iframe_cd" id="Step_Change">Logo設定 <i class="fa fa-chevron-circle-right"></i></a>
            <?php }?>
            <?php /* 動作 */ ?>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_Tip',
                intro: '依照以下的步驟操作，您可替換目前的網站Logo。'
              },
			  {
                element: '#Step_View1',
                intro: '此區域為目前網站所使用版型的Logo。'
              },
			  {
                element: '#Step_Add',
                intro: '若您欲上傳您的Logo，點此建立。'
              },
			  {
                element: '#Step_View',
                intro: '此處可查看目前的所有Logo。'
              },
			  <?php if ($row_RecordTmpShowSlect['userid'] == '1') { // 目前使用為官方版型?>
			  {
                element: '#Step_Change',

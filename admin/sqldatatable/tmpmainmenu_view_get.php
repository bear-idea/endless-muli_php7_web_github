...
$colname_RecordTmpMainMenu = "%";
if (isset($search_name) && $search_name != "") {
  $colname_RecordTmpMainMenu = $search_name;
}
...
$query_RecordCount = sprintf("SELECT count(id) as sum FROM demo_tmpmainmenu WHERE (userid=%s || userid=1)",GetSQLValueString($coluserid_RecordTmpMainMenu, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);


$query_RecordTmpMainMenu = sprintf("SELECT * FROM demo_tmpmainmenu WHERE (name LIKE %s || id LIKE %s) && indicate LIKE %s && type LIKE %s && (userid=%s || userid=1) $orderSql", GetSQLValueString("%" . $colsearch_RecordTmpMainMenu . "%", "text"), GetSQLValueString("%" . $colsearch_RecordTmpMainMenu . "%", "text"), GetSQLValueString($colindicate_RecordTmpMainMenu, "text"), GetSQLValueString($coltype_RecordTmpMainMenu, "text"),GetSQLValueString($coluserid_RecordTmpMainMenu, "int"));
...
<?php do { ?>
  <?php 
    ...
    //$link_view = "../tmpmainmenu_demo_". $row_RecordTmpMainMenu['name'].".php?tmpmainmenuid=".$row_RecordTmpMainMenu['id'];
	...
	$css_menu = "<ul id='navcss3' style='background-image:url(".$link_topmainmenu_o."); width:100%; background-position:bottom'>";
	...
	$dvalue['pic'] = "<span class='label label-purple' data-original-title='版型編號' data-toggle='tooltip' data-placement='right'>#No.".$row_RecordTmpMainMenu['id']."</span>"."<div class='img-thumbnail m-b-5 m-t-5 overflow-x-hidden overflow-y-hidden'><div class='' style='width:500px;height:100px;'>".$css_menu."</div></div>";
	 ...
	 $dvalue['name'] = "<div class='hidden-text'>".$row_RecordTmpMainMenu["name"]."</div>";
	...
	if ($row_RecordTmpMainMenu['userid'] == $w_userid) { 
		$dvalue['type'] = "<span>".$row_RecordTmpMainMenu["type"]."</span>";
	}else{
	...
	$dvalue['action'] = "<div class='radio radio-css radio-inline'><input type='radio' name='TmpMainMenuSelect' value='".$row_RecordTmpMainMenu["id"]."' id='TmpMainMenuSelect_".$row_RecordTmpMainMenu["id"]."' data-parsley-trigger='blur' data-parsley-errors-container='#error_action' required=''><label for='TmpMainMenuSelect_".$row_RecordTmpMainMenu["id"]."'>選擇目前主選單</label></div>";

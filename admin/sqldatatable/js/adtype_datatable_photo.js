var handleDataTableDefault = function(act_id) {
    "use strict";
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
            ...
            "autoWidth": false,
            "ajax": {
                "url": "sqldatatable/adtype_photo_view.php?act_id=" + act_id,
                "type": "post",
				"data": function ( d ) {
					...
					//d.search_act_id = $('#search_act_id').val();
					// etc
				},
                "error": function() {
                    alert("伺服器未回應");
            ...
            "fnDrawCallback": function(oSettings) {
                ...
                $('#data-table-default').editable({
                    ...
                    url: 'sqledit/adtypephoto_jedit.php',
                    type: 'text',
                    name: 'sdescription',
                    //title: '輸入標題',
				...
				$('#data-table-default').editable({
                    selector: ".ed_animation",
                    url: 'sqledit/adtypephoto_jedit.php',
                    name: "animation",
                    source: "sqledit/adtypephoto_get_list.php?list_id=1",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
				...
				$('#data-table-default').editable({
                    selector: ".ed_datatransition",
                    url: 'sqledit/adtypephoto_jedit.php',
                    name: "datatransition",
                    source: "sqledit/adtypephoto_get_list.php?list_id=2",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
...
var TableManageDefault = function(act_id) {
    "use strict";
    return {
        //main function
        init: function(act_id) {
            handleDataTableDefault(act_id);
        }
    };
} ();
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/adtype_photo_del.php?id_del=" + id + "&pic=" + pic,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/adtype_photo_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_ProductItemEdit")) {
	foreach($_POST['item_id'] as $key => $val){
	  $updateSQL = sprintf("UPDATE demo_productitem SET list_id=%s, sortid=%s, indicate=%s, itemname=%s, itemvalue=%s, lang=%s WHERE item_id=%s",
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordProductListItem = sprintf("SELECT demo_productitem.item_id, demo_productitem.userid, demo_productlist.list_id, demo_productlist.listname, demo_productitem.itemname, demo_productitem.itemvalue, demo_productitem.sortid, demo_productitem.indicate, demo_productitem.lang FROM demo_productlist LEFT OUTER JOIN demo_productitem ON demo_productlist.list_id = demo_productitem.list_id WHERE demo_productlist.list_id = %s && demo_productitem.lang=%s && demo_productitem.level=0 && demo_productitem.userid=%s ORDER BY demo_productitem.sortid ASC, demo_productitem.item_id DESC", GetSQLValueString($collistid_RecordProductListItem, "int"),GetSQLValueString($collang_RecordProductListItem, "text"),GetSQLValueString($coluserid_RecordProductListItem, "int"));
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0" class="TB_General_style01">
                  <tr>
                    <td width="100" align="right">編輯欄位：</td>
                  	<td width="200"><strong>分類</strong></td>
                  	<td width="200"><strong>值(更動不能)</strong></td>
                ...
                <tr>
                  ...
                  <td width="150"><label for="itemvalue[]"></label>
                  <input name="itemvalue[]" type="text" id="itemvalue[]" value="<?php echo $row_RecordProductListItem['itemvalue']; ?>" maxlength="10" /></td>

...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/salaryform_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 薪資列表 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
    ...
    <div class="row justify-content-end">
      <div class="col-md-2 col-sm-12 m-b-10">
        <div class="input-group" data-column="2"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 部門</span></span>
            <select name="department" class="form-control search_filter" id="col3_filter">
            <option value="" selected="selected">全部</option>
              <?php
do {  
?>
                  <option value="<?php echo $row_RecordSalaryformListDepartment['itemname']?>"><?php echo $row_RecordSalaryformListDepartment['itemname']?></option>
                  <?php
} while ($row_RecordSalaryformListDepartment = mysqli_fetch_assoc($RecordSalaryformListDepartment));
  $rows = mysqli_num_rows($RecordSalaryformListDepartment);
  if($rows > 0) {
      mysqli_data_seek($RecordSalaryformListDepartment, 0);
	  $row_RecordSalaryformListDepartment = mysqli_fetch_assoc($RecordSalaryformListDepartment);
  }
?>
          </select>
        </div>
      ...
      <div class="col-md-2 col-sm-12 m-b-10">
        <div class="input-group" data-column="4"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 職稱</span></span>
            <select name="job" class="form-control search_filter" id="col4_filter">
            <option value="" selected="selected">全部</option>
              <?php
do {  
?>
...
?>
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          <th width="16"></th>
          <th width="100" data-priority="1"><strong>員工編號</strong></th>
          <th width="-1" data-priority="1"><strong>姓名</strong></th>        
          <th width="100" data-priority="1"><strong>部門</strong></th>
          <th width="100" data-priority="1"><strong>職稱</strong></th>
          <th width="-1" class="none"></th>
          <th width="70"><strong>狀態</strong></th>
          <th width="100"><strong>到職日期</strong></th>
        </tr>
      </thead>
      <tfoot>
        <tr>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_salaryform.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_salaryform.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',

...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "") && $_GET['userid'] == $w_userid) {
  ...
  if($_GET['type'] == 'productlist' || $_GET['type'] == 'frilink' || $_GET['type'] == 'modlink' || $_GET['type'] == 'articlelist' || $_GET['type'] == 'fbfan' || $_GET['type'] == 'alllist' || $_GET['type'] == 'newslist' || $_GET['type'] == 'blogplist' || $_GET['type'] == 'blogcalendar' || $_GET['type'] == 'siteviewcount' || $_GET['type'] == 'sitewhoscount' || $_GET['type'] == 'alltypelist')
  {
	  ...
	  $updateSQLLock = sprintf("UPDATE demo_setting_fr SET %s=0 WHERE userid=%s",
	                       GetSQLValueString("Home" . $_GET['type'] . "Lock_kr", "none"),
  ...
  $deleteGoTo = "tmp_config_" . $_GET['board'] . "_column_kr.php?lang=" . $_SESSION['lang'] . "&board=" . $_GET['board'] . "&id_edit=" . $_GET['id_edit'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "TmpColumnFree")) {
  ...
  if($_POST['type'] == 'productlist' || $_POST['type'] == 'frilink' || $_POST['type'] == 'modlink' || $_POST['type'] == 'articlelist' || $_POST['type'] == 'fbfan' || $_POST['type'] == 'alllist' || $_POST['type'] == 'newslist' || $_POST['type'] == 'blogplist' || $_POST['type'] == 'blogcalendar' || $_POST['type'] == 'siteviewcount' || $_POST['type'] == 'sitewhoscount' || $_POST['type'] == 'alltypelist')
  {
	  ...
	  $updateSQLLock = sprintf("UPDATE demo_setting_fr SET %s=1 WHERE userid=%s",
	  					   GetSQLValueString("Home" . $_POST['type'] . "Lock_kr", "none"),
						   GetSQLValueString($_POST['userid'], "int"));
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSettingLock = sprintf("SELECT HomeproductlistLock_kr, HomefrilinkLock_kr, HomemodlinkLock_kr, HomearticlelistLock_kr, HomefbfanLock_kr, HomealllistLock_kr, HomenewslistLock_kr, HomesiteviewcountLock_kr, HomesitewhoscountLock_kr FROM demo_setting_fr WHERE userid = %s", GetSQLValueString($coluserid_RecordSettingLock, "text"));
  ...
  <div class="panel-body">
    <div class="table-responsive">
        <table class="table table-hover m-b-0">
			...
			<?php do { ?>
            <?php if (
		  ($row_RecordTmpAddHomeColumn['type'] == 'sitewhoscount' && ($row_RecordSettingLock['HomesitewhoscountLock_kr'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'siteviewcount' && ($row_RecordSettingLock['HomesiteviewcountLock_kr'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'frilink' && ($row_RecordSettingLock['HomefrilinkLock_kr'] == '1' OR $OptionFrilinkSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'modlink' && ($row_RecordSettingLock['HomemodlinkLock_kr'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'productlist' && ($row_RecordSettingLock['HomeproductlistLock_kr'] == '1' OR $OptionProductSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'articlelist' && ($row_RecordSettingLock['HomearticlelistLock_kr'] == '1' OR $OptionArticleSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'fbfan' && ($row_RecordSettingLock['HomefbfanLock_kr'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'alllist' && ($row_RecordSettingLock['HomealllistLock_kr'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'newslist' && ($row_RecordSettingLock['HomenewslistLock_kr'] == '1' OR $OptionNewsSelect == '0'))
		  ) { ?>
          <?php } else { ?>
          <form name="TmpColumn" action="<?php echo $editFormAction; ?>" method="POST" id="TmpColumn<?php echo $row_RecordTmpAddHomeColumn['type']; ?>">
             ...
             <input name="dftname_kr" type="hidden" id="dftname_kr" value="<?php echo $row_RecordTmpAddHomeColumn['dftname_kr']; ?>" />

...
$search_manufacturer = $_POST['search_manufacturer'];
...
if(isset($order_column)){
    ...
    switch($i){
        ...
        case 5;$orderSql = " ORDER BY tb_o.snumber ".$order_dir;break;
		...
		//case 10;$orderSql = " ORDER BY tb_o.carnumber ".$order_dir;break;
		case 11;$orderSql = " ORDER BY tb_o.carnumber ".$order_dir;break;
		case 12;$orderSql = " ORDER BY tb_o.postdate ".$order_dir;break;
        default;$orderSql = '';
    }
}
if($orderSql == "") {
	$orderSql = $orderSql . "ORDER BY tb_o.id DESC";
}else{
	$orderSql = $orderSql . ",tb_o.id DESC";
}
...
$query_RecordScaleorder_clearance = sprintf("SELECT tb_o.title, erp_scalepricelist.startdate, erp_scalepricelist.enddate, erp_scalepricelist.price, erp_scalepricelist.mode, erp_scalepricelist.name, tb_o.code, tb_o.type, tb_o.bigtype, tb_o.Totalweight, tb_o.Minweight, tb_o.userid, tb_o.carnumber, tb_o.snumber, tb_o.oserial, tb_o.wastecode, tb_o.postdate, tb_o.id, tb_o.lang FROM (SELECT * FROM erp_scaleorderclearancedetail WHERE lang = %s && userid=%s) AS tb_o LEFT OUTER JOIN erp_scalepricelist ON tb_o.code = erp_scalepricelist.code && tb_o.postdate BETWEEN erp_scalepricelist.startdate AND erp_scalepricelist.enddate + INTERVAL 1 DAY HAVING tb_o.postdate BETWEEN %s AND %s && (tb_o.title LIKE binary %s) && (tb_o.carnumber LIKE binary %s) $orderSql", GetSQLValueString($collang_RecordScaleorder_clearance, "text"),GetSQLValueString($coluserid_RecordScaleorder_clearance, "int"),GetSQLValueString($colstartdate_RecordScaleorder_clearance, "date"),GetSQLValueString($colenddate_RecordScaleorder_clearance, "date"),GetSQLValueString('%'.$colsearch_RecordScaleorder_clearance.'%', "text"),GetSQLValueString($colcarnumber_RecordScaleorder_clearance, "text"));
...
<?php do { ?>
  <?php 
	...
	$dvalue['Totalweight'] = $row_RecordScaleorder_clearance["Totalweight"];
	
	$dvalue['Minweight'] = $row_RecordScaleorder_clearance["Minweight"];
	
	$dvalue['Oriweight'] = $row_RecordScaleorder_clearance["Totalweight"] - $row_RecordScaleorder_clearance["Minweight"];
	...
	}elseif($row_RecordScaleorder_clearance['mode'] == '1'){
		$dvalue['allprice'] = $row_RecordScaleorder_clearance["price"];
	}else{
		$dvalue['allprice'] = ($row_RecordScaleorder_clearance["Totalweight"] - $row_RecordScaleorder_clearance["Minweight"])*$row_RecordScaleorder_clearance["price"];
	}
	...
	$dvalue['postdate'] = "<span id='postdate_".$row_RecordScaleorder_clearance["id"]."' class='ed_postdate' data-type='date' data-format='yyyy-mm-dd' data-pk='".$row_RecordScaleorder_clearance["id"]."' data-placement='top'>".$dt->format('Y-m-d')."</span>";

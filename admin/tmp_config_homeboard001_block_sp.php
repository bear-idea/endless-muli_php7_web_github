...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "") && $_GET['userid'] == $w_userid) {
  ...
  $deleteGoTo = "tmp_config_" . $_GET['board'] . "_block_sp.php?lang=" . $_SESSION['lang'] . "&board=" . $_GET['board'] . "&id_edit=" . $_GET['id_edit'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSettingLock = sprintf("SELECT HomeNewsLock_sp, HomePublishLock_sp, HomeLettersLock_sp, HomeActnewsLock_sp, HomePartnerLock_sp, HomeVideoLock_sp, HomeProductLock_sp, HomeProjectLock_sp, HomeActivitiesLock_sp, HomeSponsorLock_sp FROM demo_setting_fr WHERE userid = %s", GetSQLValueString($coluserid_RecordSettingLock, "text"));
  ...
  <div class="panel-body">
    <div class="table-responsive">
        <table class="table table-hover m-b-0">
			...
			<?php do { ?>
            <?php if (
		  ($row_RecordTmpAddColumn['type'] == 'News' && ($row_RecordSettingLock['HomeNewsLock_sp'] == '1' OR $OptionNewsSelect == '0')) || 
		  ($row_RecordTmpAddColumn['type'] == 'Publish' && ($row_RecordSettingLock['HomePublishLock_sp'] == '1' OR $OptionPublishSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Letters' && ($row_RecordSettingLock['HomeLettersLock_sp'] == '1' OR $OptionLettersSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Actnews' && ($row_RecordSettingLock['HomeActnewsLock_sp'] == '1' OR $OptionActnewsSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Partner' && ($row_RecordSettingLock['HomePartnerLock_sp'] == '1' OR $OptionPartnerSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Video' && ($row_RecordSettingLock['HomeVideoLock_sp'] == '1' OR $OptionVideoSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Product' && ($row_RecordSettingLock['HomeProductLock_sp'] == '1' OR $OptionProductSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Project' && ($row_RecordSettingLock['HomeProjectLock_sp'] == '1' OR $OptionProjectSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Activities' && ($row_RecordSettingLock['HomeActivitiesLock_sp'] == '1' OR $OptionActivitiesSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Sponsor' && ($row_RecordSettingLock['HomeSponsorLock_sp'] == '1' OR $OptionSponsorSelect == '0'))
		  ) { ?>
          <?php } else { ?>

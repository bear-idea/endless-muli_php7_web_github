...
$colname_RecordDfPage = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordDfPage = $_GET['id_edit'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordDfPage = sprintf("SELECT * FROM demo_dfpage WHERE id = %s && userid=%s", GetSQLValueString($colname_RecordDfPage, "int"),GetSQLValueString($coluserid_RecordDfPage, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_DfPage") && $_POST['sdescription'] == "") {
	$_POST['sdescription'] = TrimSummary($_POST['content']);
}
/* 當類別無傳值進來時則給定初始值 */
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_DfPage")) {
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_DfPage")) {
  $updateSQL = sprintf("UPDATE demo_dfpage SET title=%s, type1=%s, type2=%s, type3=%s, content=%s, indicate=%s, skeyword=%s, sdescription=%s, notes1=%s, lang=%s WHERE id=%s",
  ...
  $updateGoTo = "manage_dfpage.php?Opt=viewpage&lang=" . $_POST['lang'] . '&aid=' . $_POST['aid'] . '&tpt=' . $_POST['tpt'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
<div class="card bg-silver-lighter mb-10px" style="overflow:hidden">
  <div class="card-body">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 文章資料 <small>修改</small> <?php require($page_view_path_vendor."require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9">
  ...
  <form action="<?php echo $request->getRequestUri(); ?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      <div class="form-group row">
          ...
          <div class="col-md-10">
                      <input name="title" type="text" required="" class="form-control" id="title" value="<?php echo $row_RecordDfPage['title']; ?>" maxlength="200" data-parsley-trigger="blur"/>
      ...
      <div class="form-group row">
          ...
          <div class="row p-10">
                    ...
                    <select name="type1" id="type1" class="form-control col-md-4" style="display:inline-block" data-parsley-trigger="blur" >
                      <option value="-1" <?php if (!(strcmp(-1, $row_RecordDfPage['type1']))) {echo "selected=\"selected\"";} ?>>-- 選擇分類 --</option>
                      <?php
do {
?>
                      <option value="<?php echo $row_RecordDfPageListType['item_id']?>"<?php if (!(strcmp($row_RecordDfPageListType['item_id'], $row_RecordDfPage['type1']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecordDfPageListType['itemname']?></option>
                      <?php
} while ($row_RecordDfPageListType = mysqli_fetch_assoc($RecordDfPageListType));
  $rows = mysqli_num_rows($RecordDfPageListType);
  if($rows > 0) {
      mysqli_data_seek($RecordDfPageListType, 0);
	  $row_RecordDfPageListType = mysqli_fetch_assoc($RecordDfPageListType);
  }
?>
                    </select>
                    ...
                    <select name="type2" id="type2" class="form-control col-md-4" style="display:inline-block">
                      <option value="-1" <?php if (!(strcmp(-1, $row_RecordDfPageListType['item_id']))) {echo "selected=\"selected\"";} ?>>-- 選擇分類2 --</option>
                      <?php
do {
?>
                      <option value="<?php echo $row_RecordDfPageListType['item_id']?>"<?php if (!(strcmp($row_RecordDfPageListType['item_id'], $row_RecordDfPageListType['item_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecordDfPageListType['itemname']?></option>
<?php
} while ($row_RecordDfPageListType = mysqli_fetch_assoc($RecordDfPageListType));
  $rows = mysqli_num_rows($RecordDfPageListType);
  if($rows > 0) {
...
?>
                    ...
                    <select name="type3" id="type3" class="form-control col-md-4" style="display:inline-block">
                      <option value="-1" <?php if (!(strcmp(-1, $row_RecordDfPageListType['item_id']))) {echo "selected=\"selected\"";} ?>>-- 選擇分類3 --</option>
                      <?php
do {
?>
                      <option value="<?php echo $row_RecordDfPageListType['item_id']?>"<?php if (!(strcmp($row_RecordDfPageListType['item_id'], $row_RecordDfPageListType['item_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecordDfPageListType['itemname']?></option>
...
?>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <div class="form-check form-check-inline">
                <input <?php if (!(strcmp($row_RecordDfPage['indicate'],"1"))) {echo "checked=\"checked\"";} ?> class="form-check-input"  type="radio" name="indicate" id="indicate_1" value="1" checked />
                <label for="indicate_1">公佈</label>
            </div>
            <div class="form-check form-check-inline">
                <input <?php if (!(strcmp($row_RecordDfPage['indicate'],"0"))) {echo "checked=\"checked\"";} ?> class="form-check-input"  type="radio" name="indicate" id="indicate_2" value="0" />
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
               <input name="skeyword" type="text" class="form-control" id="skeyword" value="<?php echo $row_RecordDfPage['skeyword']; ?>" maxlength="300" data-role="tagsinput" />
          </div>
      </div>
      <div class="form-group row">
          ...
          <div class="col-md-10">
                 <input name="sdescription" type="text" class="form-control" id="sdescription" value="<?php echo $row_RecordDfPage['sdescription']; ?>" size="100" maxlength="150"/>
          </div>
      </div>
      <div class="form-group row">
          ...
          <div class="table-responsive">
                <textarea name="content" id="content" cols="100%" rows="35" class="form-control"><?php echo $row_RecordDfPage['content']; ?>
                </textarea>
          </div>
          </div>
      </div>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <input name="notes1" type="text" class="form-control" id="notes1" value="<?php echo $row_RecordDfPage['notes1']; ?>" size="50" maxlength="50"/>
          </div>
      </div>
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="lang" type="hidden" id="lang" value="<?php echo $_GET['lang']; ?>" />
                    <input name="id" type="hidden" id="id" value="<?php echo $row_RecordDfPage['id']; ?>" />
                    <input id="fullIdPath" type="hidden" value="<?php echo $row_RecordDfPage['type1']; ?>,<?php echo $row_RecordDfPage['type2']; ?>,<?php echo $row_RecordDfPage['type3']; ?>" />
                    <input name="aid" type="hidden" id="aid" value="<?php echo $row_RecordDfPage['aid']; ?>" />
                    <input name="tpt" type="hidden" id="tpt" value="<?php echo $row_RecordTpt['title']; ?>" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_DfPage" />
  </form>
  </div>
  <!-- end panel-body -->
</div>
...
$(function () {
    ...
    $('#type1').change(function () {
        ...
        $('#type2').removeOption(/.?/).ajaxAddOption(
            'selectbox_action/dfpage_add.php?&<?php echo time();?>',
            { 'id': $(this).val(), 'lv': 1 },

...
$query_RecordCount = sprintf("SELECT count(id) as sum FROM demo_tmpbackground WHERE (userid=%s || userid=1) && localmiddle=1",GetSQLValueString($coluserid_RecordTmpBackGround, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);


$query_RecordTmpBackGround = sprintf("SELECT * FROM demo_tmpbackground WHERE (name LIKE %s || id LIKE %s) && indicate LIKE %s && type LIKE %s && (userid=%s || userid=1) && localmiddle=1 $orderSql", GetSQLValueString("%" . $colsearch_RecordTmpBackGround . "%", "text"), GetSQLValueString("%" . $colsearch_RecordTmpBackGround . "%", "text"), GetSQLValueString($colindicate_RecordTmpBackGround, "text"), GetSQLValueString($coltype_RecordTmpBackGround, "text"),GetSQLValueString($coluserid_RecordTmpBackGround, "int"));

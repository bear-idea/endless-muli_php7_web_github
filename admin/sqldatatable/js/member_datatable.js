    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
			...
			"stateLoadParams": function (settings, data) {
				...
				$("#col4_filter").val(data.search.field.auth);
            ...
            "fnDrawCallback": function(oSettings) {
				...
				$('#data-table-default').editable({
                    ...
                    name: 'mail',
                    //title: '輸入標題',
                    tpl: "<input type='mail' style='width: 100%' maxlength='200'>",
                    select2: {
                        width: '100%',
                        // THIS DOESN'T WORK AS IT SHOULD
                        // hiding search box
				...
				$('#data-table-default').editable({
                    ...
                    url: 'sqledit/member_jedit.php',
                    name: "type",
                    source: "sqledit/member_get_list.php?list_id=1",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/member_del.php?id_del=" + id + "&pic=" + pic,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/member_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

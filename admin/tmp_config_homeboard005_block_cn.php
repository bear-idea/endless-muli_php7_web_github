...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "") && $_GET['userid'] == $w_userid) {
  ...
  if($_GET['type'] == 'News' || $_GET['type'] == 'Publish' || $_GET['type'] == 'Letters' || $_GET['type'] == 'Actnews' || $_GET['type'] == 'Partner' || $_GET['type'] == 'Video' || $_GET['type'] == 'Product' || $_GET['type'] == 'Project' || $_GET['type'] == 'Activities' || $_GET['type'] == 'Sponsor')
  {
	  ...
	  $updateSQLLock = sprintf("UPDATE demo_setting_fr SET %s=0 WHERE userid=%s",
	                       GetSQLValueString("Home" . $_GET['type'] . "Lock_cn", "none"),
  ...
  $deleteGoTo = "tmp_config_" . $_GET['board'] . "_block_cn.php?lang=" . $_SESSION['lang'] . "&board=" . $_GET['board'] . "&id_edit=" . $_GET['id_edit'];
  //if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "TmpColumnFree")) {
  ...
  if($_POST['type'] == 'News' || $_POST['type'] == 'Publish' || $_POST['type'] == 'Letters' || $_POST['type'] == 'Actnews' || $_POST['type'] == 'Partner' || $_POST['type'] == 'Video' || $_POST['type'] == 'Product' || $_POST['type'] == 'Project' || $_POST['type'] == 'Activities' || $_POST['type'] == 'Sponsor')
  {
	  ...
	  $updateSQLLock = sprintf("UPDATE demo_setting_fr SET %s=1 WHERE userid=%s",
	  					   GetSQLValueString("Home" . $_POST['type'] . "Lock_cn", "none"),
						   GetSQLValueString($_POST['userid'], "int"));
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSettingLock = sprintf("SELECT HomeNewsLock_cn, HomePublishLock_cn, HomeLettersLock_cn, HomeActnewsLock_cn, HomePartnerLock_cn, HomeVideoLock_cn, HomeProductLock_cn, HomeProjectLock_cn, HomeActivitiesLock_cn, HomeSponsorLock_cn FROM demo_setting_fr WHERE userid = %s", GetSQLValueString($coluserid_RecordSettingLock, "text"));
...
<div>
  ...
  <td width="100" align="center"><?php echo $row_RecordTmpColumn['type']; ?></td>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-left:10px; margin-right:10px;border:solid 1px #CCCCCC;" class="TB_General_style01">
		  ...
		  <?php do { ?>
          <?php if (
		  ($row_RecordTmpAddColumn['type'] == 'News' && ($row_RecordSettingLock['HomeNewsLock_cn'] == '1' OR $OptionNewsSelect == '0')) || 
		  ($row_RecordTmpAddColumn['type'] == 'Publish' && ($row_RecordSettingLock['HomePublishLock_cn'] == '1' OR $OptionPublishSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Letters' && ($row_RecordSettingLock['HomeLettersLock_cn'] == '1' OR $OptionLettersSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Actnews' && ($row_RecordSettingLock['HomeActnewsLock_cn'] == '1' OR $OptionActnewsSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Partner' && ($row_RecordSettingLock['HomePartnerLock_cn'] == '1' OR $OptionPartnerSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Video' && ($row_RecordSettingLock['HomeVideoLock_cn'] == '1' OR $OptionVideoSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Product' && ($row_RecordSettingLock['HomeProductLock_cn'] == '1' OR $OptionProductSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Project' && ($row_RecordSettingLock['HomeProjectLock_cn'] == '1' OR $OptionProjectSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Activities' && ($row_RecordSettingLock['HomeActivitiesLock_cn'] == '1' OR $OptionActivitiesSelect == '0')) ||
		  ($row_RecordTmpAddColumn['type'] == 'Sponsor' && ($row_RecordSettingLock['HomeSponsorLock_cn'] == '1' OR $OptionSponsorSelect == '0'))
		  ) { ?>
          <?php } else { ?>

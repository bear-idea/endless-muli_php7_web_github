...
if (isset($search_name) && $search_name != "") {
  $colname_RecordTmpLogo = $search_name;
}
...
$query_RecordCount = sprintf("SELECT count(id) as sum FROM demo_tmplogo WHERE (userid=%s || userid=1)",GetSQLValueString($coluserid_RecordTmpLogo, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);


$query_RecordTmpLogo = sprintf("SELECT * FROM demo_tmplogo WHERE (name LIKE %s || id LIKE %s) && indicate LIKE %s && type LIKE %s && (userid=%s || userid=1) $orderSql", GetSQLValueString("%" . $colsearch_RecordTmpLogo . "%", "text"), GetSQLValueString("%" . $colsearch_RecordTmpLogo . "%", "text"), GetSQLValueString($colindicate_RecordTmpLogo, "text"), GetSQLValueString($coltype_RecordTmpLogo, "text"),GetSQLValueString($coluserid_RecordTmpLogo, "int"));
...
<?php do { ?>
  <?php 
    ...
    //$link_view = "../tmplogo_demo_". $row_RecordTmpLogo['name'].".php?tmplogoid=".$row_RecordTmpLogo['id'];
	...
	if ($row_RecordTmpLogo['logotype'] == 0) { /* 使用圖片 */
		$dvalue['pic'] = "<span class='label label-purple' data-original-title='版型編號' data-toggle='tooltip' data-placement='right'>#No.".$row_RecordTmpLogo['id']."</span>"."<div class='img-thumbnail m-b-5 m-t-5 overflow-x-hidden overflow-y-hidden'><div class='imgLiquidFill' style='width:100%;height:100px;'><img src=".$link_pic." class='img-fluid'></div></div></div>";	
	}else{
		$dvalue['pic'] = "<span class='label label-purple' data-original-title='版型編號' data-toggle='tooltip' data-placement='right'>#No.".$row_RecordTmpLogo['id']."</span>"."<div class='img-thumbnail m-b-5 m-t-5 overflow-x-hidden overflow-y-hidden'><div style='overflow:hidden; width:100%; height:100px; line-height:120px; color:".$row_RecordTmpLogo['logocolor']."; font-size:".$row_RecordTmpLogo['logofontsize']."'>".$link_pic."</div></div></div>";	
	}
	

	 $dvalue['name'] = "<div class='hidden-text'>".$row_RecordTmpLogo["name"]."</div>";
	...
	if ($row_RecordTmpLogo['userid'] == $w_userid) { 
		$dvalue['type'] = "<span>".$row_RecordTmpLogo["type"]."</span>";
	}else{
	...
	$dvalue['action'] = "<div class='radio radio-css radio-inline'><input type='radio' name='TmpLogoSelect' value='".$row_RecordTmpLogo["id"]."' id='TmpLogoSelect_".$row_RecordTmpLogo["id"]."' data-parsley-trigger='blur' data-parsley-errors-container='#error_action' ><label for='TmpLogoSelect_".$row_RecordTmpLogo["id"]."'>選擇目前Logo</label></div>";

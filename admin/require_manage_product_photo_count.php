...
if($row_RecordProductListType['item_id'] != "")
{
...
$collist_id_RecordProductListTypeCount = "-1";
if (isset($_GET['list_id'])) {
  $collist_id_RecordProductListTypeCount = $_GET['list_id'];
}
...
$query_RecordProductListTypeCount = sprintf("SELECT * FROM demo_productitem WHERE list_id = %s && lang=%s && subitem_id = %s && userid=%s ORDER BY sortid ASC", GetSQLValueString($collist_id_RecordProductListTypeCount, "int"), GetSQLValueString($colname_RecordProductListTypeCount, "text"),GetSQLValueString($colitem_id_RecordProductListTypeCount, "int"),GetSQLValueString($coluserid_RecordProductListTypeCount, "int"));

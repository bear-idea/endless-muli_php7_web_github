...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Absence")) {
  $updateSQL = sprintf("UPDATE salary_absence SET code=%s, title=%s, calculate=%s, coefficient=%s, FixedAmount=%s, postdate=%s, indicate=%s, notes1=%s, lang=%s WHERE id=%s",
  ...
  $updateGoTo = "manage_absence.php?Opt=viewpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
/* 取得最新訊息資料 */
$colname_RecordAbsence = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordAbsence = $_GET['id_edit'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordAbsence = sprintf("SELECT * FROM salary_absence WHERE id = %s && userid=%s", GetSQLValueString($colname_RecordAbsence, "int"),GetSQLValueString($coluserid_RecordAbsence, "int"));
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i>  缺勤扣款 <small>修改</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      <div class="form-group row">
          ...
          <div class="col-md-10">
                      ...
                      <input name="code" type="text" required="" class="form-control" id="code" value="<?php echo $row_RecordAbsence['code']; ?>" maxlength="100" data-parsley-trigger="blur"/>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
                      ...
                      <input name="title" type="text" class="form-control" id="title" value="<?php echo $row_RecordAbsence['title']; ?>" maxlength="200" data-parsley-trigger="blur" required="" />
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordAbsence['calculate'],"0"))) {echo "checked=\"checked\"";} ?> type="radio" name="calculate" id="calculate_0" value="0" />
                <label for="calculate_0">固定所得-(固定所得x扣款係數)</label>
            </div>
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordAbsence['calculate'],"1"))) {echo "checked=\"checked\"";} ?> type="radio" name="calculate" id="calculate_1" value="1" />
                <label for="calculate_1">薪資總額-(薪資總額x扣款係數)</label>
            </div>
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordAbsence['calculate'],"2"))) {echo "checked=\"checked\"";} ?> type="radio" name="calculate" id="calculate_2" value="2" />
      ...
      <div class="form-group row">
          ...
          <div class="input-group p-0">
                   ...
                   <input name="coefficient" type="number" class="form-control col-md-4" id="coefficient" step="1" value="<?php echo $row_RecordAbsence['coefficient']; ?>" data-parsley-min="100" data-parsley-max="9999999" data-parsley-type="number" data-parsley-trigger="blur" >
      ...
      <div class="form-group row">
          ...
          <div class="input-group p-0">
                   ...
                   <input name="FixedAmount" type="number" class="form-control col-md-4" id="FixedAmount" step="1" value="<?php echo $row_RecordAbsence['FixedAmount']; ?>" data-parsley-min="0" data-parsley-max="9999999" data-parsley-type="number" data-parsley-trigger="blur" >
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordAbsence['indicate'],"1"))) {echo "checked=\"checked\"";} ?> type="radio" name="indicate" id="indicate_1" value="1" />
                <label for="indicate_1">公佈</label>
            </div>
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordAbsence['indicate'],"0"))) {echo "checked=\"checked\"";} ?> type="radio" name="indicate" id="indicate_2" value="0" />
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <input name="postdate" type="text" class="form-control" id="postdate" value="<?php $dt = new DateTime($row_RecordAbsence['postdate']); echo $dt->format('Y-m-d'); ?>" maxlength="10" data-provide="datepicker" data-date-format="yyyy-mm-dd" data-date-language="zh-TW" data-parsley-trigger="blur" required="" autocomplete="off"/> 
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <input name="notes1" type="text" class="form-control" id="notes1" value="<?php echo $row_RecordAbsence['notes1']; ?>" size="50" maxlength="50"/>    
          </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-10">
          <button type="submit" class="btn btn btn-primary btn-block">送出</button>
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordAbsence['id']; ?>" />
            <input name="lang" type="hidden" id="lang" value="<?php echo $row_RecordAbsence['lang']; ?>" />
            <input name="editdate" type="hidden" id="editdate" value="<?php echo date("Y-m-d H-i-s"); ?>" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Absence" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>

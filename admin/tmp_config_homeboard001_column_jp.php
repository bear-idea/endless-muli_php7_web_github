...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "") && $_GET['userid'] == $w_userid) {
  ...
  $deleteGoTo = "tmp_config_" . $_GET['board'] . "_column_jp.php?lang=" . $_SESSION['lang'] . "&board=" . $_GET['board'] . "&id_edit=" . $_GET['id_edit'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSettingLock = sprintf("SELECT HomeproductlistLock_jp, HomefrilinkLock_jp, HomemodlinkLock_jp, HomearticlelistLock_jp, HomefbfanLock_jp, HomealllistLock_jp, HomenewslistLock_jp, HomesiteviewcountLock_jp, HomesitewhoscountLock_jp FROM demo_setting_fr WHERE userid = %s", GetSQLValueString($coluserid_RecordSettingLock, "text"));
  ...
  <div class="panel-body">
    <div class="table-responsive">
        <table class="table table-hover m-b-0">
			...
			<?php do { ?>
            <?php if (
		  ($row_RecordTmpAddHomeColumn['type'] == 'sitewhoscount' && ($row_RecordSettingLock['HomesitewhoscountLock_jp'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'siteviewcount' && ($row_RecordSettingLock['HomesiteviewcountLock_jp'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'frilink' && ($row_RecordSettingLock['HomefrilinkLock_jp'] == '1' OR $OptionFrilinkSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'modlink' && ($row_RecordSettingLock['HomemodlinkLock_jp'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'productlist' && ($row_RecordSettingLock['HomeproductlistLock_jp'] == '1' OR $OptionProductSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'articlelist' && ($row_RecordSettingLock['HomearticlelistLock_jp'] == '1' OR $OptionArticleSelect == '0')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'fbfan' && ($row_RecordSettingLock['HomefbfanLock_jp'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'alllist' && ($row_RecordSettingLock['HomealllistLock_jp'] == '1')) || 
		  ($row_RecordTmpAddHomeColumn['type'] == 'newslist' && ($row_RecordSettingLock['HomenewslistLock_jp'] == '1' OR $OptionNewsSelect == '0'))
		  ) { ?>
          <?php } else { ?>
          <form name="TmpColumn" action="<?php echo $editFormAction; ?>" method="POST" id="TmpColumn<?php echo $row_RecordTmpAddHomeColumn['type']; ?>">
             ...
             <input name="dftname_jp" type="hidden" id="dftname_jp" value="<?php echo $row_RecordTmpAddHomeColumn['dftname_jp']; ?>" />

var handleDataTableDefault = function() {
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       
		    "dom":"lBprtip",buttons:[{extend:"copy",className:"btn-sm",title:"車輛管制表"},{extend:"csv",className:"btn-sm",title:"車輛管制表"},{extend:"excel",className:"btn-sm",title:"車輛管制表"},{extend:"pdf",className:"btn-sm",title:"車輛管制表"},{extend:"print",className:"btn-sm",title:"車輛管制表",exportOptions: {
                stripHtml: false,
                format: {
                ...
                }}},{extend:"colvis",className:"btn-sm",title:"車輛管制表"}],
			//"dom": "lprtip",
            "autoWidth": false,
            "ajax": {
                "url": "sqldatatable/scaleorder_out_view_carcontrol.php",
                "type": "post",
				"data": function ( d ) {
					// servermode default value

					d.search_postdate = $('#col0_filter').val();
					d.search_carnumber = $('#col2_filter').val();
			...
			"stateSaveParams": function (settings, data) {
				...
				data.search.field = {
					...
					postdate: $("#col0_filter").val(),
					carnumber: $("#col2_filter").val(),
			...
			"stateLoadParams": function (settings, data) {
				...
				$("#col0_filter").val(data.search.field.postdate);
				
				$("#col2_filter").val(data.search.field.carnumber);
            ...
            "fnDrawCallback": function(oSettings) {
                ...
                $('#data-table-default').editable({
                    selector: ".ed_title",
                    url: 'sqledit/scaleorder_out_jedit_order.php',
                    type: 'text',
                    name: 'title',
                    //title: '輸入標題',
				...
				$('#data-table-default').editable({
                    selector: ".ed_cartransport",
                    url: 'sqledit/scaleorder_out_jedit_order.php',
                    name: "cartransport",
                    source: "sqledit/scaleorder_out_get_list_cartransport.php?list_id=5",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
...
function delete_datatables(id,e) {
    ...
    swal({
        title: '確定删除?',
        text: "刪除物料時會將物料移至入庫,該出貨單若有該物料則會移除掉。",
        type: 'warning',
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/scaleorder_out_del_order.php?id_del=" + id,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/scaleorder_out_del_muti_order.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

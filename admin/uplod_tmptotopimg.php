...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/images";
	$ppu->extensions = "GIF,JPG,JPEG,BMP,PNG";
	$ppu->formName = "form_Totop";
	$ppu->storeType = "file";
	$ppu->sizeLimit = "3000";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Totop")) {
  $updateSQL = sprintf("UPDATE demo_tmp SET tmptotopimg=IFNULL(%s,tmptotopimg) WHERE id=%s",
...
$colname_RecordTotop = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordTotop = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTotop = sprintf("SELECT * FROM demo_tmp WHERE id = %s", GetSQLValueString($colname_RecordTotop, "int"));
$RecordTotop = mysqli_query($DB_Conn, $query_RecordTotop) or die(mysqli_error($DB_Conn));
$row_RecordTotop = mysqli_fetch_assoc($RecordTotop);
$totalRows_RecordTotop = mysqli_num_rows($RecordTotop);
?>
<!DOCTYPE html>
<html lang="zh-TW">
<!--<![endif]-->
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="tmp_config_md_toolbar.php?lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $_GET['id_edit']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordTotop['id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordTotop['tmptotopimg']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Totop" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 3000,  
		<?php if ($row_RecordTotop['tmptotopimg'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/images/<?php echo $row_RecordTotop['tmptotopimg']; ?>"],
		<?php } ?>

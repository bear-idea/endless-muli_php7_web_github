...
if ((isset($_POST["MM_Gen"])) && ($_POST["MM_Gen"] == "form_Yearend_Gen")) {
  ...
  $deleteSQL = sprintf("DELETE FROM salary_yearend WHERE particularyear=%s",
                       GetSQLValueString($_POST['particularyear'], "int"));
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 年終獎金 <small>刪除</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-body">
    ...
    <div class="alert alert-warning m-t-5"><i class="fa fa-info-circle"></i> <b>年終獎金報表刪除時，會刪除該年度所有資料，刪除後可至建立年終報表重新建立。</b></div>
    ...
    <?php //if ($totalRows_RecordYearend == 0) { // Show if recordset not empty ?>
     ...
     <div class="form-group row">
        <label class="col-md-2 col-form-label">選擇刪除年份<span class="text-red">*</span></label>
          <div class="col-md-10">
                 
                    <select name="particularyear" id="particularyear" class="form-control" data-parsley-trigger="blur" required="">
                		<option value="" >-- 選擇刪除年份 --</option>
                        <?php 
						do{
						?>
                		<option value="<?php echo $row_RecordYearend['particularyear']; ?>" ><?php echo $row_RecordYearend['particularyear']; ?></option>
                        <?php 
     ...
     <button type="submit" class="btn btn btn-primary btn-block">刪除報表</button>
     </form>
    <?php //} ?>

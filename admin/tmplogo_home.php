...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE demo_tmp SET tmplogoid=%s WHERE id=%s",
                       GetSQLValueString($_POST['TmpLogoSelect'], "text"),
...
$colname_RecordTmpLogoStyle = "-1";
if (isset($_GET['id'])) {
  $colname_RecordTmpLogoStyle = $_GET['id'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTmpLogoStyle = sprintf("SELECT * FROM demo_tmp WHERE id = %s", GetSQLValueString($colname_RecordTmpLogoStyle, "int"));
$RecordTmpLogoStyle = mysqli_query($DB_Conn, $query_RecordTmpLogoStyle) or die(mysqli_error($DB_Conn));
$row_RecordTmpLogoStyle = mysqli_fetch_assoc($RecordTmpLogoStyle);
$totalRows_RecordTmpLogoStyle = mysqli_num_rows($RecordTmpLogoStyle);

$colid_RecordTmpLogo = "-1";
if (isset($row_RecordTmpLogoStyle['tmplogoid'])) {
  $colid_RecordTmpLogo = $row_RecordTmpLogoStyle['tmplogoid'];
}
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/tmplogo_datatable_get.js"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="tmp_config_<?php echo $row_RecordTmpLogoStyle['name']; // 為版型風格?>.php?lang=<?php echo $_SESSION['lang']; ?>&amp;id_edit=<?php echo $row_RecordTmpLogoStyle['id']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
  ...
  <div class="row">
      <div class="col-md-12">
        <div class="card bg-grey-transparent-1">
          <div class="card-block">
            <?php if(isset($row_RecordTmpLogoStyle['tmplogoid'])) { ?>
            <?php if ($SiteBaseUrlOuter != '' && $row_RecordTmpLogo['userid'] == '1') { ?>
            <div class="pull-left m-r-10" style="width:100%">
              <span class="label label-purple" data-original-title="目前套用背景編號" data-toggle="tooltip" data-placement="right">#No.<?php echo $row_RecordTmpLogo['id']; ?></span>
              <span class="label label-purple"><?php echo $row_RecordTmpLogo['type']; ?></span>
           ...
           <div class="imgLiquidFill" style="width:160px; height:100px;">	
            <a><img src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpLogo['webname']; ?>/logo/<?php echo $row_RecordTmpLogo['logoimage']; ?>"/></a><span></span>
            </div>
            <?php } else if (GetFileExtend($row_RecordTmpLogo['logoimage']) == '.swf'){ ?>
            <div class="div_table-cell">	
            <a><div align='center'><embed type="application/x-shockwave-flash" width="" height="" src="<?php echo $SiteImgUrlOuter; ?><?php echo $row_RecordTmpLogo['webname']; ?>/logo/<?php echo $row_RecordTmpLogo['logoimage']; ?>" play="true" loop="true" quality="high"> </embed></div></a><span></span>
            </div>
            <?php } else { ?>
            <div class="imgLiquidFill" style="width:160px; height:100px;">		
            ...
            <?php } ?>
           <?php } else { ?>
           <div class="imgLiquidFill" style="width:160px; height:100px; display:inline-block; overflow:hidden; white-space:nowrap">	
           ...
           <?php } ?>
            ...
            <div class="pull-left m-r-10" style="width:100%">
              ...
              <?php if ($row_RecordTmpLogo['logotype'] == 0) { ?>
			   <?php if ($row_RecordTmpLogo['logoimage'] != "" && GetFileExtend($row_RecordTmpLogo['logoimage']) != '.swf') { ?>
           <div class="imgLiquidFill" style="width:160px; height:100px;">	
            <a><img src="<?php echo $SiteImgUrlAdmin; ?><?php echo $row_RecordTmpLogo['webname']; ?>/logo/<?php echo $row_RecordTmpLogo['logoimage']; ?>"/></a><span></span>
            </div>
            <?php } else if (GetFileExtend($row_RecordTmpLogo['logoimage']) == '.swf'){ ?>
            <div class="div_table-cell">	
        ...
        <a style="color:<?php echo $row_RecordTmpLogo['logocolor']; ?>; font-size:<?php echo $row_RecordTmpLogo['logofontsize']; ?>"><?php echo $row_RecordTmpLogo['logoname']; ?></a><span></span>
           ...
           <?php } ?>
            ...
            <?php } else { ?>
            <div class="alert alert-danger m-t-5 m-b-0"><i class="fa fa-info-circle"></i> <b>目前尚未套用任何Logo。</b></div>
            <?php } ?>
          </div>
        </div>
      </div>
    ...
    <table id="data-table-default" class="table table-bordered table-hover cards" style="width:100%">
      ...
      <tfoot>
        <tr>
          ...
          <td><span class="label label-danger p-10"><div class='radio radio-css radio-inline'><input type='radio' name='TmpLogoSelect' value='' id='TmpLogoSelect_none' data-parsley-trigger='blur' data-parsley-errors-container='#error_action'><label for='TmpLogoSelect_none' class='text-white'>不使用任何Logo</label></div></span><button type="button" id="reset_table" class="btn btn-default btn-sm pull-right"><i class="fa fa-sync"></i> 清除狀態</button></td>
        </tr>
      </tfoot>
    </table>
...
<script>
	$(document).ready(function() {
	  ...
	  $("#TransparentButtom1").click(function(){
		...
		<?php if ($row_RecordTmpLogo['tmpleftfontcolor'] == "transparent") { ?>
			$("#tmpleftfontcolor").val("transparent");
		<?php } ?>
		...
		<?php if ($row_RecordTmpLogo['tmpmiddlefontcolor'] == "transparent") { ?>
			$("#tmpmiddlefontcolor").val("transparent");
		<?php } ?>
	});
</script>
...
mysqli_free_result($RecordTmpLogoStyle);

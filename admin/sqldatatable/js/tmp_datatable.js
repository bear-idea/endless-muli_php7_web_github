        ...
        var otable = $('#data-table-default').DataTable({       //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
            ...
            "ajax": {
				...
				"data": function ( d ) {
					...
					d.search_title = $('#col1_filter').val();
					d.search_name = $('#col4_filter').val();
					d.search_type = $('#col5_filter').val();
			...
			"stateSaveParams": function (settings, data) {
				...
				data.search.field = {
					...
					type: $("#col5_filter").val(),
					name: $("#col4_filter").val(),
			...
			"stateLoadParams": function (settings, data) {
				...
				$("#col4_filter").val(data.search.field.type);
				
				$("#col3_filter").val(data.search.field.name);
				
				//$("#col4_filter").val(data.search.field.homeselect);
            ...
            "fnDrawCallback": function(oSettings) {
				...
				$('#data-table-default').editable({
                    ...
                    url: 'sqledit/tmp_jedit.php',
                    name: "type",
                    source: "sqledit/tmp_get_list.php?list_id=1",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/tmp_del.php?id_del=" + id + "&pic=" + pic,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/tmp_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"
...
function change_datatables(id,e) {
    e.preventDefault(); // prevent form submit
    swal({
			title: '確定替換?',
			text: "此動作將會更改目前設定中的版型外觀",
			type: 'warning',
			showCancelButton: true,
			cancelButtonText: '取消',
			confirmButtonText: '確定',
			...
			preConfirm: function() {
				return new Promise(function(resolve) {
					...
					$.ajax({
						url: "sqldatatable/tmp_change.php?id=" + id,
						type: "GET"
					}).done(function(response) {
						swal({
							title: '已替換',
...
function change_datatables_select(id,e) {
    e.preventDefault(); // prevent form submit
	// 異步swal
    (async function backAndForth() {
	  const steps = ['1'];
	  const stepsname = ['請選擇您要替換版型的裝置?'];
	  const values = [];
	  let currentStep;
	  ...
	  for (currentStep = 0; currentStep < 1;) {
			const result = await swal({
				title: stepsname[currentStep],
		  ...
		  if (currentStep === 1) {
			...
			preConfirm: function() {
				return new Promise(function(resolve) {
					...
					$.ajax({
						url: "sqldatatable/tmp_change_select.php?id=" + id + "&device=" + result.value,
						type: "GET"
					}).done(function(response) {
						swal({
							title: '已替換',

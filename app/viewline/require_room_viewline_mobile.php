<ol class="breadcrumb hidden-xs" data-scroll-reveal='enter top after 0.8s'>
  ...
  <li class="<?php if ($_GET['Opt'] != 'detailed') { echo 'current';}?>"><a href="<?php echo $SiteBaseUrl . url_rewrite("room",array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'subpage','type1'=>$_GET['type1']),'',$UrlWriteEnable);?>">
  <?php
		  	do {  //比較字串
  ?>
		  <?php if (!(strcmp($row_RecordRoomViewLine_l1['item_id'], $_GET['type1']))) { echo $row_RecordRoomViewLine_l1['itemname']; } ?>
		  <?php
  ...
  <?php if (isset($_GET['type2'])) { ?>
    <li class="<?php if ($_GET['Opt'] != 'detailed') { echo 'current';}?>"><a href="<?php echo $SiteBaseUrl . url_rewrite("room",array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'subpage','type1'=>$_GET['type1'],'type2'=>$_GET['type2']),'',$UrlWriteEnable);?>">
  <?php
		  	do {  //比較字串
  ?>
		  <?php if (!(strcmp($row_RecordRoomViewLine_l1['item_id'], $_GET['type2']))) { echo $row_RecordRoomViewLine_l1['itemname']; } ?>
		  <?php
} while ($row_RecordRoomViewLine_l1 = mysqli_fetch_assoc($RecordRoomViewLine_l1));
  $rows = mysqli_num_rows($RecordRoomViewLine_l1);
  if($rows > 0) {
  ...
  <?php if (isset($_GET['type3'])) { ?>
     <li class="<?php if ($_GET['Opt'] != 'detailed') { echo 'current';}?>"><a href="<?php echo $SiteBaseUrl . url_rewrite("room",array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'subpage','type1'=>$_GET['type1'],'type2'=>$_GET['type2'],'type3'=>$_GET['type3']),'',$UrlWriteEnable);?>">
  <?php
		  	do {  //比較字串
  ?>
		  <?php if (!(strcmp($row_RecordRoomViewLine_l1['item_id'], $_GET['type3']))) { echo $row_RecordRoomViewLine_l1['itemname']; } ?>
		  <?php
} while ($row_RecordRoomViewLine_l1 = mysqli_fetch_assoc($RecordRoomViewLine_l1));
  $rows = mysqli_num_rows($RecordRoomViewLine_l1);
  if($rows > 0) {
  ...
  <li class="current"><a><?php echo $row_RecordRoomKeyWord['name']; ?></a></li>
  <?php } ?>
  <?php } else { // Show if recordset not empty ?>
  <li class="current"><a href="<?php echo $SiteBaseUrl . url_rewrite("room",array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'viewpage'),'',$UrlWriteEnable);?>"><?php echo $ModuleName['Room']; //產品資訊 ?></a></li>
  <?php }  ?>
   <?php
			switch($_GET['Opt'])
			{

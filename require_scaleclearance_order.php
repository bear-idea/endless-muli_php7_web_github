...
$query_RecordScale = sprintf("SELECT * FROM erp_scaleorderclearancedetail WHERE (code $scalelink %s) && (lang = %s) && userid=%s && postdate BETWEEN %s AND %s ORDER BY postdate DESC, sortid ASC, id DESC", GetSQLValueString( $colname_RecordScale, "text"),GetSQLValueString($collang_RecordScale, "text"),GetSQLValueString($coluserid_RecordScale, "int"),GetSQLValueString($colstartdate_RecordScaleorder, "date"),GetSQLValueString($colenddate_RecordScaleorder, "date"));
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordDate = sprintf("SELECT * FROM erp_scale WHERE userid=%s ORDER BY sortid ASC, code ASC",GetSQLValueString($coluserid_RecordDate, "int"));
...
<?php } else { ?>
<?php include($TplPath . "/scaleclearance_order.php"); ?>
<?php } ?>
<?php

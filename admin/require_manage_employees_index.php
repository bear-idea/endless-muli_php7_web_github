...
/* 取得發佈者資料 */
$colname_RecordEmployeesListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordEmployeesListAuthor = $_GET['lang'];
}
$coluserid_RecordEmployeesListAuthor = "-1";
if (isset($w_userid)) {
  $coluserid_RecordEmployeesListAuthor = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordEmployeesListAuthor = sprintf("SELECT * FROM demo_employeesitem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordEmployeesListAuthor, "text"),GetSQLValueString($coluserid_RecordEmployeesListAuthor, "int"));
$RecordEmployeesListAuthor = mysqli_query($DB_Conn, $query_RecordEmployeesListAuthor) or die(mysqli_error($DB_Conn)); 
$row_RecordEmployeesListAuthor = mysqli_fetch_assoc($RecordEmployeesListAuthor);
$totalRows_RecordEmployeesListAuthor = mysqli_num_rows($RecordEmployeesListAuthor);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/employees_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 員工管理 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_employees.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_employees.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',
...
mysqli_free_result($RecordEmployeesListAuthor);
?>

...
<table width="100%" border="0" cellspacing="0" cellpadding="0"  class="TB_General_style00">
  <tr>
    <td width="20%"><?php echo $Lang_Content_Count_Display; // 顯示 ?> <?php echo ($startRow_RecordMemberFAQ + 1) ?> - <?php echo min($startRow_RecordMemberFAQ + $maxRows_RecordMemberFAQ, $totalRows_RecordMemberFAQ) ?> <?php echo $Lang_Content_Count_Lots; //筆 ?> <?php echo $Lang_Content_Count_Total; // 共計?> <?php echo $totalRows_RecordMemberFAQ ?> <?php echo $Lang_Content_Count_Lots; //筆 ?></td>
    <td width="80%" align="right">
      <div class="PageSelectBoard">
        <?php 
      # variable declaration
      $prev_RecordMemberFAQ = "<i class=\"fa fa-angle-left\"></i>";
      $next_RecordMemberFAQ = "<i class=\"fa fa-angle-right\"></i>";
      $separator = "&nbsp;";
      $max_links = 6;
      $pages_navigation_RecordMemberFAQ = buildNavigation($page,$totalPages_RecordMemberFAQ,$prev_RecordMemberFAQ,$next_RecordMemberFAQ,$separator,$max_links,true); 
       ?>
        <?php if ($page > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?page=%d%s", $currentPage, 0, $queryString_RecordMemberFAQ); ?>"><i class="fa fa-angle-double-left"></i></a>
        <?php } // Show if not first page ?>
        <?php print $pages_navigation_RecordMemberFAQ[0]; ?> <?php print $pages_navigation_RecordMemberFAQ[1]; ?> <?php print $pages_navigation_RecordMemberFAQ[2]; ?>
        <?php if ($page < $totalPages_RecordMemberFAQ) { // Show if not last page ?>
        <a href="<?php printf("%s?page=%d%s", $currentPage, $totalPages_RecordMemberFAQ, $queryString_RecordMemberFAQ); ?>"><i class="fa fa-angle-double-right"></i></a>
        <?php } // Show if not last page ?>
        <?php if (ceil($totalRows_RecordMemberFAQ/$maxRows_RecordMemberFAQ) > 1) { ?>
        <span class="Record_Board"><?php echo $Lang_PageNum;; // 頁數?>：<?php print $page+1; ?> / <?php echo ceil($totalRows_RecordMemberFAQ/$maxRows_RecordMemberFAQ); ?></span>
        <?php } ?>
      </div></td>
  </tr>
</table>

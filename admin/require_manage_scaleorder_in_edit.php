...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Scaleorder_in")) {
	
	/* 解析 title */
	list($pdname, $pdcode) = explode('_', $_POST['title']);
	
   $updateSQL = sprintf("UPDATE erp_scaleorderindetail SET title=%s, code=%s, author=%s, warehouse=%s, Totalweight=%s, Minweight=%s, Oriweight=%s, postdate=%s, notes1=%s, lang=%s WHERE id=%s",
  ...
  $updateGoTo = "manage_scaleorder_in.php?Opt=viewpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordScaleorder = sprintf("SELECT * FROM erp_scaleorderindetail WHERE id = %s && userid=%s", GetSQLValueString($colname_RecordScaleorder, "int"),GetSQLValueString($coluserid_RecordScaleorder, "int"));
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 庫存物料 <small>修改</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      ...
      <div class="form-group row">
          <label class="col-md-2 col-form-label ">廠區</label>
          <div class="col-md-10">
              <select name="warehouse" id="warehouse" class="form-control" data-parsley-trigger="blur" required="">
                        <option value="-1" <?php if (!(strcmp(-1, $row_RecordScaleorder['warehouse']))) {echo "selected=\"selected\"";} ?>>-- 選擇廠區 --</option>
                        <?php
				do {  
				?>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <select name="title" id="title" class="form-control" data-parsley-trigger="blur" required="">
                <option value="" <?php if (!(strcmp(-1, $row_RecordScaleorder['title']))) {echo "selected=\"selected\"";} ?>>-- 選擇商品 --</option>
                      <?php
				do {  
				?>
                      <option value="<?php echo $row_RecordScale['name']?>_<?php echo $row_RecordScale['code']?>" <?php if (!(strcmp($row_RecordScale['code'], $row_RecordScaleorder['code']))) {echo "selected=\"selected\"";} ?>>[<?php echo $row_RecordScale['code']?>] <?php echo $row_RecordScale['name']?></option>
                      <?php
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <input name="notes1" type="text" class="form-control" id="notes1" value="<?php echo $row_RecordScaleorder_in['notes1']; ?>" size="50" maxlength="50"/>    
          </div>
      </div>
      <div class="form-group row">
      ...
      <input type="hidden" name="MM_update" value="form_Scaleorder_in" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>

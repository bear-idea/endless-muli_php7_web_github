...
if($orderSql == "") {
	$orderSql = $orderSql . "ORDER BY sumtotal DESC";
}else{
	$orderSql = $orderSql . ",sumtotal DESC";
}
...
$query_RecordCount = sprintf("SELECT count(oid) as sum FROM demo_cartorders WHERE lang = %s && userid=%s GROUP BY ocbuyname",GetSQLValueString($collang_RecordCart, "text"),GetSQLValueString($coluserid_RecordCart, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);
$totalRows_RecordCount = mysqli_num_rows($RecordCount);

$query_RecordCart = sprintf("SELECT ocbuyname, ocbuymail, ocbuyphone, SUM(octotal) AS sumtotal, lang, userid, postdate, oid FROM demo_cartorders WHERE (ocbuyname LIKE binary %s) && lang = %s && userid=%s && postdate BETWEEN %s AND %s GROUP BY ocbuyname $orderSql",GetSQLValueString("%" . $DT_search . "%", "text"),GetSQLValueString($collang_RecordCart, "text"),GetSQLValueString($coluserid_RecordCart, "int"),GetSQLValueString($colstartdate_RecordCart, "date"),GetSQLValueString($colenddate_RecordCart, "date"));
...
<?php do { ?>
  <?php 
	...
	$dvalue['sumtotal'] = $row_RecordCart["sumtotal"];

...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_CartItemAdd")) {
  $insertSQL = sprintf("INSERT INTO demo_cartitem (list_id, itemname, lang, userid) VALUES (%s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_CartItemEdit")) {
	foreach($_POST['item_id'] as $key => $val){
	  $updateSQL = sprintf("UPDATE demo_cartitem SET list_id=%s, sortid=%s, indicate=%s, itemname=%s, lang=%s WHERE item_id=%s",
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordCartListItem = sprintf("SELECT demo_cartitem.item_id, demo_cartitem.userid, demo_cartlist.list_id, demo_cartlist.listname, demo_cartitem.itemname, demo_cartitem.sortid, demo_cartitem.indicate, demo_cartitem.lang FROM demo_cartlist LEFT OUTER JOIN demo_cartitem ON demo_cartlist.list_id = demo_cartitem.list_id WHERE demo_cartlist.list_id = %s && demo_cartitem.lang=%s && demo_cartitem.userid=%s ORDER BY demo_cartitem.sortid ASC, demo_cartitem.item_id DESC", GetSQLValueString($collistid_RecordCartListItem, "int"),GetSQLValueString($collang_RecordCartListItem, "text"),GetSQLValueString($coluserid_RecordCartListItem, "int"));
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 出貨狀態 <small>設定</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="manage_cart.php?wshop=<?php echo $wshop;?>&amp;Opt=listpage&amp;lang=<?php echo $_SESSION['lang']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
    <h4 class="panel-title"><i class="fa fa fa-edit"></i> 修改次分類</h4>
  </div>
  <!-- end panel-heading --> 
  <!-- begin panel-body -->
  <div class="panel-body">
  <div class="alert alert-warning"><i class="fa fa-info-circle"></i> <b>預設欄位已有未處理、未出貨、缺貨中、處理中、備貨中、已出貨、交易完成、交易取消，不需再重複設定。 </b></div>
    ...
    <?php do { ?>
    <div class="card bg-aqua-transparent-1" data-post-id="<?php echo $row_RecordCartListItem['item_id']; ?>">
          <div class="card-block">
              <div class="row">
              <div class="col-md-3">
              ...
              <div class="form-group row">
                  <!--<div class="col-md-6" style="border:0">-->
                      <div class="input-group p-0">
                            ...
                            <input name="sortid[]" type="number" id="sortid[]" value="<?php echo $row_RecordCartListItem['sortid']; ?>" class=" form-control" maxlength="10" data-parsley-trigger="blur" required=""/>
              ...
              <div class="form-group row">
                  ...
                  <option <?php if (!(strcmp(1, $row_RecordCartListItem['indicate']))) {echo "selected=\"selected\"";} ?> value="1" >顯示</option>
                  <option <?php if (!(strcmp(0, $row_RecordCartListItem['indicate']))) {echo "selected=\"selected\"";} ?> value="0" >隱藏</option>
                </select>

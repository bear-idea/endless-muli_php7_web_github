...
{
    GLOBAL $maxRows_RecordWebuser,$totalRows_RecordWebuser;
		...
		if($totalPages_Recordset1 >= 1) {
			...
			if(!empty($_GET) || !empty($HTTP_GET_VARS)){
				...
				foreach ($_GET as $_get_name => $_get_value) {
					if ($_get_name != "pageNum_RecordWebuser") {
			...
			$firstArray = ($pageNum_Recordset1 > 0) ? "<a href=\"$_SERVER[PHP_SELF]?pageNum_RecordWebuser=$precedente$_get_vars\">$prev_Recordset1</a>" :  "<span>$prev_Recordset1</span>"/*  css */;
			# ----------------------
			# page numbers
			# ----------------------
			for($a = $fgp+1; $a <= $egp; $a++){
				...
				if($show_page)
				{
					...
					$min_l = (($a-1)*$maxRows_RecordWebuser) + 1;
					$max_l = ($a*$maxRows_RecordWebuser >= $totalRows_RecordWebuser) ? $totalRows_RecordWebuser : ($a*$maxRows_RecordWebuser);
					$textLink = "$min_l - $max_l";
				}
				$_ss_k = floor($theNext/26);
				if ($theNext != $pageNum_Recordset1)
				{
					$pagesArray .= "<a href=\"$_SERVER[PHP_SELF]?pageNum_RecordWebuser=$theNext$_get_vars\">";
			...
			$lastArray = ($pageNum_Recordset1 < $totalPages_Recordset1) ? "<a href=\"$_SERVER[PHP_SELF]?pageNum_RecordWebuser=$successivo$_get_vars\">$next_Recordset1</a>" : "<span>$next_Record1</span>"; /* css */
		}
	}
...
/*echo $deleteSQLSetting = sprintf("DELETE FROM $row[0] WHERE userid=%s",
                       GetSQLValueString($_GET['id_del'], "int"));

 		 //mysqli_select_db($database_DB_Conn, $DB_Conn);
  		$ResultSetting = mysqli_query($DB_Conn, $deleteSQLSetting) or die(mysqli_error($DB_Conn)); */
	//$row = mysqli_fetch_row($RecordShowTable);
	
	
	/*while ($row = mysqli_fetch_row($RecordShowTable)) {
		echo $deleteSQLSetting = sprintf("DELETE FROM `$row[0]` WHERE userid=%s",
                       GetSQLValueString($_GET['id_del'], "int"));
...
/* 取得最新訊息資訊 */
$maxRows_RecordWebuser = 25;
$pageNum_RecordWebuser = 0;
if (isset($_GET['pageNum_RecordWebuser'])) {
  $pageNum_RecordWebuser = $_GET['pageNum_RecordWebuser'];
}
$startRow_RecordWebuser = $pageNum_RecordWebuser * $maxRows_RecordWebuser;
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordWebuser = sprintf("SELECT * FROM demo_admin WHERE grouptype='sub' && groupid=%s ORDER BY id DESC",GetSQLValueString($coluserid_RecordWebuser, "int"));
...
<script src="<?php echo $SiteAdminPath; ?>sqldatatable/js/webuser_datatable_sub.js"></script>
<!-- ================== END Datatables JS ================== -->
  ...
  <div class="panel-body">
    <div class="row justify-content-end">
      ...
      <div class="col-md-5 m-b-10">
        <div class="input-group"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 帳號</span></span>
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          ...
          <th width="100" data-priority="1"><strong>等級</strong></th>
         
          <th width="70"><strong>帳號</strong></th>
          <th width="100"><strong>最近登入</strong></th>
          <th width="70"><strong>登入次數</strong></th>

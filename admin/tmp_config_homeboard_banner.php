...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Tmp")) {
  $updateSQL = sprintf("UPDATE demo_tmp SET tmphomebannerselect=%s WHERE id=%s",
                       GetSQLValueString($_POST['tmphomebannerselect'], "int"),
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="tmp_config_<?php echo $_GET['board']; ?>.php?lang=<?php echo $_GET['lang']; ?>&amp;id_edit=<?php echo $row_RecordTmp['id']; ?>&amp;board=<?php echo $_GET['board']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post" id="form_Tmp" name="form_Tmp">
       <div class="form-group row">
        ...
        <div class="col-md-12"><span class="badge badge-primary"><i class="fa fa-paper-plane"></i> Banner</span></div>
      </div>
       <div class="form-group row">
          <label class="col-md-2 col-form-label">橫幅選擇<span class="text-red">*</span></label>                       	
          <div class="col-md-10">
          
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordTmp['tmphomebannerselect'],"0"))) {echo "checked=\"checked\"";} ?> type="radio" name="tmphomebannerselect" id="tmphomebannerselect_0" value="0" />
                <label for="tmphomebannerselect_0">延續使用內頁橫幅設定</label> <i class="fa fa-info-circle text-orange" data-original-title="此選項會延續您內頁版型所設定之橫幅狀態，若要更改橫幅您可至《版型設定》中修改。" data-toggle="tooltip" data-placement="top"></i>
            </div>
            <div class="radio radio-css radio-inline">
                <input <?php if (!(strcmp($row_RecordTmp['tmphomebannerselect'],"1"))) {echo "checked=\"checked\"";} ?> type="radio" name="tmphomebannerselect" id="tmphomebannerselect_1" value="1" />
                <label for="tmphomebannerselect_1">使用首頁橫幅</label> <a href="manage_ads_home_image.php?wshop=<?php echo $wshop;?>&amp;Opt=viewpage&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;board=<?php echo $_GET['board']; ?>" target="_top" class="btn btn-primary btn-xs" data-original-title="點選進入橫幅上傳畫面。" data-toggle="tooltip" data-placement="top">前往首頁輪播橫幅設定 <i class="fa fa-chevron-circle-right"></i></a>
            </div>

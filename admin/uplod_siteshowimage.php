...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image";
	$ppu->extensions = "JPG,JPEG,BMP,PNG";
	$ppu->formName = "form_SiteShowImage";
	$ppu->storeType = "file";
	$ppu->sizeLimit = "500";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_SiteShowImage")) {
  $updateSQL = sprintf("UPDATE demo_setting_fr SET SiteFBShowImage=IFNULL(%s,SiteFBShowImage) WHERE id=%s",
...
$colname_RecordSiteShowImage = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordSiteShowImage = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSiteShowImage = sprintf("SELECT * FROM demo_setting_fr WHERE id = %s", GetSQLValueString($colname_RecordSiteShowImage, "int"));
$RecordSiteShowImage = mysqli_query($DB_Conn, $query_RecordSiteShowImage) or die(mysqli_error($DB_Conn));
$row_RecordSiteShowImage = mysqli_fetch_assoc($RecordSiteShowImage);
$totalRows_RecordSiteShowImage = mysqli_num_rows($RecordSiteShowImage);
?>
<!DOCTYPE html>
<html lang="zh-TW">
<!--<![endif]-->
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordSiteShowImage['id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordSiteShowImage['SiteFBShowImage']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_SiteShowImage" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 500,  
		<?php if ($row_RecordSiteShowImage['SiteFBShowImage'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/<?php echo $row_RecordSiteShowImage['SiteFBShowImage']; ?>"],
		<?php } ?>

...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
  ...
  $sip->quality = "100";
  $sip->makeThumb = "true";
  $sip->pathThumb = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/imageshow/thumb";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Imageshow")) {
  $updateSQL = sprintf("UPDATE demo_imageshow SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/imageshow/' . $_POST['oldpic']);
  //@unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/imageshow/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_imageshow.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordImageshow = sprintf("SELECT * FROM demo_imageshow WHERE id = %s", GetSQLValueString($colname_RecordImageshow, "int"));
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordImageshow['pic']; ?>" />
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/imageshow/<?php echo $row_RecordImageshow['pic']; ?>"],
		<?php } ?>

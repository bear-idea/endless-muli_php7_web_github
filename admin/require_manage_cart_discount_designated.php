...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Form_Choose" && $_POST['id'] != "")) {
	foreach($_POST['id'] as $i => $val){	
	  $updateSQL = sprintf("UPDATE demo_product SET discounttype=%s, discountid=%s WHERE id =%s",
						   ...
						   GetSQLValueString($_POST['id_edit'], "int"),
      ...
      $coluserid_RecordCartlist = "-1";
        if (isset($w_userid)) {
          $coluserid_RecordCartlist = $w_userid;
      }
        
	  $query_RecordCartlist = sprintf("SELECT id,pid FROM demo_cart WHERE pid=%s && userid=%s ORDER BY id DESC",GetSQLValueString($val, "int"),GetSQLValueString($coluserid_RecordCartlist, "int"));
...
if (isset($_GET['id_edit'])) {
  $colid_RecordDiscountSelect = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/cart_datatable_discount_product_get.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 商品折扣 <small>選擇</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
?>
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          ...
          <th width="100" data-priority="1"><strong>折扣名稱</strong></th>
          <th width="100" data-priority="1"><strong>折扣方式</strong></th>
    ...
    <button type="submit" class="btn btn btn-primary btn-block m-t-10"><i class='fa fa-plus-circle'></i> 送出所選取指定商品</button>
    <input type="hidden" name="MM_update" value="Form_Choose" />
    <input name="id_edit" type="hidden" id="id" value="<?php echo $row_RecordDiscountSelect['id']; ?>" />

...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/timeline";
	$ppu->extensions = "GIF,JPG,JPEG,BMP,PNG";
	$ppu->formName = "form_Timeline";
	$ppu->storeType = "file";
	$ppu->sizeLimit = "3000";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Timeline")) {
  $updateSQL = sprintf("UPDATE demo_timeline SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/timeline/' . $_POST['oldpic']);
  //@unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/tmp/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_timeline.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_RecordTimeline = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordTimeline = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordTimeline = sprintf("SELECT * FROM demo_timeline WHERE id = %s", GetSQLValueString($colname_RecordTimeline, "int"));
$RecordTimeline = mysqli_query($DB_Conn, $query_RecordTimeline) or die(mysqli_error($DB_Conn));
$row_RecordTimeline = mysqli_fetch_assoc($RecordTimeline);
$totalRows_RecordTimeline = mysqli_num_rows($RecordTimeline);
?>
<!DOCTYPE html>
<html lang="zh-TW">
<!--<![endif]-->
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordTimeline['id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordTimeline['pic']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Timeline" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 3000,  
		<?php if ($row_RecordTimeline['pic'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/timeline/<?php echo $row_RecordTimeline['pic']; ?>"],
		<?php } ?>

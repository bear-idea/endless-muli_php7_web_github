...
$query_RecordAboutListType = sprintf("SELECT * FROM demo_aboutitem WHERE list_id = 1 && lang=%s && level = 0 && userid=%s", GetSQLValueString($colname_RecordAboutListType, "text"),GetSQLValueString($coluserid_RecordAboutListType, "int"));
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/about_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> <?php echo $ModuleName['About']; ?> <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
<?php if ($totalRows_RecordAboutListType > 0) { ?>
<!-- begin panel -->
    ...
    <table id="data-table-type" class="table table-striped table-bordered" style="width:100%">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><?php echo $row_RecordAboutListType['itemname']; ?></td>
          <td><?php 
			/* 判斷訊息是否要顯示 */
			if($row_RecordAboutListType['indicate'] == "1") { 
				echo "公佈"; 
			} else { 
				echo "隱藏";
			}
			?></td>
          <td><a href='manage_about.php?wshop=<?php echo $wshop;?>&amp;Opt=viewpage_sub&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;item_id=<?php echo $row_RecordAboutListType['item_id']; ?>&amp;level=1&amp;type1=<?php echo $row_RecordAboutListType['item_id']; ?>' class='btn btn-xs btn-primary btn-block' style='text-align:center'><i class='fa fa-eye'></i> 查看</a></td>
        </tr>
        <?php } while ($row_RecordAboutListType = mysqli_fetch_assoc($RecordAboutListType)); ?>
      </tbody>
    </table>
  </div>
  <!-- end panel-body --> 
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-body">
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr class="hidden-xs hidden-sm" id="FitterColumn" style="display:none">
          <th></th>
          <th width="auto"><input type="text" class="form-control" placeholder="過濾標題" id="tb_filter_title"/>
          </th>
          <th width="70"><input type="text" class="form-control" placeholder="過濾排序" id="tb_filter_sortid"/>
          </th>
          <th width="70"> <select name="indicate" class="form-control" id="tb_filter_indicate">
              <option value="">全部</option>
              <option value="公佈">公佈</option>
              <option value="隱藏">隱藏</option>
            </select>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_about.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_Tip',
                intro: '若此時您新增的為您目前第一筆資料，您必須替目前模組設定一個首頁；若您目前有多筆資料，您可以選擇一個您喜歡的首頁。'
              },
              {
                element: '#Step_Home',
                intro: '設置目前模組的首頁項目。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="about_home.php?lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置首頁</a></span></div>',
                position: 'bottom'
              },
              {
                element: '#Step_View',
              ...
              {
                element: '#Step_Edit',
                intro: '<img src="images/tip/tip059.jpg" width="500" class="img-fluid" /><br /><br />點選文字可直接修改。',
                position: 'bottom'
              },
              {
                element: '#Step_Sort',
                intro: '<img src="images/tip/tip060.jpg" class="img-fluid"/><br /><br />點選文字可直接修改，更改數字即可排序。',
                position: 'bottom'
              },
              {
                element: '#Step_Indicate',
                intro: '<img src="images/tip/tip061.jpg"  class="img-fluid"/><br /><br />點選文字可直接修改，勾選為公佈、反之則隱藏。',
                position: 'bottom'
              }
            ],

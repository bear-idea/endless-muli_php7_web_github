...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "")) {
  ...
  if (isset($_GET['pic'])) {
	$colname_RecordBulletinGet = $_GET['pic'];
  }
  ...
  if($totalRows_RecordBulletinGet == 1 ){ /* 如果資料庫只有一筆 就刪除 若此圖有多張 */
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/bulletin/' . $_GET['pic']);
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/bulletin/thumb/small_' . GetFileThumbExtend($_GET['pic']));
  }
  
  $deleteSQL = sprintf("DELETE FROM demo_bulletin WHERE userid=%s && id=%s",

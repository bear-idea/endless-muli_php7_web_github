var handleDataTableDefault = function(aid) {
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({ //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
            ...
            "autoWidth": false,
            /*"ajax": {
                "url": "sqldatatable/accounts_summonsdetail_view_add.php?aid=" + aid,
                "type": "post",
				"data": function ( d ) {
					...
					//d.search_postdate = $('#col6_filter').val();
            ...
            "footerCallback": function(row, data, start, end, display) {
                ...
                // Update footer
                //$(api.column(6).footer()).html('借貸差額:' + total_minamount);
                $(api.column(2).footer()).html('借方合計:' + total_debitamount);
                $(api.column(3).footer()).html('貸方合計:' + total_creditamount);
            },
            'columnDefs': [{
                'targets': 0,
                'searchable': false,
            ...
            "stateLoadParams": function(settings, data) {
                ...
                //$("#col6_filter").val(data.search.field.postdate);
            ...
            "fnDrawCallback": function(oSettings) {
                ...
                $('#data-table-default').editable({
                    selector: ".ed_notes1",
                    //url: 'sqledit/accounts_summonsdetail_jedit.php',
                    type: 'text',
                    name: 'notes1',
                    //title: '輸入標題',
        ...
        // 取得 key input focus 
        /*$('#data-table-default').DataTable().on('key-focus', function ( e, datatable, cell, originalEvent ) {
       		$('input', cell.node()).focus();
		}).on("focus", "td input", function(){
			   $(this).select();
		});
		
		$('#data-table-default tbody').on( 'mouseenter', 'td', function () {
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/accounts_summonsdetail_del.php?id_del=" + id,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/accounts_summonsdetail_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"
...
function delRow(Obj) {
    ...
    //alert(DomObj.className);
	//alert($('#data-table-default').DataTable().row( this ).id());
	//console.log($(Obj).parents("tr").index());
	rowID = $(Obj).parents("tr").index();
	$('#data-table-default').DataTable().row(rowID).remove().draw();
    //console.log($('#data-table-default').DataTable().search($('#global_filter').val()).draw());
}

...
if(isset($order_column)){
    ...
    switch($i){
        ...
        case 3;$orderSql = " ORDER BY inventory ".$order_dir;break;
        case 4;$orderSql = " ORDER BY inventoryshow ".$order_dir;break;
		case 5;$orderSql = " ORDER BY inventorynotsale ".$order_dir;break;
...
if (isset($search_indicate) && $search_indicate != "") {
  ...
  if($search_indicate == "隱藏頁面") {$search_indicate = '2';}
...
$query_RecordProduct = sprintf("SELECT * FROM demo_product WHERE name LIKE %s && indicate LIKE %s && type1 LIKE %s && type2 LIKE %s && type3 LIKE %s && lang = %s && userid=%s $orderSql", GetSQLValueString("%" . $colsearch_RecordProduct . "%", "text"), GetSQLValueString($colindicate_RecordProduct, "text"),GetSQLValueString($coltype1_RecordProduct, "text"),GetSQLValueString($coltype2_RecordProduct, "text"),GetSQLValueString($coltype3_RecordProduct, "text"),GetSQLValueString($collang_RecordProduct, "text"),GetSQLValueString($coluserid_RecordProduct, "int"));
...
<?php do { ?>
  ...
  <?php 
	...
	$link_edit = "manage_product.php?wshop=".$wshop."&amp;Opt=editinventory&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordProduct['id'];
	...
	if($row_RecordProduct["inventory"] != ""){        
		$dvalue['inventory'] = "<span id='inventory_".$row_RecordProduct["id"]."' class='ed_inventory' data-type='text' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>".$row_RecordProduct["inventory"]."</span>";
	}else{
		$dvalue['inventory'] = "<span id='inventory_".$row_RecordProduct["id"]."' class='ed_inventory editable-click editable-empty' data-type='text' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>"."Empty"."</span>";
	}

	if($row_RecordProduct["inventoryshow"] == '1') {$row_RecordProduct["inventoryshow"] = "顯示";}else{$row_RecordProduct["inventoryshow"] = "隱藏";}
	$dvalue['inventoryshow'] = "<span id='inventoryshow_".$row_RecordProduct["id"]."' class='ed_inventoryshow' data-type='select' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>".$row_RecordProduct["inventoryshow"]."</span>";
	
	if($row_RecordProduct["inventorynotsale"] == '1') {$row_RecordProduct["inventorynotsale"] = "啟用";}else{$row_RecordProduct["inventorynotsale"] = "關閉";}
	$dvalue['inventorynotsale'] = "<span id='inventorynotsale_".$row_RecordProduct["id"]."' class='ed_inventorynotsale' data-type='select' data-pk='".$row_RecordProduct["id"]."' data-placement='top'>".$row_RecordProduct["inventorynotsale"]."</span>";
	...
	if($row_RecordProduct["indicate"] == '1') {$row_RecordProduct["indicate"] = "上架";}
	if($row_RecordProduct["indicate"] == '0') {$row_RecordProduct["indicate"] = "下架";}
	if($row_RecordProduct["indicate"] == '2') {$row_RecordProduct["indicate"] = "隱藏頁面";}

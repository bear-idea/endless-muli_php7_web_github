...
if(isset($order_column)){
    ...
    switch($i){
		...
		case 8;$orderSql = " ORDER BY notes1 ".$order_dir;break;
...
$query_RecordCount = sprintf("SELECT count(id) as sum FROM invoicing_accounts_summonsdetail WHERE lang = %s && userid=%s && state!=%s",GetSQLValueString($collang_RecordAccounts_summonsdetail, "text"),GetSQLValueString($coluserid_RecordAccounts_summonsdetail, "int"),GetSQLValueString('system', "text"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);

//if($DT_search != "") {

$query_RecordAccounts_summonsdetail = sprintf("SELECT * FROM invoicing_accounts_summonsdetail WHERE IFNULL(notes1,1) LIKE %s && ordertype REGEXP %s && state!=%s && type1 LIKE %s && type2 LIKE %s && type3 LIKE %s && type4 LIKE %s && lang=%s && userid=%s && postdate BETWEEN %s AND %s $orderSql", GetSQLValueString("%" . $colsearch_RecordAccounts_summonsdetail . "%", "text"),GetSQLValueString($ordertype, "text"),GetSQLValueString('system', "text"),GetSQLValueString($coltype1_RecordAccounts_summonsdetail, "text"),GetSQLValueString($coltype2_RecordAccounts_summonsdetail, "text"),GetSQLValueString($coltype3_RecordAccounts_summonsdetail, "text"),GetSQLValueString($coltype4_RecordAccounts_summonsdetail, "text"),GetSQLValueString($collang_RecordAccounts_summonsdetail, "text"),GetSQLValueString($coluserid_RecordAccounts_summonsdetail, "int"),GetSQLValueString($colstartdate_RecordAccounts_summonsdetail, "date"),GetSQLValueString($colenddate_RecordAccounts_summonsdetail, "date"));
...
<?php do { ?>
  <?php 
	...
	$dvalue['ordertype'] = $row_RecordAccounts_summonsdetail["ordertype"];
	
	$dvalue['summonsnumber'] = "<a href='".$link_detail."' class='btn btn-xs btn-link colorbox_iframe_cd' style='text-align:center'><i class='fa fa-link'></i> ".$row_RecordAccounts_summonsdetail["summonsnumber"]."</a>";

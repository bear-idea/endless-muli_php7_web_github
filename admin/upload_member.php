...
if (isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/member";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Member")) {
  $updateSQL = sprintf("UPDATE demo_member SET avatar=IFNULL(%s,avatar) WHERE id=%s",
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordMember = sprintf("SELECT * FROM demo_member WHERE id = %s", GetSQLValueString($colname_RecordMember, "int"));
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordMember['avatar']; ?>" />
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/member/<?php echo $row_RecordMember['avatar']; ?>"],
		<?php } ?>

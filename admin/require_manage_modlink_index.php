...
/* 取得發佈者資料 */
$colname_RecordModlinkListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordModlinkListAuthor = $_GET['lang'];
}
$coluserid_RecordModlinkListAuthor = "-1";
if (isset($w_userid)) {
  $coluserid_RecordModlinkListAuthor = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordModlinkListAuthor = sprintf("SELECT * FROM demo_modlinkitem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordModlinkListAuthor, "text"),GetSQLValueString($coluserid_RecordModlinkListAuthor, "int"));
$RecordModlinkListAuthor = mysqli_query($DB_Conn, $query_RecordModlinkListAuthor) or die(mysqli_error($DB_Conn)); 
$row_RecordModlinkListAuthor = mysqli_fetch_assoc($RecordModlinkListAuthor);
$totalRows_RecordModlinkListAuthor = mysqli_num_rows($RecordModlinkListAuthor);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/modlink_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> <?php echo $ModuleName['Modlink']; ?> <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_Tip1',
                intro: '<img src="images/tip/tip052.jpg" width="300" height="300" /><br /><br />模組連結可加入目前可使用的模組於您的側邊欄位中。',
                position: 'bottom'
              },
			  {
                element: '#Step_Tip2',
                intro: '<img src="images/tip/tip053.jpg" width="300" height="300" /><br /><br />可以使用內建圖片或自訂圖片。',
                position: 'bottom'
              },
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_modlink.php?wshop=<?php echo $wshop;?>&Opt_Modlink=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_modlink.php?wshop=<?php echo $wshop;?>&Opt_Modlink=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',
              ...
              {
                element: '#Step_Column',
                intro: '<img src="images/tip/tip051.jpg" width="488" height="148" /><br /><br />此項目欲顯示在網站中必須搭配自訂欄位，在自訂欄位加入模組連結至您的側邊欄位中。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_leftcolumn.php?wshop=<?php echo $wshop;?>&amp;Opt_Tmp=tmpcolumn&amp;lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往自訂欄位</a></span></div>',
                position: 'bottom'
              },
              {
                element: '#Step_End',
                intro: '<img src="images/tip/tip032.jpg" width="300" height="300" /><br /><br />設置完後前台顯示效果如圖。',
                position: 'bottom'
              }
            ]
          });
...
mysqli_free_result($RecordModlinkListAuthor);
?>

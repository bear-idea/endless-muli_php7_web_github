...
$colid_RecordContact = "-1";
if (isset($_GET['id'])) {
  $colid_RecordContact = $_GET['id'];
}
...
$query_RecordContact = sprintf("SELECT * FROM demo_contact WHERE lang = %s && type1 = %s && type2 = %s && type3 = %s && id = %s  && indicate = 1 && userid=%s ORDER BY sortid ASC, id DESC", GetSQLValueString($colnamelang_RecordContact, "text"),GetSQLValueString($coltype1_RecordContact, "int"),GetSQLValueString($coltype2_RecordContact, "int"),GetSQLValueString($coltype3_RecordContact, "int"),GetSQLValueString($colid_RecordContact, "int"),GetSQLValueString($coluserid_RecordContact, "int"));
...
?>
<?php } else { ?>
<?php include($TplPath . "/contact_detailed.php"); ?>
<?php } ?>
<?php

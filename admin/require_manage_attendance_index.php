...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/attendance_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 出勤紀錄 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
do {  
?>
                  <option value="<?php echo $row_RecordAttendanceListDepartment['itemname']?>"><?php echo $row_RecordAttendanceListDepartment['itemname']?></option>
                  <?php
} while ($row_RecordAttendanceListDepartment = mysqli_fetch_assoc($RecordAttendanceListDepartment));
  $rows = mysqli_num_rows($RecordAttendanceListDepartment);
  if($rows > 0) {
      mysqli_data_seek($RecordAttendanceListDepartment, 0);
	  $row_RecordAttendanceListDepartment = mysqli_fetch_assoc($RecordAttendanceListDepartment);
  }
?>
          </select>
        </div>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_attendance.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_attendance.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',

...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Scaleorder_source")) {
   ...
   $updateSQL = sprintf("UPDATE erp_scaleorderclearancedetail SET title=%s, code=%s, snumber=%s, carnumber=%s, author=%s, manufacturer=%s, Totalweight=%s, Minweight=%s, Oriweight=%s, postdate=%s, notes1=%s, lang=%s WHERE id=%s",
  ...
  $updateGoTo = "manage_scaleorder_clearance.php?Opt=viewpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordScaleorder = sprintf("SELECT * FROM erp_scaleorderclearancedetail WHERE id = %s && userid=%s", GetSQLValueString($colname_RecordScaleorder, "int"),GetSQLValueString($coluserid_RecordScaleorder, "int"));
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 清運明細 <small>修改</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      ...
      <div class="form-group row">
          <label class="col-md-2 col-form-label ">廠商<span class="text-red">*</span></label>
          <div class="col-md-10">
              <select name="manufacturer" id="manufacturer" class="form-control" data-parsley-trigger="blur" required="">
                        <option value="-1" <?php if (!(strcmp(-1, $row_RecordScaleorder['manufacturer']))) {echo "selected=\"selected\"";} ?>>-- 選擇倉庫 --</option>
                        <?php
				do {  
				?>
      ...
      <div class="form-group row">
          <label class="col-md-2 col-form-label ">倉庫<span class="text-red">*</span></label>
          <div class="col-md-10">
	  ...
	  <div class="form-group row">
          ...
          <div class="col-md-10">
              <select name="carnumber" id="carnumber" class="form-control" data-parsley-trigger="blur" required="">
                        <option value="-1" <?php if (!(strcmp(-1, $row_RecordScaleorder['carnumber']))) {echo "selected=\"selected\"";} ?>>-- 選擇車號 --</option>
                        <?php
				do {  
				?>
                        <option value="<?php echo $row_RecordCarnumber['name']?>" <?php if (!(strcmp($row_RecordCarnumber['name'], $row_RecordScaleorder['carnumber']))) {echo "selected=\"selected\"";} ?>>[<?php echo $row_RecordCarnumber['code']?>] <?php echo $row_RecordCarnumber['name']; ?></option>
                        <?php
      ...
      <div class="form-group row">
          <label class="col-md-2 col-form-label ">物料<span class="text-red">*</span></label>
          <div class="col-md-10">
              <select name="title" id="title" class="form-control" data-parsley-trigger="blur" required="">
                <option value="-1" <?php if (!(strcmp(-1, $row_RecordScaleorder['title']))) {echo "selected=\"selected\"";} ?>>-- 選擇物料 --</option>
                      <?php
				do {  
				?>
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
              <input name="notes1" type="text" class="form-control" id="notes1" value="<?php echo $row_RecordScaleorder['notes1']; ?>" size="50" maxlength="50"/>    
          </div>
      </div>
      <div class="form-group row">

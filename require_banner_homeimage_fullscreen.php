...
?>
<script src="<?php if($SiteBaseUrlOuter != "") { echo $SiteJsUrlOuter; } else { echo $SiteJsUrl; } ?>/zepto.min.js"></script>
<link rel="stylesheet" href="<?php if($SiteBaseUrlOuter != "") { echo $SiteCssUrlOuter; } else { echo $SiteCssUrl; } ?>/vegas.min.css">
<script src="<?php if($SiteBaseUrlOuter != "") { echo $SiteJsUrlOuter; } else { echo $SiteJsUrl; } ?>/vegas.min.js"></script>
<script type="text/javascript">
$(function() {
    $('body').vegas({
        slides: [
		<?php do { ?>
            { src: '<?php echo $SiteImgUrl; ?><?php echo $_GET['wshop']; ?>/image/banner/<?php echo $row_RecordAds['pic']; ?>' },
	    <?php } while ($row_RecordAds = mysqli_fetch_assoc($RecordAds)); ?>
        ],
		overlay: "css/overlays/01.png", // 01~09
		transition: 'fade',
		animation: 'random'
    });
});

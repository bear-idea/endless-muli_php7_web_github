...
if($orderSql == "") {
	$orderSql = $orderSql . "ORDER BY sumsell DESC";
}else{
	$orderSql = $orderSql . ",sumsell DESC";
}
...
$query_RecordCount = sprintf("SELECT count(did) as sum FROM demo_cartdetail WHERE lang = %s && userid=%s GROUP BY dcproductname",GetSQLValueString($collang_RecordCart, "text"),GetSQLValueString($coluserid_RecordCart, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);
$totalRows_RecordCount = mysqli_num_rows($RecordCount);

$query_RecordCart = sprintf("SELECT dcproductname, SUM(dcquantiry) AS sumsell, SUM(dcitemtotal) AS sumdcitemtotal, (@SN:=@SN+1) as snnum, lang, userid, postdate, did FROM demo_cartdetail WHERE (dcproductname LIKE binary %s) && lang = %s && userid=%s && postdate BETWEEN %s AND %s GROUP BY dcproductname $orderSql",GetSQLValueString("%" . $DT_search . "%", "text"),GetSQLValueString($collang_RecordCart, "text"),GetSQLValueString($coluserid_RecordCart, "int"),GetSQLValueString($colstartdate_RecordCart, "date"),GetSQLValueString($colenddate_RecordCart, "date"));
...
<?php do { ?>
  <?php 
  	//$nestedData = array();
	$dvalue['id'] = $row_RecordCart["did"];
	
  	$dvalue['chk'] = "<input name='delCart[]' type='checkbox' id='delCart[]' value='".$row_RecordCart["did"]."\'/>";

	$dvalue['num'] = "";
	
	$dvalue['dcproductname'] = $row_RecordCart["dcproductname"];
	
	$dvalue['sumdcitemtotal'] = $row_RecordCart["sumdcitemtotal"];
	
	$dvalue['sumsell'] = $row_RecordCart["sumsell"];
...
<?php 
    $json_data = array(
            ...
            "recordsTotal"    => intval($totalRows_RecordCount),  // total number of records

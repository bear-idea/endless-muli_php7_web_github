...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "")) {
  ...
  if (isset($_GET['pic'])) {
	$colname_RecordCommodityGet = $_GET['pic'];
  }
  ...
  if($totalRows_RecordCommodityGet == 1 ){ /* 如果資料庫只有一筆 就刪除 若此圖有多張 */
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/commodity/' . $_GET['pic']);
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/commodity/thumb/small_' . GetFileThumbExtend($_GET['pic']));
  }
  
  $deleteSQL = sprintf("DELETE FROM invoicing_commodity WHERE userid=%s && id=%s",

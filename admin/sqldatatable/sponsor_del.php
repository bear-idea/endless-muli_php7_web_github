...
if ((isset($_GET['id_del'])) && ($_GET['id_del'] != "")) {
  ...
  if (isset($_GET['pic'])) {
	$colname_RecordSponsorGet = $_GET['pic'];
  }
  ...
  if($totalRows_RecordSponsorGet == 1 ){ /* 如果資料庫只有一筆 就刪除 若此圖有多張 */
	  @unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/sponsor/' . $_GET['pic']);
	  //@unlink('../' . $SiteImgFilePathAdmin . $wshop . '/image/sponsor/thumb/small_' . GetFileThumbExtend($_GET['pic']));
  }
  
  $deleteSQL = sprintf("DELETE FROM demo_sponsor WHERE userid=%s && id=%s",

var handleDataTableDefault = function() {
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
            ...
            "autoWidth": false,
            "ajax": {
                "url": "sqldatatable/album_view.php",
                "type": "post",
				"data": function ( d ) {
            ...
            "fnDrawCallback": function(oSettings) {
                ...
                $('#data-table-default').editable({
                    selector: ".ed_title",
                    url: 'sqledit/albumalbum_jedit.php',
                    type: 'text',
                    name: 'title',
                    //title: '輸入標題',
				...
				$('#data-table-default').editable({
                    ...
                    url: 'sqledit/albumalbum_jedit.php',
                    name: "type",
                    source: "sqledit/albumalbum_get_list.php?list_id=1",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/album_del.php?id_del=" + id,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/album_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

...
class pureFileUpload
{
	...
	function createTimeUniqName($fileName,$field,$key) {
		$this->debug("<b>建立一個唯一的名稱</b><br/>");
		$dname = date("YmdHis");
		$uniq = 0;
		...
		echo "目前key值=" .$key;
		echo "<br>";
		echo "圖片名稱(前)=" . $_FILES[$field]['name'][$key];
		echo "<br>";
		echo "圖片名稱(後)=" . $dname.$uniq.'.'.$extension;
		echo "<br>";*/
		
		if(is_array($_FILES[$field]['name'])){
			//echo "ARRAY狀態" . "<br>";
			$_FILES[$field]['name'][$key] = $dname.$uniq.'.'.$extension;
			$_POST[$field][$key] = $_FILES[$field]['name'][$key];
		}else{
			//echo "NO ARRAY狀態" . "<br>";
			$_FILES[$field]['name'] = $dname.$uniq.'.'.$extension;
		...
		while (++$uniq) {
			...
			if (!file_exists($this->path.'/'.$dname.$uniq.'.'.$extension) && $name != "") {
				...
				//if($name != ""){
					//usleep(50000);
					/*echo "<br>";
					echo "(loop)目前key值=" .$key;
					echo "<br>";
					echo "(loop)圖片名稱(前)=" . $_FILES[$field]['name'][$key];
					echo "<br>";
					echo "(loop)圖片名稱(後)=" . $dname.$uniq.'.'.$extension;
					echo "<br>";*/
					if(is_array($_FILES[$field]['name'])){
						$_FILES[$field]['name'][$key] = $dname.$uniq.'.'.$extension;
						$_POST[$field][$key] = $_FILES[$field]['name'][$key];
	...
	function doUpload() {
		...
		$this->debug("PHP version(".phpversion().")");
		...
		foreach ($_FILES as $field => $value) {
			...
			for ($k=0; $k<@count($_FILES[$field]['name']); $k++) {
			
			if(is_array($_FILES[$field]['name'])){
				$P_State = "array";
				$P_UploadFileName = $_FILES[$field]['name'][$k];
				$P_UploadFileSize = $_FILES[$field]['size'][$k];
				$P_UploadFileTmpName = $_FILES[$field]['tmp_name'][$k];
			}else{
				$P_State = "string";
				$P_UploadFileName = $_FILES[$field]['name'];
				$P_UploadFileSize = $_FILES[$field]['size'];
				$P_UploadFileTmpName = $_FILES[$field]['tmp_name'];
			}
			...
			$file->field = $field;
			$file->setFileName($P_UploadFileName);
			...
			if (is_uploaded_file($P_UploadFileTmpName)) {
				...
				if (file_exists($this->path.'/'.$file->fileName)) {
					...
					case 'over':
						...
						$this->moveFile($P_UploadFileTmpName, $this->path.'/'.$file->fileName);
						break;
					// Give error message
					case 'error':
						$this->debug("錯誤<br/>");
					...
					// Make an unique name
					case 'uniq':
						$file->setFileName($this->createUniqName($file->fileName,$field,$key));
						$this->moveFile($P_UploadFileTmpName, $this->path.'/'.$file->fileName);
						break;
					// 使用時間函式建立唯一函式
					case 'timeuniq':
						$file->setFileName($this->createTimeUniqName($file->fileName,$field,$key));
						$this->moveFile($P_UploadFileTmpName, $this->path.'/'.$file->fileName);
						break;
					}
				} else {
				...
				if(is_array($_FILES[$field]['name'])){
					$this->uploadedFiles[$field.$k] = $file;
				}else{
...
class fileInfo
{
	...
	function setFileName($newFileName) {
		...
		if ($this->upload->storeType == 'path') {
			...
			//echo $_POST[$this->field] = $this->fileName;
		}
		
		if(is_array($_FILES[$this->field]['name'])){
			//for ($k=0; $k<count($_FILES); $k++) {
			for ($k=0; $k<count($_FILES[$this->field]['name']); $k++) {
				$this->upload->uploadedFiles[$this->field . $k] = $this;
			}
		}else{

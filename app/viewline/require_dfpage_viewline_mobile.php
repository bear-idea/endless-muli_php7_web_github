...
<ol class="breadcrumb hidden-xs" data-scroll-reveal='enter top after 0.8s'>
  ...
  <li class="current"><a href="<?php echo $SiteBaseUrl . url_rewrite('dfpage',array('wshop'=>$_GET['wshop'],'lang'=>$_SESSION['lang'],'Opt'=>'subpage','aid'=>$_GET['aid'],'type1'=>$_GET['type1']),'',$UrlWriteEnable);?>">
  <?php do {  //比較字串 ?>
  ...
  <li class="current"><a><?php echo $row_RecorddfpageKeyWord['title']; ?></a></li>
  <?php } else if($_GET['Opt'] == 'search'){  ?>
  <li class="current"><a><?php echo $Lang_Dfpage_Search; //文章搜尋 ?></a></li>
  <?php } else {  ?>
  <li class="current"><a><?php if ((isset($_GET['id'])) && ($_GET['id'] != "")) { echo $row_RecorddfpageKeyWord['title'];}else if(isset($_GET['type1']) && $_GET['type1'] != ""){echo $row_RecorddfpageKeyWordType['title'];}else{echo $row_RecorddfpageKeyWordHome['title'];} ?></a></li>
  <?php }  ?>
</ol>

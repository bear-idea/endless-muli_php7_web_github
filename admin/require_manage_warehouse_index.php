...
/* 取得發佈者資料 */
$colname_RecordWarehouseListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordWarehouseListAuthor = $_GET['lang'];
}
$coluserid_RecordWarehouseListAuthor = "-1";
if (isset($w_userid)) {
  $coluserid_RecordWarehouseListAuthor = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordWarehouseListAuthor = sprintf("SELECT * FROM erp_warehouseitem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordWarehouseListAuthor, "text"),GetSQLValueString($coluserid_RecordWarehouseListAuthor, "int"));
$RecordWarehouseListAuthor = mysqli_query($DB_Conn, $query_RecordWarehouseListAuthor) or die(mysqli_error($DB_Conn)); 
$row_RecordWarehouseListAuthor = mysqli_fetch_assoc($RecordWarehouseListAuthor);
$totalRows_RecordWarehouseListAuthor = mysqli_num_rows($RecordWarehouseListAuthor);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/warehouse_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 倉庫管理 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
do {  
?>
                  <option value="<?php echo $row_RecordWarehouseListType['itemname']?>"><?php echo $row_RecordWarehouseListType['itemname']?></option>
                  <?php
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      <thead>
        <tr>
          ...
          <th width="70" data-priority="1"><strong>代碼</strong></th>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_warehouse.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_warehouse.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',
...
mysqli_free_result($RecordWarehouseListAuthor);
?>

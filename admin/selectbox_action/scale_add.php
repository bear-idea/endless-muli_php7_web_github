...
$query_RecordScaleListItem = sprintf("SELECT item_id, itemname FROM erp_scaleitem WHERE subitem_id = %s && level = %s ORDER BY sortid ASC, item_id DESC", GetSQLValueString($colitem_id_RecordScaleListItem, "int"),GetSQLValueString($collevel_RecordScaleListItem, "int"));

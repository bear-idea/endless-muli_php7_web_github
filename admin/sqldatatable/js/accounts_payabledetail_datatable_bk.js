var handleDataTableDefault = function(aid) {
    ...
    if ($('#data-table-default').length !== 0) {
        var otable = $('#data-table-default').DataTable({       //"dom":"lBfrtip",buttons:[{extend:"copy",className:"btn-sm"},{extend:"csv",className:"btn-sm"},{extend:"excel",className:"btn-sm"},{extend:"pdf",className:"btn-sm"},{extend:"print",className:"btn-sm"},{extend:"colvis",className:"btn-sm"}],
            ...
            "autoWidth": false,
            "ajax": {
                "url": "sqldatatable/accounts_payabledetail_view.php",
                "type": "post",
				"data": function ( d ) {
					...
					d.search_suppliercode = $('#col2_filter').val();
					d.search_company = $('#col3_filter').val();
					d.search_postdate = $('#col8_filter').val();
			...
			"stateSaveParams": function (settings, data) {
				...
				data.search.field = {
					...
					suppliercode: $("#col2_filter").val(),
					company: $("#col3_filter").val(),
					postdate: $("#col8_filter").val(),
			...
			"stateLoadParams": function (settings, data) {
				...
				$("#col2_filter").val(data.search.field.suppliercode);
				
				$("#col3_filter").val(data.search.field.company);
				
				$("#col8_filter").val(data.search.field.postdate);
            ...
            "fnDrawCallback": function(oSettings) {
                ...
                $('#data-table-default').editable({
                    selector: ".ed_supplier",
                    url: 'sqledit/accounts_payabledetail_jedit.php',
                    type: 'text',
                    name: 'supplier',
                    //title: '輸入標題',
				...
				$('#data-table-default').editable({
                    ...
                    name: 'amountr',
                    //title: '輸入排序',
                    tpl: "<input type='number' style='width: 120px' maxlength='200'>",
                    select2: {
                        width: '100%',
                        // THIS DOESN'T WORK AS IT SHOULD
                        // hiding search box
				...
				$('#data-table-default').editable({
                    ...
                    url: 'sqledit/accounts_payabledetail_jedit.php',
                    name: "type",
                    source: "sqledit/accounts_payabledetail_get_list.php?list_id=1",
                    validate: function(value) {
                        if ($.trim(value) == '') {
                            return '值不能為空';
                        }
    ...
    swal({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                    url: "sqldatatable/accounts_payabledetail_del.php?id_del=" + id,
                    type: "GET"
        ...
        swal({
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    ...
                    $.ajax({
                        url: "sqldatatable/accounts_payabledetail_del_muti.php",
                        data: {
                            id: list_id
                        },
                        type: "POST"

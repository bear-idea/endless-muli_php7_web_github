...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_KnowledgeItemAdd")) {
  $insertSQL = sprintf("INSERT INTO demo_knowledgeitem (list_id, itemname, subitem_id, lang, level, userid) VALUES (%s, %s, %s, %s, %s, %s)",
  ...
  // 更新父節點 目前新增為子節點
  $updateSQL2 = sprintf("UPDATE demo_knowledgeitem SET endnode='parent' WHERE item_id=%s",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_KnowledgeItemEdit")) {
	foreach($_POST['item_id'] as $key => $val){
	  $updateSQL = sprintf("UPDATE demo_knowledgeitem SET list_id=%s, sortid=%s, indicate=%s, itemname=%s, lang=%s, level=%s WHERE item_id=%s",
...
if ((isset($_POST['deltype'])) && ($_POST['deltype'] != "")) {
	...
	$MM_flag="MM_update";
		...
		foreach($_POST['deltype'] as $key => $val){
		  ...
		  {
			  	case "1":
		  		$MM_dupKeyRedirect="manage_knowledge.php?wshop=" . $wshop . "&Opt=sub_mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&item_id=" . $_GET['item_id'] . "&level=" . $_GET['level'] . "&Operate=delErrorP";
				break;
				case "2":
		  		$MM_dupKeyRedirect="manage_knowledge.php?wshop=" . $wshop . "&Opt=sub_mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&item_id=" . $_GET['item_id'] . "&subitem_id=" . $_GET['subitem_id'] . "&level=" . $_GET['level'] . "&Operate=delErrorP";
				break;
				default:
				break;
		  }
		  ...
		  {
			  	case "1":
		  		$LoginRS__query = sprintf("SELECT * FROM demo_knowledge WHERE type2=%s", GetSQLValueString($loginUsername, "int")); // 分類
				break;
				case "2":
		  		$LoginRS__query = sprintf("SELECT * FROM demo_knowledge WHERE type3=%s", GetSQLValueString($loginUsername, "int")); // 分類
				break;
				default:
				break;
		  }
	...
	if (isset($_POST[$MM_flag])) {
		...
		foreach($_POST['deltype'] as $key => $val){
		  ...
		  {
			  	case "1":
		  		$MM_dupKeyRedirect="manage_knowledge.php?wshop=" . $wshop . "&Opt=sub_mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&item_id=" . $_GET['item_id'] . "&level=" . $_GET['level'] . "&Operate=delErrorT";
				break;
				case "2":
		  		$MM_dupKeyRedirect="manage_knowledge.php?wshop=" . $wshop . "&Opt=sub_mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&item_id=" . $_GET['item_id'] . "&subitem_id=" . $_GET['subitem_id'] . "&level=" . $_GET['level'] . "&Operate=delErrorT";
				break;
				default:
				break;
		  }
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordDateCount = sprintf("SELECT * FROM demo_knowledgeitem WHERE subitem_id = %s && userid=%s", GetSQLValueString($colname_RecordDateCount, "int"), GetSQLValueString($coluserid_RecordDateCount, "int"));
...
$totalRows_RecordDateCount = mysqli_num_rows($RecordDateCount);
  ...
  {
	// 更新父節點為根節點
  	$updateSQL3 = sprintf("UPDATE demo_knowledgeitem SET endnode='child' WHERE item_id=%s",
                       GetSQLValueString($_GET['item_id'], "int"));
...
$colitemid_RecordKnowledgeListItem = "-1";
if (isset($_GET['item_id'])) {
  $colitemid_RecordKnowledgeListItem = $_GET['item_id'];
}
$collevel_RecordKnowledgeListItem = "0";
if (isset($_GET['level'])) {
  $collevel_RecordKnowledgeListItem = $_GET['level'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordKnowledgeListItem = sprintf("SELECT * FROM demo_knowledgeitem WHERE list_id = %s && lang=%s && level = %s && subitem_id = %s && userid=%s ORDER BY sortid ASC, item_id DESC", GetSQLValueString($collistid_RecordKnowledgeListItem, "int"),GetSQLValueString($collang_RecordKnowledgeListItem, "text"),GetSQLValueString($collevel_RecordKnowledgeListItem, "int"),GetSQLValueString($colitemid_RecordKnowledgeListItem, "int"),GetSQLValueString($coluserid_RecordKnowledgeListItem, "int"));
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <?php if($_GET['level'] == '1'){ ?>
    <div class="btn-group pull-right"><a href="manage_knowledge.php?wshop=<?php echo $wshop;?>&amp;Opt=mulilistitempage&amp;lang=<?php echo $_SESSION['lang']; ?>&list_id=<?php echo $_GET['list_id'] ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
    <?php } ?>
    <?php if($_GET['level'] == '2'){ ?>
    <div class="btn-group pull-right"><a href="manage_knowledge.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage&amp;lang=<?php echo $_SESSION['lang']; ?>&item_id=<?php echo $_GET['subitem_id'] ?>&level=1&list_id=<?php echo $_GET['list_id'] ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
    <?php } ?>
  ...
  <div class="panel-body">
    ...
    <form id="form_KnowledgeItemEdit" name="form_KnowledgeItemEdit" method="POST" action="manage_knowledge.php?wshop=<?php echo $wshop;?>&amp;Opt=<?php echo $_GET['Opt'] ?>&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;list_id=<?php echo $_GET['list_id']; ?>&amp;item_id=<?php echo $_GET['item_id'] ?>&amp;subitem_id=<?php echo $_GET['subitem_id'] ?>&amp;level=<?php echo $_GET['level'] ?>" class="form-horizontal form-bordered" data-parsley-validate="">
    <?php do { ?>
          ...
          <div class="card-block">
              ...
              <div class="form-group row">
                  <!--<div class="col-md-6" style="border:0">-->
                      <div class="input-group p-0">
                       <?php if ($_GET['level'] == '') { ?>
                      <a class="btn btn-info btn-block" href="manage_knowledge.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;item_id=<?php echo $_GET['item_id']; ?>&amp;level=1&amp;list_id=<?php echo $_GET['list_id']; ?>" data-original-title="點選查看下層的分類項目" data-toggle="tooltip" data-placement="top">子分類 <i class="fa fa-chevron-circle-right"></i><?php require("require_manage_knowledge_mulilistitem_count.php"); ?></a>  
                       <?php } ?> 
                       <?php if ($_GET['level'] == '1') { ?>
                      <a class="btn btn-info btn-block" href="manage_knowledge.php?wshop=<?php echo $wshop;?>&amp;Opt=sub_mulilistitempage&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;item_id=<?php echo $row_RecordKnowledgeListItem['item_id']; ?>&amp;subitem_id=<?php echo $row_RecordKnowledgeListItem['subitem_id']; ?>&amp;level=2&amp;list_id=<?php echo $_GET['list_id']; ?>" data-original-title="點選查看下層的分類項目" data-toggle="tooltip" data-placement="top">子分類 <i class="fa fa-chevron-circle-right"></i><?php require("require_manage_knowledge_mulilistitem_count.php"); ?></a>  
                       <?php } ?>         
                      </div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <!-- begin panel-body -->
  <div class="panel-body">
      <form id="form_KnowledgeItemAdd" name="form_KnowledgeItemAdd" method="POST" action="manage_knowledge.php?wshop=<?php echo $wshop;?>&amp;Opt=<?php echo $_GET['Opt'] ?>&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;list_id=<?php echo $_GET['list_id']; ?>&amp;item_id=<?php echo $_GET['item_id'] ?>&amp;level=<?php echo $_GET['level'] ?>" class="form-horizontal form-bordered" data-parsley-validate="">
      <div class="card bg-aqua-transparent-1">
          <div class="card-block">
              <div class="row">

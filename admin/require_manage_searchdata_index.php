...
/* 取得發佈者資料 */
$colname_RecordSearchdataListAuthor = "zh-tw";
if (isset($_GET['lang'])) {
  $colname_RecordSearchdataListAuthor = $_GET['lang'];
}
$coluserid_RecordSearchdataListAuthor = "-1";
if (isset($w_userid)) {
  $coluserid_RecordSearchdataListAuthor = $w_userid;
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordSearchdataListAuthor = sprintf("SELECT * FROM mail_searchdataitem WHERE list_id = 2 && lang=%s && userid=%s", GetSQLValueString($colname_RecordSearchdataListAuthor, "text"),GetSQLValueString($coluserid_RecordSearchdataListAuthor, "int"));
$RecordSearchdataListAuthor = mysqli_query($DB_Conn, $query_RecordSearchdataListAuthor) or die(mysqli_error($DB_Conn)); 
$row_RecordSearchdataListAuthor = mysqli_fetch_assoc($RecordSearchdataListAuthor);
$totalRows_RecordSearchdataListAuthor = mysqli_num_rows($RecordSearchdataListAuthor);
...
<script src="<?php echo $SiteBaseUrl . $SiteBaseAdminPath; ?>sqldatatable/js/searchdata_datatable.js?<?php echo time(); ?>"></script>
<!-- ================== END Datatables JS ================== -->
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> Mail搜尋 <small>總覽</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-body">
    ...
    <div class="alert alert-warning m-b-15 m-t-0"><i class="fa fa-info-circle"></i> <b>修改次分類設定中的類別，即可修改所有當次搜尋的所有分類。</b></div>
...
do {  
?>
                  <option value="<?php echo $row_RecordSearchdataListType['item_id']?>"><?php echo $row_RecordSearchdataListType['itemname']?></option>
                  <?php
...
?>
      ...
      <div class="col-md-2 col-sm-12 m-b-10">
        <div class="input-group" data-column="5"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 狀態</span></span>
            <select name="indicate" class="form-control search_filter" id="col5_filter">
      ...
      <div class="col-md-2 col-sm-12 m-b-10">
        <div class="input-group" data-column="6"> <span class="input-group-prepend"><span class="input-group-text"><i class="fas fa-search fa-fw"></i> 資料模式</span></span>
            <select name="mode" class="form-control search_filter" id="col6_filter">
              <option value="">全部</option>
              <option value="0">搜尋 E-mail</option>
              <option value="1">自傳 E-mail</option>
            </select>
        </div>
      </div>
    ...
    <table id="data-table-default" class="table table-striped table-bordered table-hover table-condensed">
      ...
      <tfoot>
        <tr>
          <td>└</td>
          <td><button type="button" class="btn btn-default btn-sm" onclick="delete_muti_datatables(event);"><i class="fa fa-trash-alt"></i> 刪除選取項目</button><button type="button" class="btn btn-default btn-sm" onclick="add_muti_datatables_blacklist(event);"><i class="fa fa-plus"></i> 加入選取黑名單</button></td>
          <td>&nbsp;</td> 
          <td>&nbsp;</td> 
          <td>&nbsp;</td> 
          <td>&nbsp;</td>
        ...
        var intro = introJs();
          intro.setOptions({
            steps: [
			  {
                element: '#Step_List',
                intro: '您可以點選按鈕設置清單或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_searchdata.php?wshop=<?php echo $wshop;?>&Opt=listpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往設置清單</a></span></div>'
              },
			  {
                element: '#Step_Add',
                intro: '您可以點選按鈕新增資料或點選下一步繼續導覽。<div style="text-align:center;margin-top:10px;"><span class = "InnerPage" style="float:none"><a href="manage_searchdata.php?wshop=<?php echo $wshop;?>&Opt=addpage&lang=<?php echo $_SESSION['lang']; ?>" target="_blank"><i class="fa fa-arrow-circle-right"></i> 前往新增資料</a></span></div>'
              },
              {
                element: '#Step_View',
...
mysqli_free_result($RecordSearchdataListAuthor);
?>

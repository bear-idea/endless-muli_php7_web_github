...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Knowledge")) {
  $updateSQL = sprintf("UPDATE demo_knowledge SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/knowledge/' . $_POST['oldpic']);
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/knowledge/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_knowledge.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordKnowledge = sprintf("SELECT * FROM demo_knowledge WHERE id = %s", GetSQLValueString($colname_RecordKnowledge, "int"));
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordKnowledge['pic']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Knowledge" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/knowledge/thumb/small_<?php echo $row_RecordKnowledge['pic']; ?>"],
		<?php } ?>

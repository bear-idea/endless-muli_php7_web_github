...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordScaleorder = sprintf("SELECT * FROM erp_scaleorderout WHERE userid=%s && (arrivals IS NULL || indicate=0)",GetSQLValueString($coluserid_RecordScaleorder, "int"));
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      ...
      <div class="form-group row">
        <label class="col-md-2 col-form-label">出貨單號<span class="text-red">*</span></label>
          <div class="col-md-10">
                 
                    <select name="oserial" id="oserial" class="form-control" data-parsley-trigger="blur" required="">
                <?php
				do {  
				?>
								<?php //if($row_RecordScaleorder['arrivals'] == "") { ?>
								<option value="<?php echo $row_RecordScaleorder['oserial']?>"><?php echo $row_RecordScaleorder['oserial']?> / <?php echo $row_RecordScaleorder['driver']; ?> / <?php echo $row_RecordScaleorder['carnumber']; ?> / <?php echo $row_RecordScaleorder['warehouse']; ?> / <?php $dt = new DateTime($row_RecordScaleorder['postdate']); echo $dt->format('Y-m-d g:i A'); ?></option>
                                <?php //} ?>
                                
								<?php
				} while ($row_RecordScaleorder = mysqli_fetch_assoc($RecordScaleorder));
				  $rows = mysqli_num_rows($RecordScaleorder);
				  if($rows > 0) {
					  mysqli_data_seek($RecordScaleorder, 0);
					  $row_RecordScaleorder = mysqli_fetch_assoc($RecordScaleorder);
				  }
				?>
				    </select>

...
$initialPreview = !empty($row_RecordNews['metas']['ogimage']) ? BASEURL . "/site/" . $_SESSION['wshop'] . "/image/seo/" . $row_RecordNews['metas']['ogimage'] : '';
...
$formFields_SEO = [
    ...
    'ogimage' => ['label' => '預覽圖(og:image)', 'type' => 'file', 'maxlength' => 200, 'required' => false, 'initialPreview' => $initialPreview, 'value' => $row_RecordNews['metas']['ogimage'], 'tooltip' => 'og:image 用戶將內容分享至社群時顯示的圖像網址。 (網頁的預覽圖)', 'errorContainer' => 'error_ogImage'],
...
// 載入圖片資訊
if (!empty($row_RecordNews['photos'])) {
    $initialPreview = [];
    $initialPreviewConfig = [];
    foreach ($row_RecordNews['photos'] as $photo) {
...
$formFields_Photo = [
    ...
    //'photodescription' => ['label' => '描述', 'type' => 'text', 'maxlength' => 150, 'required' => false, 'value' => $row_RecordNews['photos']['photodescription'], 'tooltip' => '']
];

...
$colpid_RecordNewsPost = "-1";
if (isset($_GET['id'])) {
  $colpid_RecordNewsPost = $_GET['id'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordNewsPost = sprintf("SELECT demo_newspost.id, demo_newspost.pid, demo_newspost.userid, demo_newspost.content, demo_newspost.author, demo_newspost.postdate , demo_newsreply.rid FROM demo_newspost LEFT OUTER JOIN demo_newsreply ON demo_newspost.id = demo_newsreply.pid GROUP BY demo_newspost.id HAVING (demo_newspost.pid = %s) && ((demo_newspost.postdate LIKE %s) || (demo_newspost.author LIKE %s)) && demo_newspost.userid=%s ORDER BY demo_newspost.id DESC", GetSQLValueString($colpid_RecordNewsPost, "int"),GetSQLValueString("%" . $colname_RecordNewsPost . "%", "text"),GetSQLValueString("%" . $colname_RecordNewsPost . "%", "text"),GetSQLValueString($coluserid_RecordNewsPost, "int"));
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0"  class="TB_General_style00">
      <tr>
        ...
        <td width="50%" align="right">
          ...
          <?php if ($ManageNewsPostSearchSelect == "1") { // 搜索功能開啟設定?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablesorter" id="TBSort">
        ...
        <?php
		 ...
		 ?>
            ...
            <td valign="middle">
            <span class="ed_content" id="content_<?php echo  $row_RecordNewsPost['id'];?>"><?php echo $row_RecordNewsPost['content']; ?></span>             
              </td>
            ...
            <a href="manage_news.php?wshop=<?php echo $wshop;?>&amp;Opt=replyaddpage&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;post_id=<?php echo $row_RecordNewsPost['id']; ?>&amp;pd_id=<?php echo $_GET['id']; ?>&amp;pdname=<?php echo $_GET['pdname']; ?>">[新增回應]</a>
            </span><?php } else {?>
            <span class = "InnerPage">
            <a href="manage_news.php?wshop=<?php echo $wshop;?>&amp;Opt=replypage&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;post_id=<?php echo $row_RecordNewsPost['id']; ?>&amp;pd_id=<?php echo $_GET['id']; ?>&amp;pdname=<?php echo $_GET['pdname']; ?>">[回應一覽]</a>
            </span><?php } ?></td>

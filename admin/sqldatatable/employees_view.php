...
$coltype_RecordEmployees = "%";
if (isset($search_type) && $search_type != "") {
  $coltype_RecordEmployees = $search_type;
}
...
$query_RecordCount = sprintf("SELECT count(id) as sum FROM demo_employees WHERE lang = %s && userid=%s",GetSQLValueString($collang_RecordEmployees, "text"),GetSQLValueString($coluserid_RecordEmployees, "int"));
$RecordCount = mysqli_query($DB_Conn, $query_RecordCount) or die(mysqli_error($DB_Conn));
$row_RecordCount = mysqli_fetch_assoc($RecordCount);


$query_RecordEmployees = sprintf("SELECT * FROM demo_employees WHERE name LIKE %s && indicate LIKE %s && type LIKE %s && lang = %s && userid=%s $orderSql", GetSQLValueString("%" . $colsearch_RecordEmployees . "%", "text"), GetSQLValueString($colindicate_RecordEmployees, "text"), GetSQLValueString($coltype_RecordEmployees, "text"),GetSQLValueString($collang_RecordEmployees, "text"),GetSQLValueString($coluserid_RecordEmployees, "int"));
...
<?php do { ?>
  <?php 
    ...
    $link_add = "manage_employees.php?wshop=".$wshop."&amp;Opt=addpage&amp;lang=".$_SESSION['lang'];
	$link_copy = "manage_employees.php?wshop=".$wshop."&amp;Opt=copypage&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordEmployees['id'];
	$link_edit = "manage_employees.php?wshop=".$wshop."&amp;Opt=editpage&amp;lang=".$_SESSION['lang']."&amp;id_edit=".$row_RecordEmployees['id'];
	//$link_start = "manage_employees.php?wshop=".$wshop."&amp;Opt=startpage&amp;lang=".$_SESSION['lang'];
	$link_worktime = "inner_employees.php?wshop=".$wshop."&amp;Opt=worktimeviewpage&amp;lang=".$_SESSION['lang']."&amp;aid=".$row_RecordEmployees['id'];
	$link_hoilday = "inner_employees.php?wshop=".$wshop."&amp;Opt=hoildayviewpage&amp;lang=".$_SESSION['lang']."&amp;aid=".$row_RecordEmployees['id'];
	...
	$but_worktime = "<a href='".$link_worktime."' class='btn btn-xs btn-primary colorbox_iframe_cd' style=text-align:center'><i class='fa fa-clock'></i> 基本排班</a>";
	$but_hoilday = "<a href='".$link_hoilday."' class='btn btn-xs btn-primary colorbox_iframe_cd' style=text-align:center'><i class='fa fa-calendar-alt'></i> 休假管理</a>";
	...
	$dvalue['name'] = "<span id='name_".$row_RecordEmployees["id"]."' class='ed_name' data-type='text' data-pk='".$row_RecordEmployees["id"]."' data-placement='top'>".$row_RecordEmployees["name"]."</span>";
	//if($row_RecordEmployees['pushtop'] == 1){$dvalue['title'] = $dvalue['title']."<span class='label label-warning pull-right'><i class='fa fa-check-circle'></i> 置頂<span>";}
	
	$dvalue['type'] = "<span id='type_".$row_RecordEmployees["id"]."' class='ed_type' data-type='select' data-pk='".$row_RecordEmployees["id"]."' data-placement='top'>".$row_RecordEmployees["type"]."</span>";
	...
	if($row_RecordEmployees["indicate"] == '1') {$row_RecordEmployees["indicate"] = "公佈";}else{$row_RecordEmployees["indicate"] = "隱藏";}
	...
	$dt = new DateTime($row_RecordEmployees['postdate']); 
	$dvalue['postdate'] = "<span id='postdate_".$row_RecordEmployees["id"]."' class='ed_postdate' data-type='date' data-format='yyyy-mm-dd' data-pk='".$row_RecordEmployees["id"]."' data-placement='top'>".$dt->format('Y-m-d')."</span>";
	
	$dvalue['action'] = "<div class='btn-group'>".$but_add.$but_edit.$but_del."</div>";
	$dvalue['action'] .= "<div class='btn-group'>".$but_worktime.$but_hoilday."</div>";

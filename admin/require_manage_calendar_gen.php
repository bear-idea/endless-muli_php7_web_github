...
$query_RecordCalendar = sprintf("SELECT * FROM salary_calendar WHERE userid=%s",GetSQLValueString($coluserid_RecordCalendar, "int"));
...
if ((isset($_POST["MM_del"])) && ($_POST["MM_del"] == "form_Calendar") && $totalRows_RecordCalendar > 0 ) {
  $deleteSQL = sprintf("DELETE FROM salary_calendar WHERE userid=%s",
                       GetSQLValueString($_POST['userid'], "int"));

  //mysqli_select_db($database_DB_Conn, $DB_Conn);
  $Result1 = mysqli_query($DB_Conn, $deleteSQL) or die(mysqli_error($DB_Conn));
  
  $insertGoTo = "manage_calendar.php?Opt=genpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
...
if (isset($_SESSION['lang'])) {
  $colname_RecordHoilday = $_SESSION['lang'];
}
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordHoilday = sprintf("SELECT * FROM salary_hoilday WHERE lang = %s && userid=%s", GetSQLValueString($colname_RecordHoilday, "int"),GetSQLValueString($coluserid_RecordHoilday, "int"));
...
if($totalRows_RecordHoilday > 0) {
	do{
		$hoilday['hdate'][] = $row_RecordHoilday['hdate'];
		$hoilday['notes1'][] = $row_RecordHoilday['title'];
		$hoilday['indicate'][] = $row_RecordHoilday['indicate'];
	} while ($row_RecordHoilday = mysqli_fetch_assoc($RecordHoilday));
}

if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_Calendar") && $totalRows_RecordCalendar == 0 ) {
  $ThisYear = $_POST['hyear'];
  $weeklist = array('日', '一', '二', '三', '四', '五', '六');
  
  for($i=1; $i<=12; $i++){
	  
	  if($i<10){
      	  $dt = new DateTime($ThisYear."-0".$i);
	  }else{
		  $dt = new DateTime($ThisYear."-".$i);
	  }
      $dt->modify('last day of this month');
      $last_day = $dt->format('d');
  
	  for($j=1; $j<=$last_day; $j++){
		  
		  if($i<10){
			  if($j<10){
				  $g_date = $ThisYear."-0".$i."-0".$j;
			  }else{
				  $g_date = $ThisYear."-0".$i."-".$j;
			  }
		  }else{
			  if($j<10){
				  $g_date = $ThisYear."-".$i."-0".$j;
			  }else{
				  $g_date = $ThisYear."-".$i."-".$j;
			  }
		  } 
		  
		  $dt = new DateTime($g_date);
		  $this_week = $dt->format('w');
		  
		  if($_POST['hoildayindicate'] == 1)
		  {
			  if($i<10){
				  if($j<10){
					  $key = array_search("0".$i."-0".$j,$hoilday['hdate']);
				  }else{
					  $key = array_search("0".$i."-".$j,$hoilday['hdate']);
				  }
			  }else{
				  if($j<10){
					  $key = array_search($i."-0".$j,$hoilday['hdate']);
				  }else{
					  $key = array_search($i."-".$j,$hoilday['hdate']);
				  }
			  }  
		  }
		  ...
		  if($_POST['hoildaytype'] == '0'){
			  $indicate = 0;
		  }else if($_POST['hoildaytype'] == '1'){
			  if($this_week == "0"){
				  $indicate = 1;
			  }
		  }else if($_POST['hoildaytype'] == '2'){
			  if($this_week == "0" || $this_week == "6"){
				  $indicate = 1;
			  }
		  }else if($_POST['hoildaytype'] == '3'){
			  if($this_week == "0"){
				  $indicate = 1;
			  }
			  if($this_week == "6"){
				  $dt = new DateTime($g_date);
		          $which_week = $dt->format('W'); /* 第幾周 */
				  if($which_week%2==0)
				  {
					  $indicate = 1;
				  }
			  }
		  }else if($_POST['hoildaytype'] == '4'){
			  if($this_week == "0"){
				  $indicate = 1;
			  }
			   if($this_week == "6"){
		  ...
		  if($key != ""){
			  $indicate = $hoilday['indicate'][$key];
			  $notes1 = $hoilday['notes1'][$key];
		  }
			
		  $insertSQL = sprintf("INSERT INTO salary_calendar (weekday, hyear, hdate, indicate, notes1, lang, userid) VALUES (%s, %s, %s, %s, %s, %s, %s)",
  ...
  $insertGoTo = "manage_calendar.php?Opt=viewpage&lang=" . $_POST['lang'];
  /*if (isset($_SERVER['QUERY_STRING'])) {
    $insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
    $insertGoTo .= $_SERVER['QUERY_STRING'];
  }*/
...
<div class="card bg-silver-lighter" style="overflow:hidden">
  <div class="card-block">
    <h4 class="card-title m-0"><i class="far fa-bookmark"></i> 年度行事 <small>產生</small> <?php require("require_lang_show.php"); ?></h4>
  </div>
</div>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <?php if($totalRows_RecordCalendar == 0 && $totalRows_RecordHoilday > 0) { ?>
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      <?php 
	 	$dt = new DateTime(); $ThisYear = $dt->format('Y');
	  ?>
      
      <div class="alert alert-warning m-5"><i class="fa fa-info-circle"></i> <b>因每年日期及行事會有所變動，故此項目需每年重新產生建立。</b></div>
        ...
        <label class="col-md-2 col-form-label">年份<span class="text-red">*</span></label>
          <div class="col-md-10">
                    ...
                    <select name="hyear" id="hyear" class="form-control" data-parsley-trigger="blur" required="">
                        <?php for($i=$ThisYear-1; $i <=$ThisYear+1; $i++) { ?>
      ...
      <input type="hidden" name="MM_insert" value="form_Calendar" />
  </form>
  <?php } else if($totalRows_RecordCalendar == 0 && $totalRows_RecordHoilday == 0) { ?>
  <div class="alert alert-danger m-10"><i class="fa fa-info-circle"></i> <b>目前尚未設定例假維護，請先前往設定。<a href="manage_hoilday.php?wshop=<?php echo $wshop;?>&amp;Opt=viewpage&amp;lang=<?php echo $_SESSION['lang']; ?>" class="btn btn-primary btn-xs" data-original-title="前往更改設定" data-toggle="tooltip" data-placement="top"><i class="fa fa-chevron-right"></i> 前往設定</a></b></div>	
  <?php } ?>
  ...
  <form action="<?php echo $editFormAction;?>"  class="form-horizontal form-bordered" data-parsley-validate="" method="post">
      ...
      <input type="hidden" name="MM_del" value="form_Calendar" />
  </form>
  <?php } ?>

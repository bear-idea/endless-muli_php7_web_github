...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/bannershow";
	$ppu->extensions = "GIF,JPG,JPEG,BMP,PNG";
	$ppu->formName = "form_Bannershow";
	$ppu->storeType = "file";
	$ppu->sizeLimit = "3000";
...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
  ...
  $sip->quality = "100";
  $sip->makeThumb = "true";
  $sip->pathThumb = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/bannershow/thumb";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Bannershow")) {
  $updateSQL = sprintf("UPDATE demo_bannershow SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/bannershow/' . $_POST['oldpic']);
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/bannershow/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_bannershow.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_RecordBannershow = "-1";
if (isset($_GET['id_edit'])) {
  $colname_RecordBannershow = $_GET['id_edit'];
}
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordBannershow = sprintf("SELECT * FROM demo_bannershow WHERE id = %s", GetSQLValueString($colname_RecordBannershow, "int"));
$RecordBannershow = mysqli_query($DB_Conn, $query_RecordBannershow) or die(mysqli_error($DB_Conn));
$row_RecordBannershow = mysqli_fetch_assoc($RecordBannershow);
$totalRows_RecordBannershow = mysqli_num_rows($RecordBannershow);
?>
<!DOCTYPE html>
<html lang="zh-TW">
<!--<![endif]-->
  ...
  <form action="<?php echo $editFormAction;?>" class="form-horizontal form-bordered" data-parsley-validate="" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group row">
          ...
          <div class="col-md-10">
            ...
            <input name="id" type="hidden" id="id" value="<?php echo $row_RecordBannershow['id']; ?>" />
            <input name="oldpic" type="hidden" id="oldpic" value="<?php echo $row_RecordBannershow['pic']; ?>" />
            <input name="wshop" type="hidden" id="wshop" value="<?php echo $wshop; ?>" />
            <input name="GP_upload" type="hidden" id="GP_upload" value="true" />
          </div>
      </div>
      <input type="hidden" name="MM_update" value="form_Bannershow" />
  </form>
  </div>
  <!-- end panel-body --> 
</div>
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		maxFileSize: 3000,  
		<?php if ($row_RecordBannershow['pic'] != "") { ?>
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/bannershow/thumb/small_<?php echo $row_RecordBannershow['pic']; ?>"],
		<?php } ?>

...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_CommodityItemAdd")) {
  $insertSQL = sprintf("INSERT INTO invoicing_commodityitem (list_id, itemname, subitem_id, lang, userid) VALUES (%s, %s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_CommodityItemEdit")) {
	foreach($_POST['item_id'] as $key => $val){
	  $updateSQL = sprintf("UPDATE invoicing_commodityitem SET list_id=%s, sortid=%s, indicate=%s, itemname=%s, lang=%s WHERE item_id=%s",
...
if ((isset($_POST['deltype'])) && ($_POST['deltype'] != "")) {
	...
	$MM_flag="MM_update";
		...
		foreach($_POST['deltype'] as $key => $val){
		  $MM_dupKeyRedirect="manage_commodity.php?wshop=" . $wshop . "&Opt=mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&Operate=delErrorP";
		  $loginUsername = $_POST['deltype'][$key];
		  $LoginRS__query = sprintf("SELECT * FROM invoicing_commodity WHERE type1=%s", GetSQLValueString($loginUsername, "int")); // 分類
		  //mysqli_select_db($database_DB_Conn, $DB_Conn);
	...
	if (isset($_POST[$MM_flag])) {
		foreach($_POST['deltype'] as $key => $val){
		  $MM_dupKeyRedirect="manage_commodity.php?wshop=" . $wshop . "&Opt=mulilistitempage&lang=" . $_GET['lang'] . "&list_id=" . $_GET['list_id'] . "&Operate=delErrorT";
		  $loginUsername = $_POST['deltype'][$key];
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordCommodityListItem = sprintf("SELECT invoicing_commodityitem.item_id, invoicing_commodityitem.userid, invoicing_commoditylist.list_id, invoicing_commoditylist.listname, invoicing_commodityitem.itemname, invoicing_commodityitem.sortid, invoicing_commodityitem.indicate, invoicing_commodityitem.lang FROM invoicing_commoditylist LEFT OUTER JOIN invoicing_commodityitem ON invoicing_commoditylist.list_id = invoicing_commodityitem.list_id WHERE invoicing_commoditylist.list_id = %s && invoicing_commodityitem.lang=%s && invoicing_commodityitem.level=0 && invoicing_commodityitem.userid=%s ORDER BY invoicing_commodityitem.sortid ASC, invoicing_commodityitem.item_id DESC", GetSQLValueString($collistid_RecordCommodityListItem, "int"),GetSQLValueString($collang_RecordCommodityListItem, "text"),GetSQLValueString($coluserid_RecordCommodityListItem, "int"));
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <div class="panel-heading">
    ...
    <div class="btn-group pull-right"><a href="manage_commodity.php?wshop=<?php echo $wshop;?>&amp;Opt=listpage&amp;lang=<?php echo $_SESSION['lang']; ?>" data-original-title="" data-toggle="tooltip" data-placement="top" class="btn btn-default btn-sm"><i class="fa fa-reply fa-fw"></i> 上一頁</a></div>
  ...
  <div class="panel-body">
    ...
    <form id="form_CommodityItemEdit" name="form_CommodityItemEdit" method="POST" action="manage_commodity.php?wshop=<?php echo $wshop;?>&amp;Opt=<?php echo $_GET['Opt'] ?>&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;list_id=<?php echo $_GET['list_id']; ?>" class="form-horizontal form-bordered" data-parsley-validate="">
    <?php do { ?>
...
<div class="panel panel-inverse bg-white-transparent-9"> 
  ...
  <!-- begin panel-body -->
  <div class="panel-body">
      <form id="form_CommodityItemAdd" name="form_CommodityItemAdd" method="POST" action="manage_commodity.php?wshop=<?php echo $wshop;?>&amp;Opt=<?php echo $_GET['Opt'] ?>&amp;lang=<?php echo $_SESSION['lang']; ?>&amp;list_id=<?php echo $_GET['list_id']; ?>" class="form-horizontal form-bordered" data-parsley-validate="">
      <div class="card bg-aqua-transparent-1">
          <div class="card-block">
              <div class="row">

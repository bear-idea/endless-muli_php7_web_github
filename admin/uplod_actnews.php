...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
	...
	$ppu->path = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/actnews";
...
if ((isset($_POST["MM_update"])) && isset($_GET['GP_upload'])) {
  ...
  $sip->quality = "100";
  $sip->makeThumb = "true";
  $sip->pathThumb = $SiteImgFilePathAdmin . $_POST['wshop'] . "/image/actnews/thumb";
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form_Actnews")) {
  $updateSQL = sprintf("UPDATE demo_actnews SET pic=IFNULL(%s,pic) WHERE id=%s",
  ...
  /* 刪除原先圖片 */
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/actnews/' . $_POST['oldpic']);
  @unlink($SiteImgFilePathAdmin . $_POST['wshop'] . '/image/actnews/thumb/small_' . GetFileThumbExtend($_POST['oldpic']));
  
  $updateGoTo = "uplod_actnews.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordActnews = sprintf("SELECT * FROM demo_actnews WHERE id = %s", GetSQLValueString($colname_RecordActnews, "int"));
...
$(document).ready(function() {
	$("#pic").fileinput({
		...
		initialPreview: ["<?php echo $SiteImgUrlAdmin; ?><?php echo $wshop; ?>/image/actnews/thumb/small_<?php echo $row_RecordActnews['pic']; ?>"],
		<?php } ?>

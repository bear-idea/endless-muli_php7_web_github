...
$query_RecordMultiLeftMenu_l1 = sprintf("SELECT * FROM demo_aboutitem WHERE list_id = 1 && lang = %s && level = '0' && indicate = '1' && userid=%s ORDER BY sortid ASC, item_id DESC", GetSQLValueString($collang_RecordMultiLeftMenu_l1, "text"),GetSQLValueString($coluserid_RecordMultiLeftMenu_l1, "int"));
...
<ul class="nav flex-column">
        <?php do { ?>
            ...
            <a class="menu-link" href="manage_about.php?wshop=<?php echo $wshop;?>&amp;Opt=viewpage_sub&lang=<?php echo $_GET['lang']; ?>&level=<?php echo $row_RecordMultiLeftMenu_l1['level']; ?>&type1=<?php echo $row_RecordMultiLeftMenu_l1['item_id']; ?>&item_id=<?php echo $row_RecordMultiLeftMenu_l1['item_id']; ?>"><?php echo $row_RecordMultiLeftMenu_l1['itemname']; ?>
            </a>
				    <?php
					 #
...
//mysqli_select_db($database_DB_Conn, $DB_Conn);
$query_RecordMultiLeftMenu_l2 = sprintf("SELECT * FROM demo_aboutitem WHERE list_id = 1 && lang = %s && level = '1' && subitem_id = %s && indicate = '1' && userid=%s ORDER BY sortid ASC, item_id DESC", GetSQLValueString($collang_RecordMultiLeftMenu_l2, "text"),GetSQLValueString($colsubitem_id_RecordMultiLeftMenu_l2, "int"),GetSQLValueString($coluserid_RecordMultiLeftMenu_l2, "int"));
